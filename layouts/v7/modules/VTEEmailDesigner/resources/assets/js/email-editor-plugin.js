...
(function (factory) {
    ...
    EmailBuilder.prototype = {
        defaults: {
            ...
            registerChangeEventForModule: function (e) {},
            registerFillTemplateContentEvent: function (e) {},
            registerEventLoadTemplateById: function (e) {},
        },
        /**
         * Init Plugin
         */
        init: function () {
            ...
            //_this.config.registerChangeEventForModule();
        ...
        generate: function (elementsHtml) {
            ...
            if (_this.config.showDetailTab == true) {
                _nav += '<li class="menu-item tab-selector" data-tab-selector="tab-detail">' +
                    '<i class="fa fa-pencil"></i>' +
                    '<span class="menu-name">' + _this.langArr.detailTab + '</span>' +
                    '</li>'+
                    '<li class="menu-item setting-item preview" data-tab-selector="tab-preview" >' +
                    '<i class="fa fa-eye"></i>' +
                    '<span class="menu-name">Preview</span>' +
                    '</li>'+
                    '<li class="menu-item setting-item save-template" data-tab-selector="tab-preview" >' +
                    '<i class="fa fa-floppy-o"></i>' +
                    '<span class="menu-name">Save</span>' +
                    '</li>';
            }
            ...
            _tabElements = _this.config.elementsHTML;
            _tabProperty = _this.config.elementsProperty;
            ...
            _this.config.registerChangeEventForModule();
            _this.config.registerFillTemplateContentEvent();
            _this.config.registerEventLoadTemplateById();
        ...
        default_func: function () {
            ...
            _this.$elem.find('#bg-color-inner').ColorPicker({
                ...
                onChange: function (hsb, hex, rgb) {
                    ...
                    $('#bg-color-inner').css('background-color', '#' + hex);
                    $('[name="bg_color_inner"]').val('#' + hex);
                }
            });
            _this.$elem.find('#bg-color-outer').ColorPicker({
                ...
                onChange: function (hsb, hex, rgb) {
                    ...
                    $('#bg-color-outer').css('background-color', '#' + hex);
                }
            });
            ...
            _this.$elem.find('#button-bg-color').ColorPicker({
                ...
                onChange: function (hsb, hex, rgb) {
                    ...
                    _this.getCurContentSelection().find('a').css('background-color', '#' + hex);
            ...
            _this.$elem.find('#button-text-color').ColorPicker({
                ...
                onChange: function (hsb, hex, rgb) {
                    ...
                    _this.getActiveElementContent().find('#curContentSelection.button-link').css('color', '#' + hex);
                    //_this.changeSettings($('#bg-color').attr('setting-type'), '#' + hex);
                }
            });
            //button-bg-color
         ...
         *  Get content active element for change setting
         */
        getCurContentSelection: function () {
            _element = _this.$elem.find('.sortable-row.active .sortable-row-content .element-content');
            ...
            if (_element.find('[contenteditable="true"]').hasClass('element-contenteditable')) {
                ...
                _element = _element.find('#curContentSelection').closest('td');
            }
        ...
        events: function () {
            ...
            _this.$elem.find('.elements-accordion .elements-accordion-item-title').on('click', function (j) {
                ...
                var liAccordion = _element.closest('.elements-accordion-item');
                var datatype = liAccordion.data('type');
                if(datatype=='email-template'){
                    var elTitle = liAccordion.find('.elements-accordion-item-title');
                    if(elTitle.hasClass('focus-email-template')){
                        var elContent = liAccordion.find('.elements-accordion-item-contentA');
                        elContent.css('display','none');
                        elContent.removeClass('elements-accordion-item-contentA');
                        elContent.addClass('elements-accordion-item-content');
                        elTitle.removeClass('focus-email-template');
                        elTitle.removeClass('active');
                    }else {
                        elTitle.addClass('focus-email-template');
                        var dropDown = liAccordion.find('.elements-accordion-item-content');
                        dropDown.removeClass('elements-accordion-item-content');
                        dropDown.addClass('elements-accordion-item-contentA');

                    }
                }else {
                    var elTitle =  _this.$elem.find('.tab-property .elements-accordion .elements-accordion-item[data-type="email-template"] .elements-accordion-item-title');
                    elTitle.removeClass('active');
                    elTitle.removeClass('focus-email-template');
                    var dropDown = _this.$elem.find('.tab-property .elements-accordion .elements-accordion-item[data-type="email-template"] .elements-accordion-item-contentA');
                    dropDown.css('display','none');
                    dropDown.removeClass('elements-accordion-item-contentA');
                    dropDown.addClass('elements-accordion-item-content');
                }
            });
            ...
            _this.$elem.find('.blank-page').on('click', function () {
                ...
                swal({
                    ...
                    _this.makeSortable();
                    _this.remove_row_elements();
                    _this.$elem.find('.tab-detail .elements-accordion-item input[name="emailtemplateid"]').val('');
                    _this.$elem.find('.tab-detail .elements-accordion-item input[name="templatename"]').val('');
                    _this.$elem.find('.tab-detail .elements-accordion-item input[name="subject"]').val('');
                    _this.$elem.find('.tab-detail .elements-accordion-item select[name="modulename"]').val('');
            ...
            _this.$elem.on('click', '.content-wrapper', function (event) {
                ...
                _this.$elem.find('select[name="modulename"]').on('change',function(e){
                    if (_this.config.registerChangeEventForModule !== undefined) {
                        _this.config.registerChangeEventForModule(event);
                    }
                });
                _this.$elem.find('select[name="templateFields"]').on('change',function(e){
                    if (_this.config.registerFillTemplateContentEvent !== undefined) {
                        _this.config.registerFillTemplateContentEvent(event);
                    }
                });
                _this.$elem.find('select[name="generalFields"]').on('change',function(e){
                    if (_this.config.registerFillTemplateContentEvent !== undefined) {
                        _this.config.registerFillTemplateContentEvent(event);
                    }
                });
        ...
        events_of_row: function () {
            //remove button
            _this.$elem.on('click', '.sortable-row .row-remove, .sortable-row .fa-remove', function (e) {
            ...
            //duplicate button
            _this.$elem.on('click', '.sortable-row .row-duplicate, .sortable-row .fa-files-o', function (e) {
            ...
            _this.$elem.on('click', '.sortable-row .row-code, .sortable-row .fa-code', function (e) {
            ...
            _this.$elem.on('click', '.email-editor-elements-sortable *', function (event) {
                _this.$elem.find('select[name="templateFields"]').on('change',function(e){
                    if (_this.config.registerFillTemplateContentEvent !== undefined) {
                        _this.config.registerFillTemplateContentEvent(event);
                    }
                });
                jQuery('*[id*=curContentSelection]').each(function(e) {
                    jQuery(this).removeAttr('id');
                });
                _this.$elem.find('#curContentSelection').removeAttr('id');
                _this.$elem.find('[contenteditable="true"]').removeClass('element-contenteditable active');

                if (jQuery(this).attr('contenteditable')) {
                    jQuery(this).addClass('element-contenteditable active');
                }else {
                    jQuery(this).closest('td').addClass('element-contenteditable active');
                }
                _this.sortableClick(jQuery(this));
                ...
                if ($(currentElement).children().length > 0 ) {
                    ...
                    if($(currentElement).is('br')){
                        ...
                        currentElement.setAttribute('id','curContentSelection');
                        _this.getContentSelectionStart(jQuery(this));
                    }else {
                        currentElement.attr('id','curContentSelection');
                        _this.getContentSelectionStart(currentElement);
                    }
                }else {
                    currentElement.setAttribute('id','curContentSelection');
                    _this.getContentSelectionStart(jQuery(this));
            ...
            _this.$elem.on('click', '[contenteditable="true"]', function (event) {
               ...
               // _this.$elem.find('.content [contenteditable="true"] #curContentSelection').removeAttr('id');
        ...
        events_of_property: function () {
            ...
            _this.$elem.on('keyup', '.email-width', function (event) {
                ...
                jQuery('.content-wrapper').attr('data-width', _val);
                jQuery('.content-main').attr('data-width', _val);
                jQuery('.main').css('width', _val + 'px');
                jQuery('.sortable-row').css('width', _val + 'px');
            });
            ...
            _this.$elem.on('keyup', '.elements-accordion-item-content .button-text', function (event) {
                _element = jQuery(this);
                _val = _element.val();
                _activeElement = _this.getCurContentSelection();
                var _buttonlink = _activeElement.find('#curContentSelection.button-link');
                if(_buttonlink.length==1){
                    _buttonlink.text(_val);
                }else {
                    _buttonlink.first().text(_val);
                }
            });
            _this.$elem.on('keyup', '.elements-accordion-item-content .button-hyperlink', function (event) {
                ...
                _val = _element.val();
                _activeElement = _this.getActiveElementContent();
                var _elelink = _activeElement.find('a');
                if(_element.find('a').is(':has(#curContentSelection)')){
                    _elelink = _element.find('a:has(#curContentSelection)');
                }
                if(_elelink.length==1){
                    _elelink.attr('href', _val);
                }else {
                    _elelink.first().attr('href', _val);
                }
            });
            ...
            _this.$elem.on('keyup', '.elements-accordion-item-content .youtube', function (event) {
              ...
              //
                var aLink='<a href="http://www.youtube.com/watch?v='+_this.getYoutubeVideoId(_val)+'&feature=em-share_video_user" style="text-decoration:none;" class="nonplayable-ytb" target="_blank">'+
                    '<span style="display: block; width: 498px;height: 274px;background-image: url(https://img.youtube.com/vi/'+_this.getYoutubeVideoId(_val)+'/sddefault.jpg);background-position: center;">'+
                    '<img src="https://www.vtexperts.com/files/youtubeplay.png" height="56" width="80" style="margin-top: 116px; margin-left: 199px; width: 80px; height: 56px;"></span>'+
                    '</a>';
            ...
            _this.$elem.on('keyup', '.social-content-box .social-input', function (event) {
                _element = jQuery(this);
                _socialType = _element.parents('div').attr('data-social-type');
                _val = _element.val();
                _activeElement = _this.getActiveElementContent();
            ...
            _this.$elem.on('keyup', '.image-size', function (event) {
                ...
                if (jQuery(this).hasClass('image-height')) {
                    var heightVal = jQuery(this).val();
                    heightVal = heightVal.replace(/[^0-9]/g,'');
                    _activeElement.find('.content-image').css('height', jQuery(this).val());
                    _activeElement.find('.content-image').attr('height', heightVal);
                } else if (jQuery(this).hasClass('image-width')) {
                    var widthVal = jQuery(this).val();
                    widthVal = widthVal.replace(/[^0-9]/g,'');
                    _activeElement.find('.content-image').css('width', jQuery(this).val());
                    _activeElement.find('.content-image').attr('width', widthVal);
                }
                else if (jQuery(this).hasClass('image-url')) {
                  _activeElement.find('.image-hyperlink').attr('href', jQuery(this).val());
                  _activeElement.find('.content-image').attr('src', jQuery(this).val());
                }
            });
        ...
        menuAccordionClick: function (_element, toggle) {
            ...
            _element.closest('.elements-accordion').find('.elements-accordion-item-content').removeClass('active');
            _element.closest('.elements-accordion').find('.elements-accordion-item-content').not(dropDown).slideUp();

            if (!toggle) {
                _element.closest('.elements-accordion').find('.elements-accordion-item .elements-accordion-item-title.active').not('.elements-accordion-item[data-type="email-template"] .elements-accordion-item-title.active').removeClass('active');
                _element.addClass('active');
                dropDown.stop(false, true).slideToggle();
                /*setTimeout(function(){
                    var accordionMenuItemContent = _this.$elem.find('.tab-property .elements-accordion .elements-accordion-item[data-type="email-template"] .elements-accordion-item-content');
                    accordionMenuItemContent.css('display', 'block');
                },1000);*/
            } else {
                if (_element.hasClass('active')) {
                    _element.removeClass('active');
        ...
        getContentHtml: function () {
            ...
            var demoH=$('<div/>');
            demoH.html(_html);
            demoH.find('.nonplayable-ytb').css('display','none');
        ...
        node_walk: function (node, func) {
            ...
            for(node = node.firstChild; result !== false && node; node = node.nextSibling)
                result = _this.node_walk(node, func);
            return result;
        },
        ...
        getCaretPosition: function (elem) {
            ...
            if(sel.anchorNode == elem && sel.anchorNode.nodeValue !== null ) {
                cum_length = [sel.anchorOffset, sel.extentOffset];
                var eleContent = $("<div>").text(elem.firstChild.innerHTML).html();
                var noteContent = sel.anchorNode.nodeValue.trim();
                var noteIndex = eleContent.indexOf(noteContent);
                cum_length[0] = noteIndex;
                cum_length[1] = noteIndex;
            }else {
                ...
                if(!elem.contains(sel.anchorNode) || !elem.contains(sel.extentNode))
                    ...
                    var found = [0,0];
                    var i;
                    _this.node_walk(elem, function(node) {
                        for(i = 0; i < 2; i++) {
                        ...
                        if(node.innerHTML && !node.firstChild) {
                            for(i = 0; i < 2; i++) {
                                if(!found[i]) {
                                    var content = $("<div>").text(node.innerHTML).html();
                                    cum_length[i] += content.length;
                                }
                            }
                        }
                    });
            ...
            var eleContent = '';
            if(elem.firstChild){
                eleContent = $("<div>").text(elem.firstChild.innerHTML).html();
            }
            var nodeValue = sel.anchorNode.nodeValue;
            var noteContent ='';
            var noteIndex =0;
            var anchorNoteLength =0;
            if(nodeValue != null) {
                noteContent = $("<div>").text(nodeValue).html();
                noteContent = $("<div>").text(noteContent).html();
                noteIndex = eleContent.indexOf(noteContent);
                anchorNoteLength = nodeValue.length;
            }
            if(noteIndex<0) noteIndex=0;
            var curNoteIndex0=0;
            var curNoteIndex1=0;
            var nodeValueLength = noteContent.length;

            if(anchorNoteLength!=nodeValueLength) {
                curNoteIndex0 = nodeValueLength - (anchorNoteLength - cum_length[0]);
                curNoteIndex1 = nodeValueLength - (anchorNoteLength - cum_length[1]);
            }
            if(noteIndex>=0 && curNoteIndex0>0)
                cum_length[0]=noteIndex+curNoteIndex0;
            if(noteIndex>=0 && curNoteIndex1>0)
                cum_length[1]=noteIndex+curNoteIndex1;
            if(cum_length[0] <= cum_length[1])
                return cum_length;
            return [cum_length[1], cum_length[0]];
        },
        /**
         * Content row mouseout event
         */
        getContentSelectionStart: function (_thisElement) {
            var update = function() {
                var selectionId = this.id;
                if(selectionId=='' || (typeof selectionId !='undefined' && selectionId=='curContentSelection')){
                    var pos = _this.getCaretPosition(this);
                    if(typeof pos !='undefined') {
                        _this.$elem.find('.tab-property .elements-accordion-item input[name="contentSelectionStart"]').val(_this.getCaretPosition(this));
                    }
                }
            };
            $('#curContentSelection').closest('td').on("mouseout", update);
        },
        /**
        ...
        getSettings: function () {
            var intervalRunDefaultFunc = setInterval(function () {
                if (jQuery('#curContentSelection').length > 0){
                    ...
                    var curElement = _element.find('a');
                    if(_element.find('a').is(':has(#curContentSelection)')){
                        curElement = _element.find('a:has(#curContentSelection)');
                    }
                    if(curElement.length>1){
                        curElement = curElement.first();
                    }
                    _href = curElement.text();
                    _this.$elem.find('.button-text').val($.trim(_href)+'');

                    _href = curElement.attr('href');
        ...
        tinymceContextMenu: function () {
            ...
            $.when(
                $.getScript( "layouts/v7/modules/VTEEmailDesigner/resources/assets/vendor/tinymce/js/tinymce/themes/inlite/theme.js" ),
                $.getScript( "layouts/v7/modules/VTEEmailDesigner/resources/assets/vendor/tinymce/js/tinymce/plugins/link/plugin.js" ),
                $.getScript( "layouts/v7/modules/VTEEmailDesigner/resources/assets/vendor/tinymce/js/tinymce/plugins/textcolor/plugin.js" ),
