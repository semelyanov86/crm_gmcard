<form class="form-horizontal recordEditView" id="pivot_report_step2" method="post" action="index.php">
    ...
    <textarea style="display: none" name="content_mail">{Vtiger_Util_Helper::toSafeHTML($REPORT_MODEL->get('content_mail'))}</textarea>
    ...
    <div class="" style="border:1px solid #ccc;padding:2%;">
        <div class="row">
            ...
            <span class="col-lg-12" id="filter_conditions">
                {if is_array($SELECTED_ADVANCED_FILTER_FIELDS)}
                    {assign var=COUNT_SELECTED_ADVANCED_FILTER_FIELDS value=count($SELECTED_ADVANCED_FILTER_FIELDS)}
                {else}
                    {assign var=COUNT_SELECTED_ADVANCED_FILTER_FIELDS value=0}
                {/if}
                     {if $COUNT_SELECTED_ADVANCED_FILTER_FIELDS > 0}
