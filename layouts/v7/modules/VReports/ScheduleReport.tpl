...
{strip}
    ...
    {if $show_report_scheduled eq true}
        <div id="scheduleBox" class='row well contentsBackground {if $SCHEDULEDREPORTS->get('scheduleid') eq ''} hide {/if}'>
            ...
            {* email address send mail *}
            {*<div class='col-lg-12' id='div_from_address' style='padding:5px 0px 10px 0px;'>
                <div class='col-lg-3' style='position:relative;top:5px;'>
                    {vtranslate('FROM_ADDRESS', $MODULE)}
                </div>
                <div class='col-lg-4'>
                    <input id="from_address" style="width: 281px !important;" class="col-lg-6 inputElement" type="text" value="{$SCHEDULEDREPORTS->get('from_address')}" name="from_address"/>
                </div>
            </div>
            {* subject mail*}
            <div class='col-lg-12' id='div_subject_mail' style='padding:5px 0px 10px 0px;'>
                <div class='col-lg-3' style='position:relative;top:5px;'>
                    {vtranslate('SUBJECT_MAIL', $MODULE)}
                </div>
                <div class='col-lg-4'>
                    <input id="subject_mail" style="width: 281px !important;" class="col-lg-6 inputElement" type="text" value="{$SCHEDULEDREPORTS->get('subject_mail')}" name="subject_mail"/>
                    <button type="button" id="selectEmailTemplate" class="btn btn-success pull-right" data-url="module=EmailTemplates&view=Popup">Select Email Template</button>
                </div>
            </div>
            {* content mail *}
            <div class='col-lg-12' id='div_content_mail' style='padding:5px 0px 10px 0px;'>
                <div class='col-lg-3' style='position:relative;top:5px;'>
                    {vtranslate('BODY_MAIL', $MODULE)}
                </div>
                <div class='col-lg-4'>
                    <textarea id="content_mail" style="width: 100% !important; height: 100px; padding: 3px 8px" class="col-lg-12 inputElement" type="text" name="content_mail">
                         {$SCHEDULEDREPORTS->get('content_mail')}
                    </textarea>
                </div>
            </div>
            <div class='col-lg-12' id='div_signature_mail' style='padding:5px 0px 10px 0px;'>
                <div class='col-lg-3' style='position:relative;top:5px;'>
                    {vtranslate('Include Signature', $MODULE)}
                    <input class="pull-right" type="checkbox" name="signature" value="Yes" id="signature" {if $SCHEDULEDREPORTS->get('signature') == 'Yes'}checked="checked"{/if}/>
                </div>
                <div class='col-lg-4'>
                    <span id="span_signature_user" {if $SCHEDULEDREPORTS->get('signature') != 'Yes'}style="display: none" {/if}>
                    <select class="select2" id='signature_user' name='signature_user' style="width: 280px !important;">
                        {foreach key=USER_ID item=USER_NAME from=$ALL_ACTIVEUSER_LIST}
                            <option value="{$USER_ID}" {if $SCHEDULEDREPORTS->get('signature_user') == $USER_ID}selected="selected"{/if}> {$USER_NAME} </option>
                        {/foreach}
                    </select>
                    </span>
                </div>
            ...
            {if $SCHEDULEDREPORTS->get('next_trigger_time')}
                <div class="col-lg-12" style="padding:5px 0px 10px 0px;">
                    ...
                    <div class='col-lg-5'>
                        {$SCHEDULEDREPORTS->getNextTriggerTimeInUserFormat()}<span>&nbsp;({$ACTIVE_ADMIN->time_zone})</span>
                    </div>
                </div>
            {/if}
        </div>
