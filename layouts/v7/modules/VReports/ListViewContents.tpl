					...
					<thead>
						<tr class="listViewContentHeader">
							...
							{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY from=$LISTVIEW_HEADERS}
								<th {if $COLUMN_NAME eq $LISTVIEW_HEADER_KEY} nowrap="nowrap" {/if} {if $LISTVIEW_HEADER_KEY eq 'reportname' }style="width: 30%!important;" {/if}>
									{if $LISTVIEW_HEADER_KEY eq 'schedule'}
								...
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY from=$LISTVIEW_HEADERS}
									<th>
										...
										{elseif $DATA_TYPE == 'picklist'}
											{assign var=PICKLIST_VALUES value=VReports_Field_Model::getPicklistValueByField($LISTVIEW_HEADER_KEY)}
											{assign var=SEARCH_VALUES value=explode(',',$SEARCH_DETAILS[$LISTVIEW_HEADER_KEY]['searchValue'])}
											{if $LISTVIEW_HEADER_KEY eq 'modifiedtime'}
												{continue}
											{/if}
											<div class="row-fluid">
												<select class="select2 listSearchContributor report-type-select" name="{$LISTVIEW_HEADER_KEY}" multiple data-fieldinfo='{$FIELD_INFO|escape}'>
													{foreach item=PICKLIST_LABEL key=PICKLIST_KEY from=$PICKLIST_VALUES}
														{if $PICKLIST_LABEL eq 'Chart'}
															{assign var="ICON_CLASS" value='fa fa-pie-chart greenColor'}
                                                        {elseif $PICKLIST_LABEL eq 'Pivot'}
                                                            {assign var="ICON_CLASS" value='fa fa-table fa-2x blueColor'}
														{elseif $PICKLIST_LABEL eq 'Detail'}
															{assign var="ICON_CLASS" value='vicon-detailreport'}
														{elseif $PICKLIST_LABEL eq 'Sql Report'}
															{assign var="ICON_CLASS" value='vicon-list'}
														{/if}
														{if $PICKLIST_LABEL eq 'Sql Report' && $CURRENT_USER_MODEL->user_name neq 'admin'}
															{continue}
														{else}
						...
						{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
							<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
								...
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY  from=$LISTVIEW_HEADERS}
									...
									<td class="listViewEntryValue" style="vertical-align: top;" data-name="{$LISTVIEW_HEADERNAME}" title="{$LISTVIEW_ENTRY_RAWVALUE}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="">
											...
											<span class="value">
												{if $LISTVIEW_HEADERNAME eq 'reporttype'}
													...
													{elseif $LISTVIEW_ENTRY_VALUE eq 'chart'}
														<center title="{vtranslate('LBL_CHART_REPORT', $MODULE)}"><span class='fa fa-pie-chart fa-2x greenColor' style="font-size:1.7em;"></span></center>
													{elseif $LISTVIEW_ENTRY_VALUE eq 'sql'}
														<center title="{vtranslate('LBL_SQL_REPORT', $MODULE)}"><span class='vicon-list' style="font-size:1.7em;"></span></center>
													{else}
														<center title="{vtranslate('LBL_PIVOT_REPORT', $MODULE)}"><span class='fa fa-table fa-2x blueColor' style="font-size:1.7em;"></span></center>
													{/if}
												{elseif $LISTVIEW_HEADERNAME eq 'primarymodule'}
													<span title="{Vtiger_Util_Helper::tosafeHTML(decode_html(vtranslate($LISTVIEW_ENTRY_VALUE, $LISTVIEW_ENTRY_VALUE)))}">{Vtiger_Util_Helper::tosafeHTML(decode_html(vtranslate($LISTVIEW_ENTRY_VALUE, $LISTVIEW_ENTRY_VALUE)))}</span>
												{elseif $LISTVIEW_HEADERNAME eq 'foldername'}
													<span title="{Vtiger_Util_Helper::tosafeHTML(vtranslate($LISTVIEW_ENTRY_VALUE, $MODULE))}">{Vtiger_Util_Helper::tosafeHTML(vtranslate($LISTVIEW_ENTRY_VALUE, $MODULE))}</span>
												{else}
													{if $LISTVIEW_HEADERNAME eq 'reportname'}
														<a href="{$LISTVIEW_ENTRY->getDetailViewUrl()}" target="_blank">
													{/if}
													<span {if $LISTVIEW_HEADERNAME eq 'modifiedtime'} style="white-space: nowrap" {/if} title="{Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE)}">
														{if $LISTVIEW_HEADERNAME eq 'sharingtype'}
															{assign var="owner" value="{Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY->get('owner'))}"}
															{assign var="sharingtypevalue" value={Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE)}}
															{assign var=identified value=","|explode:"$sharingtypevalue"}
															{assign var=exclude value=$identified[0]|cat:', '|cat:$owner|cat:', '}
															{assign var=ortheruser value="{str_replace($exclude ,'',$sharingtypevalue)}"}
															<span style="white-space: nowrap" title="{$ortheruser}">
																<i class="fa fa-{if $identified[0] eq 'Private'}lock{else}unlock{/if}">&nbsp;</i>{$owner}
															</span>
														{elseif strlen(Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE)) > $LISTVIEW_MAX_TEXTLENGTH}
															{textlength_check(Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE))}
														{else}
													...
													{if $LISTVIEW_HEADERNAME eq 'reportname'}
														...
														<div>
															{if strlen(Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY->get('description'))) > 200}
																<span title="{substr(Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY->get('description')), 0, 200)}">
																	<var class="italic_small_size" style="color: #666">
																		{substr(decode_html($LISTVIEW_ENTRY->get('description')), 0, 200)}
																	</var>
																</span>
															{else}
																<span title="{Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY->get('description'))}">
																	<var class="italic_small_size" style="color: #666">
																		{decode_html($LISTVIEW_ENTRY->get('description'))}
																	</var>
																</span>
															{/if}
														</div>
