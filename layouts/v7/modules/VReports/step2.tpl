...
{strip}
    <form class="form-horizontal recordEditView" id="report_step2" method="post" action="index.php">
        ...
        <input type="hidden" name="from_address" value="{$REPORT_MODEL->get('from_address')}">
        ...
        <div class="" style="border:1px solid #ccc;padding:4%;">
            <div class="form-group">
                <label>{vtranslate('LBL_SELECT_COLUMNS',$MODULE)}</label>
                    ...
                    <table class="table table-bordered CalculationFields" width="100%">
                        <thead>
                            <tr class="calculationHeaders blockHeader">
                                <th>{vtranslate('LBL_RENAME_COLUMNS',$MODULE)}</th>
                        ...
                        {foreach key=CALCULATION_FIELDS_MODULE_LABEL item=CALCULATION_FIELDS_MODULE from=$CALCULATION_FIELDS}
                            {foreach key=CALCULATION_FIELD_KEY item=CALCULATION_FIELD from=$CALCULATION_FIELDS_MODULE}
                                ...
                                {assign var=RENAME_STATUS value=$SELECTED_CALCULATION_FIELDS_RENAME_COLUMNS[$columnName]['rename_status']}
                                {assign var=RENAME_COLUMN value=$SELECTED_CALCULATION_FIELDS_RENAME_COLUMNS[$columnName]['rename']}

                                <tr class="calculationFieldRow">
                                    <td width="5%">
                                        <input class="calculationType" name="rename_columns_selected[{$columnName}]" type="checkbox" {if $RENAME_STATUS eq TRUE} checked="" {/if} value="true"/>
                                    </td>
                                    <td>
                                        <span class="default-columns-name" {if $RENAME_STATUS eq TRUE} style="display: none" {/if}>{vtranslate($CALCULATION_FIELDS_MODULE_LABEL,$MODULE)}-{vtranslate($CALCULATION_FIELD,$CALCULATION_FIELDS_MODULE_LABEL)}</span>
                                        <input type="text" class="inputElement" name="rename_columns[{$columnName}]" value="{if !empty($RENAME_COLUMN)} {$RENAME_COLUMN} {else} {vtranslate($CALCULATION_FIELDS_MODULE_LABEL,$MODULE)}-{vtranslate($CALCULATION_FIELD,$CALCULATION_FIELDS_MODULE_LABEL)}{/if}" {if $RENAME_STATUS neq TRUE } style="display: none"{/if}>
                                    </td>
