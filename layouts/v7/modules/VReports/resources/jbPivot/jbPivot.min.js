    ...
    $.widget("unc.jbPivot", {
                    ...
                    if (this.indexes.hasOwnProperty(mask)) {
                        ...
                        for (f = 0; f < this.afields.length; f++) {
                            if (this.afields[f] !== null) {
                                ...
                                if(this.indexes[mask][groupidx][f].decimalSeparator){
                                    this.decimalSeparator = this.indexes[mask][groupidx][f].decimalSeparator;
                                }
                                if(this.indexes[mask][groupidx][f].digitGroupingSeparator){
                                    this.digitGroupingSeparator = this.indexes[mask][groupidx][f].digitGroupingSeparator;
                                }
                            }
                        }
                    }
        ...
        _tree2table: function(axis, node, table) {
            ...
            if (this._isCollapsed(axis, table[y].index)) {
                ...
                // table[y].cells[x].isCollapsed = true;
                m = 1;
            } else {
                ...
                // table[y].cells[x].isCollapsed = false;
        ...
        _renderHtml: function() {
            ...
            var datetime = "Start Generate: " + currentdate.getDate() + "/"
            ...
            S += "<th id=\"groupAllLabelTh\" style='padding:0;background: #F4F4F4;border: 1px solid #a6a6a6;' width='100' height='50' colspan=\"" + (this.xfields.length + 1) + "\" rowspan=\"" + (this.yfields.length + 2) + "\"";
            ...
            var widthLabelRow = 10;
            if(this.xfields.length == 2){
                widthLabelRow = 17;
            }else if(this.xfields.length == 3){
                widthLabelRow = 24;
            }
            S += '<div style="height: 50%;position: relative;"><div class="groupAllLabel" style="position: absolute;bottom: 0;right: 0;"><a style="margin-right: 0;" href="#" class="collapse_button" rel="y">';
            S += this._isCollapsed("y", []) ? "+" : "-";
            S += '</a></div></div><div style="height: 50%;position: relative;width:'+widthLabelRow+'vw;"><div class="groupAllLabel" style="position: absolute;bottom: 0;width: 100%;text-align: center;"><a style="margin-right: 0;" href="#" class="collapse_button" rel="x">';
            S += this._isCollapsed("x", []) ? "+" : "-";
            S += '</a></div></div></th>';
            colspan = tabley.length * (this.zfields.length > 0 ? this.zfields.length : 1);
            if(this.CollapsedNodes['y']){
            ...
            for (y = 1; y <= this.yfields.length; y++) {
                ...
                var hideTotal = true;
                // for (x = 0; x < this.options.data.length ; x++) {
                //     if (typeof this.options.data[x].currency_field == "undefined"){
                //         hideTotal = false;
                //     }
                // }
                if((tabley.length>1 || this.zfields.length>1) && hideTotal){
                    S += "<td style='background:#F4F4F4;border: 1px solid #a6a6a6' class='summary line_top_3 line_bottom_3 line_left_0  line_right_1'";
                    S += " >";
                    S += "</td>";
                }
                S += "</tr>\n";
            ...
            //     }
            // }
            if((this.zfields.length>1 || tabley.length >1) && hideTotal){
                S += "<th style='background:#F4F4F4;border: 1px solid #a6a6a6' class='summary line_top_3 line_bottom_3 line_left_0  line_right_1'";
                S += " >";
                S += "</th>";
            }
            ...
            var TotalRowLevel = [];
            var classFindRemove = [];
            var countClassFind = 0;
            var breakY = false;
            // var xfieldsLength = this.xfields.length;
            for (y = 0; y < tablex.length; y++) {
                var row = '';
                var rowth = ''
                var rowtd = ''
                var valueBolean = false;
                for (x = 0; x <= this.xfields.length; x++) {
                    if (tablex[y].cells[x]) {
                        if(x == 1){
                            if (y + tablex[y].cells[x].spany > 64000){
                                breakY = true;
                            }
                        }
                        // TotalRowLevel[x] = {'total' : tablex[y].cells[x].spany,
                        //     'count' : 1,
                        //     'bolean': false,
                        //     'class' : "level" + x + "_row" + y,
                        //     'is_collapsed' : tablex[y].cells[x].isCollapsed,
                        // };
                        rowth += "<th";

                        rowth += tablex[y].cells[x].spanx > 1 ? " colspan=\"" + tablex[y].cells[x].spanx + "\"" : "";
                        rowth += tablex[y].cells[x].spany > 1 ? " rowspan=\"" + tablex[y].cells[x].spany + "\"" : "";
                        ...
                        if (x === 0) {
                            cls += " line_left_" + cmax;
                            if(!this.CollapsedNodes['x']){
                                cls += " border_right_white";
                            }
                        }
                        ...
                        rowth += " class=\"" + cls + " level" + x + "_row" + y + "\"";

                        rowth += "\"";
                        rowth += " rel=\"X," + x + "\"";

                        if (x > 0) {
                            rowth += " draggable=\"true\"";
                        }
                        // if(xfieldsLength > 1 && TotalRowLevel.length > 1 && x == xfieldsLength - 1){
                        //     if(TotalRowLevel[xfieldsLength-1].count == 1 && (TotalRowLevel[0].count + TotalRowLevel[xfieldsLength-1].total <= TotalRowLevel[0].total)){
                        //         rowth += 'style="border-bottom:2px solid #a6a6a6 !important;"';
                        //     }
                        // }
                        rowth += ">";

                        if (x < this.xfields.length && tablex[y].index.slice(0, x).length > 0) {
                            rowth += this._collapseLink("x", tablex[y].index.slice(0, x));
                        }

                        if (x === 0) {
                            if(this.CollapsedNodes['x']){
                                rowth += this.options.l_all;
                            }else{
                                rowth += '';
                            }
                        } else {
                            rowth += this.fields[this.xfields[x - 1]].getStringValue(tablex[y].index[x - 1]);
                        }
                        rowth += "</th>";
                ...
                for (x = 0; x < tabley.length; x++) {
                    ...
                    for (z = 0; z < this.zfields.length; z++) {
                        ...
                        if(tabley.length - 1 == x){
                            style += ' border-right-color:#a6a6a6;'
                        }
                        if(TotalRowLevel[0].total == TotalRowLevel[0].count || TotalRowLevel[0].count == 64000){
                            style += ' border-bottom-color:#a6a6a6;'
                        }
                        rowtd += 'style="'+ style +'"';

                        rowtd += ">";
                        var valueTd = this._format(this.afields[this.zfields[z]].getValue(V[this.zfields[z]]), this.zfields[z], this.afields[this.zfields[z]].field);
                        if(valueTd){
                            valueBolean = true;
                        }
                        rowtd += valueTd;
                        rowtd += "</td>";
                        valueCls += this.afields[this.zfields[z]].getValue(V[this.zfields[z]]);
                    }
                    valueTotal += valueCls;
                    if (this.zfields.length === 0) {
                        rowtd += "<td";
                ...
                if(tabley.length>1){
                    rowtd += "<td style='background:#F4F4F4;border: 1px solid #a6a6a6' class='summary line_top_3 line_bottom_3 line_left_0  line_right_1'>";
                    rowtd +=this._format(valueTotal);
                    rowtd +="</td>";
                }
                for(x = 0; x <= this.xfields.length; x++){
                    try{TotalRowLevel[x].bolean}catch(err){continue;}
                    if(valueBolean == true){
                        TotalRowLevel[x].bolean = valueBolean;
                    }else{
                        if(TotalRowLevel[x].bolean != true){
                            TotalRowLevel[x].bolean = valueBolean;
                        }
                    }
                    if(TotalRowLevel[x].count >= TotalRowLevel[x].total && TotalRowLevel[x].bolean == false){
                        var rowthObject = $(rowth);
                        delete rowthObject[rowthObject.length-1];
                        rowth = '';
                        for(var countRowTh = 0 ; countRowTh < rowthObject.length; countRowTh++){
                            if(typeof rowthObject[countRowTh] != "undefined"){
                                rowth += rowthObject[countRowTh].outerHTML;
                            }
                        }
                        if(x < TotalRowLevel.length -1){
                            classFindRemove[countClassFind] = TotalRowLevel[x].class;
                            countClassFind++;
                        }
                        rowtd = '';
                        // if(xfieldsLength > 1 && (TotalRowLevel[xfieldsLength-1].total == TotalRowLevel[xfieldsLength-1].count || TotalRowLevel[xfieldsLength-1].count == 64000)){
                        //     var colspan = tabley.length + 1;
                        //     rowtd = '<td class="" colspan="'+ colspan +'" style="padding: 0; border-bottom:1px solid #a6a6a6; height: 1px">';
                        // }
                    }
                    TotalRowLevel[x].count = TotalRowLevel[x].count + 1;
                    if(TotalRowLevel[x].is_collapsed){
                        var tempTd = $('<div />').append(rowtd);
                        tempTd.find('td').addClass('is_collapsed');
                        rowtd = tempTd.html();
                    }
                }
                ...
                contentTable += "<tr>" + row + "</tr>";
            }
            console.log('Total rows in pivot table : ' + y);
            var holdContent = $('<div />').append(contentTable);
            var classRemoveStr = classFindRemove.join(',.');
            if(classRemoveStr != ''){
                holdContent.find('.'+classFindRemove.join(',.')).remove();
            }
            contentTable = holdContent.html();
            S += contentTable;

            if ((this.options.summary) && (this.zfields.length > 0)) {
                S += "<tr>";
                S += "<td style='background:#F4F4F4;border: 1px solid #a6a6a6' colspan='" + (this.xfields.length + 1) + "'";
                S += " >";
                S += "</td>";
                ...
                for (x = 0; x < tabley.length; x++) {
                    ...
                    for (z = 0; z < this.zfields.length; z++) {
                        ...
                        S += " class=\"" + cls + "\"";
                        S += ">";
                        S += this._format(this.afields[this.zfields[z]].getValue(V[this.zfields[z]]), this.zfields[z], this.afields[this.zfields[z]].field);
                        S += "</td>";
                        value += this.afields[this.zfields[z]].getValue(V[this.zfields[z]]),
                            this.zfields[z];
                    }
                    valueS += value;
                }
                if(tabley.length>1){
                    S += "<td style='background:#F4F4F4;border: 1px solid #a6a6a6' class='summary line_top_3 line_bottom_3 line_left_0  line_right_1'";
                    S += " >";
                    S += this._format(valueS);
                    S += "</td>";
                }
                S += "</tr>";
            }
            //remove last tr of table
            ...
            var currentdate = new Date();
            var datetime = "End Generate: " + currentdate.getDate() + "/"
                + (currentdate.getMonth()+1)  + "/"
                + currentdate.getFullYear() + " @ "
                + currentdate.getHours() + ":"
                + currentdate.getMinutes() + ":"
            ...
            console.log(datetime);
            if(breakY == true){
                $('.message-alert-data-too-big').show();
            }
        },
        ...
        _format: function(value, field, fieldName) {
            var V = null;
            if(typeof value == 'number'){
                var decimalSeparator = $('.decimal_separator').val();
                var digitGroupingSeparator = $('.grouping_separator').val();
                var numberDecimals = parseInt($('.no_of_currency_decimals').val());
                var truncateDecimals = parseInt($('.truncate_trailing_zeros').val());
                var currencySymbolPlacement = $('.currency_symbol_placement').val();
                var currencySymbol = $('.currency_symbol').val();
                var currencySymbolPattern = $('.currency_grouping_pattern').val();
                if (currencySymbolPattern){
                    var currencySymbolPatternLength = currencySymbolPattern.split(",").length;
                }
                var regexDecimalNumber = /(\d)(?=(\d{3})+(?!\d)(?=\W))/g;
                if(truncateDecimals == 1){
                    regexDecimalNumber = /(\d)(?=(\d{3})+(?!\d))/g;
                }
                value = parseFloat(value.toFixed(numberDecimals));
                value = value.toString();
                if (digitGroupingSeparator == '.') {
                    value= value.replace('.', 'x');
                }
                value = value.replace(regexDecimalNumber, "$1"+digitGroupingSeparator);
                if (digitGroupingSeparator == '.') {
                    value= value.replace('x', decimalSeparator);
                }else{
                    value =value.replace('.', decimalSeparator);
                }
                if (typeof this.options.data[field] != 'undefined' && typeof this.options.data[field]["currency_field"] != 'undefined'){
                    if(this.options.data[field]["currency_field"].indexOf(fieldName) > -1) {
                        if (currencySymbolPatternLength == 2){
                            value = value.replace(/(\w{3})$/g, "$1"+digitGroupingSeparator);
                        }else if (currencySymbolPatternLength == 3){
                            value = value.replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1"+digitGroupingSeparator);
                        }else if (currencySymbolPatternLength == 4){
                            value = value.replace(/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/g, "$1"+digitGroupingSeparator);
                        }
                        if (currencySymbolPlacement == '1.0$') {
                            value = value.concat(currencySymbol);
                        }
                        else if (currencySymbolPlacement == '$1.0') {
                            value = currencySymbol.concat(value);
                        }
                    }
                } else if (!field && !fieldName && typeof this.options.data[0]["currency_field"] != 'undefined') {
                    if (currencySymbolPlacement == '1.0$') {
                        value = value.concat(currencySymbol);
                    }
                    else if (currencySymbolPlacement == '$1.0') {
...
function agregate_sum(options) {
    ...
    self.agregate = function (a, b) {
        ...
        if (b.type === "agregate_sum") {
            ...
            if (jQuery.inArray(this.field, b['currency_field'])>=0 || typeof b[this.field] === "number" || typeof b[this.field] === "string") {
                try {
                    var decimalSeparator = $('.decimal_separator').val();
                    var digitGroupingSeparator = $('.grouping_separator').val();
                    var value = b[this.field]
                    if (jQuery.inArray(this.field, b['currency_field'])>=0 || typeof b[this.field] === "number") {
                        value = value.replace(' ','');
                    }
                    var regex = new RegExp("\\"+digitGroupingSeparator,'g');
                    value = value.replace(regex,'');
                    value = value.replace(decimalSeparator,'.');
                    res.sum += parseFloat(value,10);
                } catch (err) {
...
function grp_distinct(options) {
    ...
    self.DisplayValues = function (UsedValues) {
        ...
        if (this.fieldtype === "string") {
            if(/(month)/.test(this.field == false)){
