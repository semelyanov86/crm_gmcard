...
Vtiger_AdvanceFilter_Js("VReports_AdvanceFilter_Js",{
	
	getInstance: function(container,newgroup){
		var module = app.getModuleName();
		var moduleClassName = module+"_AdvanceFilter_Js";
		var fallbackClassName = "Vtiger_AdvanceFilter_Js";
		if(typeof window[moduleClassName] != 'undefined'){
			var instance = new window[moduleClassName](container,newgroup);
		}else{
			instance = new window[fallbackClassName](container,newgroup);
		}
		return instance;
	}
},{
	...
	init : function(container,newgroup) {
	...
	initialize : function() {
		...
		this.loadFieldSpecificUiForAll(this.getFilterContainer());
	},
	...
	loadFieldSpecificUi : function(fieldSelect) {
		...
		var conditionSelectElement = row.find('select[name="comparator"]');
		var conditionSelect = row.find('select[name="columnname"]');
		var conditionSelectVal = conditionSelect.val();
        if (typeof this.conditionValue[conditionSelectVal] == 'undefined'){
            this.conditionValue[conditionSelectVal] = row.find('div.col-lg-4.col-md-4.col-sm-4.fieldUiHolder > input').val();
        }
		...
		if (this.conditionValue[conditionSelectVal] && conditionSelect.find(":selected").data('field-name') == 'currency_id') {
            fieldSpecificUi.val(this.conditionValue[conditionSelectVal]).select2();
		}
		...
		if(fieldSpecificUi.is('input.select2')){
			...
			if(fieldSpecificUi.hasClass('chzn-select')) {
				...
				var id = fieldSpecificUi[0].id;
				if(id != undefined && id != null && id != ''){
					vtUtils.showSelect2ElementView($('#'+id));
				}else{
					var name = fieldSpecificUi[0].name;
					var obj = $('select[name="'+name+'"]');
					setTimeout(function(){
						obj.removeAttr('multiple');
						vtUtils.showSelect2ElementView(obj);
					},1)
				}
			}
	 ...
	 */
	loadFieldSpecificUiForAll : function(filterContainer) {
		var conditionsContainer = filterContainer.find('.conditionList');
	...
	getValues : function() {
        ...
        var values = {};
        var valuesAll = {};
		filterContainer.each(function(jndex,filterContainer){
			values = {};
			valuesAll[jndex+1] = {};
			filterContainer = jQuery(filterContainer);
			var groupParentCondition = '';
			var columnIndex = 0;
			...
			conditionGroups.each(function(index,domElement){
                ...
                values.groupParentCondition = groupParentCondition;
			});
			valuesAll[jndex+1] = values;
            if(jndex > 0){
                groupParentCondition = filterContainer.prev().find('select.group-condition').val();
                valuesAll[jndex].groupParentCondition = groupParentCondition;
            }
		});
        return valuesAll;
    },
