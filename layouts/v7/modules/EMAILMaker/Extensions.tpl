                ...
                <tr class="blockHeader">
                    <th colspan="2">
                        <div class="textAlignLeft">{vtranslate($arr.label, 'EMAILMaker')}
                            {if $arr.download neq ""}<span class="pull-right"><a class="btn" href="{$arr.download}">{vtranslate('LBL_DOWNLOAD', 'EMAILMaker')}</a></span>{/if}
                        </div>
                    </th>
                </tr>
                </thead>
                    ...
                    <td class="padding5per">
                        <div class="padding10">
                            ...
                            {if $arr.manual neq ""}
                                <br>
                                <br>
                                {if $extname eq "UnsubscribeEmail"}
                                    <button type="button" id="showUnsubscribeEmailInstructions" class="btn cursorPointer"><strong>{vtranslate('LBL_SHOW_INSTRUCTIONS','EMAILMaker')}</strong></button>
                                    <br>
                                    <div id="UnsubscribeEmailInstructionsDiv" class="hide">
                                        {include file=$arr.manual|@vtemplate_path:$MODULE}
                                        <br>
                                        <button type="button" id="hideUnsubscribeEmailInstructions" class="btn cursorPointer"><strong>{vtranslate('LBL_CLOSE_INSTRUCTIONS','EMAILMaker')}</strong></button>
                                    </div>
                                {else}
                                    {vtranslate('LBL_CUSTOM_INSTAL_EXT', 'EMAILMaker')}<b> <a href="{$arr.manual}" style="cursor: pointer">{vtranslate($arr.label, 'EMAILMaker')}.txt</a></b>
                                {/if}
                            {/if}
