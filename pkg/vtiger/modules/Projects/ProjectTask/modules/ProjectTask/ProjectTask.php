...
class ProjectTask extends CRMEntity
{
    ...
    public function vtlib_handler($modulename, $event_type)
    {
        ...
        if ($event_type == 'module.postinstall') {
            $projectTaskResult = $adb->pquery('SELECT tabid FROM vtiger_tab WHERE name=?', ['ProjectTask']);
            ...
            if (getTabid('CustomerPortal')) {
                $checkAlreadyExists = $adb->pquery('SELECT 1 FROM vtiger_customerportal_tabs WHERE tabid=?', [$projecttaskTabid]);
                if ($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
                    ...
                    $adb->pquery('INSERT INTO vtiger_customerportal_tabs(tabid,visible,sequence) VALUES (?, ?, ?)', [$projecttaskTabid, 1, $nextSequence]);
                    $adb->pquery('INSERT INTO vtiger_customerportal_prefs(tabid,prefkey,prefvalue) VALUES (?, ?, ?)', [$projecttaskTabid, 'showrelatedinfo', 1]);
                }
            }
            ...
            if (!$adb->num_rows($result)) {
                ...
                $adb->pquery('INSERT INTO vtiger_modentity_num values(?,?,?,?,?,?)', [$adb->getUniqueId('vtiger_modentity_num'), $modulename, 'PT', 1, 1, 1]);
            }
    ...
    public function generateReportsSecQuery($module, $secmodule, $queryPlanner)
    {
        ...
        $matrix->setDependency('vtiger_crmentityProjectTask', ['vtiger_groupsProjectTask', 'vtiger_usersProjectTask', 'vtiger_lastModifiedByProjectTask']);

        if (!$queryPlanner->requireTable('vtiger_projecttask', $matrix)) {
            return '';
        }
        $matrix->setDependency('vtiger_projecttask', ['vtiger_crmentityProjectTask']);
    ...
    public function transferRelatedRecords($module, $transferEntityIds, $entityId)
    {
        ...
        $rel_table_arr = ['Documents' => 'vtiger_senotesrel'];

        $tbl_field_arr = ['vtiger_senotesrel' => 'notesid'];

        $entity_tbl_field_arr = ['vtiger_senotesrel' => 'crmid'];
