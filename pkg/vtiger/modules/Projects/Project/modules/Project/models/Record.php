...
class Project_Record_Model extends Vtiger_Record_Model
{
    ...
    public function getProjectTasks()
    {
        ...
        while ($record = $db->fetchByAssoc($result)) {
            ...
            $record['start'] = strtotime($record['startdate']) * 1_000;
            $record['duration'] = $this->getDuration($record['startdate'], $record['enddate']);
            $record['end'] = strtotime($record['enddate']) * 1_000;
            $projectTasks[] = $record;
            --$i;
        }
    ...
    public function getDuration($startDate, $endDate)
    {
        ...
        $duration = floor($difference / (3_600 * 24) + 1);

        // if the start date and end date are same
        if ($duration == 0) {
            return $duration + 0.1;
        }
        if ($duration < 0) { // if end date is null or less than start date
            return 0;
        }
