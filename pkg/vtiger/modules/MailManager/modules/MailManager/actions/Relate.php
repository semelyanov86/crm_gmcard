...
class MailManager_Relate_Action extends Vtiger_MailScannerAction
{
    ...
    public function __SaveAttachements($mailrecord, $basemodule, $recordModel)
    {
        ...
        foreach ($mailrecord->_attachments as $attachmentData) {
            ...
            if ($issaved) {
                ...
                $attachRes = $db->pquery('SELECT * FROM vtiger_attachments WHERE attachmentsid = ?', [$attachid]);
    ...
    {
        $detailViewLink = sprintf(
            "<a target='_blank' href='index.php?module=%s&view=Detail&record=%s'>%s</a>",
            $module,
            $record,
            textlength_check($label),
        );

        return ['record' => $record, 'module' => $module, 'label' => $label, 'detailviewlink' => $detailViewLink];
    }

    /**
     * Returns the related entity for a Mail.
     * @global PearDataBase $db
     * @param int $mailuid - Mail Number
     * @return array
     */
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT name FROM vtiger_ws_entity WHERE id=?', [$modulewsid]);
        if ($db->num_rows($result)) {
