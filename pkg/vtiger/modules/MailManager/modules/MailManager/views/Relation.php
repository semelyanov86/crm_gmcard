...
class MailManager_Relation_View extends MailManager_Abstract_View
{
     ...
     * @var array
     */
    public static $MODULES =  ['Contacts', 'Accounts', 'Leads', 'HelpDesk', 'Potentials'];
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($this->getOperationArg($request) == 'find') {
            ...
            $jsFileNames = ['~libraries/jquery/instaFilta/instafilta.min.js'];
            ...
            $response->setResult(['ui' => $viewer->view('Relationship.tpl', 'MailManager', true)]);

        } elseif ($this->getOperationArg($request) == 'link') {
        ...
        } elseif ($this->getOperationArg($request) == 'create_wizard') {
        ...
        } elseif ($this->getOperationArg($request) == 'create') {
            ...
            try {
                ...
                if ($linkModule === 'Calendar') {
                    ...
                    if ($followupMode == 'on' && $startTime != '' && $startDate != '') {
                        ...
                        [$startDate, $startTime] = explode(' ', $dueDateTime);
    ...
    public function processFormData($mail, $isSentFolder = false)
    {
        ...
        $defaultFieldValueMap =  ['lastname'	=>	$name[0],
            'email'			=> $email[0],
            'email1'		=> $email[0],
    ...
    {
        $moduleListForCreateRecordFromMail = ['Contacts', 'Accounts', 'Leads', 'HelpDesk', 'Calendar', 'Potentials'];
    ...
    {
        $moduleListForLinkTo = ['Calendar', 'HelpDesk', 'ModComments', 'Emails', 'Potentials'];
    ...
    public function lookupModuleRecordsWithEmail($module, $email)
    {
        ...
        if (php7_count($activeEmailFields) > 0) {
            ...
            foreach ($qresults as $qresult) {
                ...
                $results[] = ['wsid' => $qresult['id'], 'id' => $ids[1], 'label' => implode(' ', $labelValues)];
            }
        }
