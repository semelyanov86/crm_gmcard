...
function CustomerCommentFromPortal($entityData)
{
    ...
    if ($moduleName == 'HelpDesk' && !empty($customerWSId)) {
        ...
        $result = $adb->pquery('SELECT email FROM vtiger_contactdetails WHERE contactid=?', [$customerId[0]]);
...
function TicketOwnerComments($entityData)
{
    ...
    if ($moduleName == 'HelpDesk') {
        ...
        if (!empty($contactId)) {
            $result = $adb->pquery(
                'SELECT email, emailoptout FROM vtiger_contactdetails WHERE contactid=?',
                [$contactId],
            );
            ...
            if ($parentEmail != '') {
                ...
                $isPortalUser = $adb->query_result($adb->pquery($sql, [$parentEmail]), 0, 'isactive');
            }
