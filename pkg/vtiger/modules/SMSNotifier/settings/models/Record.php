...
class Settings_SMSNotifier_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function save()
    {
        ...
        $params = [$this->get('providertype'), $this->get('isactive'), $this->get('username'), $password, $this->get('parameters')];
        $id = $this->getId();
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT * FROM vtiger_smsnotifier_servers WHERE id = ?', [$recordId]);
