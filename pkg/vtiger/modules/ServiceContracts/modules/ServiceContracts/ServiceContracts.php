...
class ServiceContracts extends CRMEntity
{
     ...
     */
    public $tab_name = ['vtiger_crmentity', 'vtiger_servicecontracts', 'vtiger_servicecontractscf'];
    ...
    public function vtlib_handler($moduleName, $eventType)
    {
        ...
        if ($eventType == 'module.postinstall') {
            ...
            $accModuleInstance->setRelatedList($moduleInstance, 'Service Contracts', ['add'], 'get_dependents_list');

            $conModuleInstance = Vtiger_Module::getInstance('Contacts');
            $conModuleInstance->setRelatedList($moduleInstance, 'Service Contracts', ['add'], 'get_dependents_list');
            ...
            $adb->pquery('INSERT into vtiger_modentity_num values(?,?,?,?,?,?)', [$adb->getUniqueId('vtiger_modentity_num'), $moduleName, 'SERCON', 1, 1, 1]);

            // Make the picklist value 'Complete' for status as non-editable
            $adb->pquery('UPDATE vtiger_contract_status SET presence=0 WHERE contract_status=?', ['Complete']);
    ...
    public function updateUsedUnits($usedUnits)
    {
        ...
        $this->db->pquery($updateQuery, [$this->id]);
    }
    ...
    public function calculateProgress()
    {
        ...
        $checkServiceContractExistence = $this->db->pquery('SELECT if (EXISTS (select vsc.servicecontractsid from vtiger_servicecontracts  vsc WHERE vsc.servicecontractsid = ?), 1, 0) AS exist_sc', [$this->id]);
        $ServiceContractExistFlag = $checkServiceContractExistence->fields['exist_sc'] ?? '1';
        ...
        if ($contractStatus == 'Complete' || (!empty($usedUnits) && !empty($totalUnits) && $usedUnits >= $totalUnits)) {
            if (empty($endDate)) {
                $endDate = date('Y-m-d');
                $this->db->pquery('UPDATE vtiger_servicecontracts SET end_date=? WHERE servicecontractsid = ?', [date('Y-m-d'), $this->id]);
            }
        } else {
            $endDate = null;
            if ($ServiceContractExistFlag !== '0') {
                $this->db->pquery('UPDATE vtiger_servicecontracts SET end_date=? WHERE servicecontractsid = ?', [null, $this->id]);
            }
        }
