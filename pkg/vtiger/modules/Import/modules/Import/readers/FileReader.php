...
class Import_FileReader_Reader
{
    ...
    public function getDBColumnType($fieldObject, $fieldTypes)
    {
        ...
        $skipDataType = ['reference', 'owner', 'currencyList', 'date', 'datetime', 'productTax', 'ownergroup'];
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT tablename FROM vtiger_field WHERE tabid=? GROUP BY tablename', [$this->moduleModel->getId()]);
        $tables = [];
