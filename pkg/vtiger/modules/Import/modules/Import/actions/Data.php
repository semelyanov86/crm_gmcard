...
class Import_Data_Action extends Vtiger_Action_Controller
{
    ...
    public function updateImportStatus($entryId, $entityInfo)
    {
        ...
        $adb->pquery('UPDATE ' . Import_Utils_Helper::getDbTableName($this->user) . ' SET status=?, recordid=? WHERE id=?', [$entityInfo['status'], $recordId, $entryId]);
    }
        ...
        for ($i = 0; $i < $numberOfRecords; ++$i) {
            ...
            if ($createRecordExists) {
                ...
                if (!empty($mergeType) && $mergeType != Import_Utils_Helper::$AUTO_MERGE_NONE) {
                    ...
                    $duplicatesResult = $adb->pquery($query, []);
                        ...
                        if ($mergeType == Import_Utils_Helper::$AUTO_MERGE_IGNORE) {
                            ...
                            for ($index = 0; $index < $noOfDuplicates - 1; ++$index) {
                                ...
                                if ($userPriviligesModel->hasModuleActionPermission($tabId, 'Delete')) {
                                    $baseRecordModel->transferRelationInfoOfRecords([$duplicateRecordId]);
                                    if ($moduleName == 'Calendar') {
            ...
            if ($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE) {
                ...
                if (!empty($recordId)) {
                    ...
                    switch ($this->module) {
                        case 'HelpDesk': $entityfields['fieldname'] = ['ticket_title'];
                            break;
                        case 'Documents': $entityfields['fieldname'] = ['notes_title'];
                            break;
                    }
                    $label = '';
                ...
                if (in_array($entityInfo['status'], [self::$IMPORT_RECORD_MERGED, self::$IMPORT_RECORD_UPDATED])) {
                    $recordModel = Vtiger_Record_Model::getCleanInstance($this->module);
                    $focus = $recordModel->getEntity();
                    $focus->id = $recordId;
    ...
    public function transformForImport($fieldData, $moduleMeta, $fillDefault = true, $checkMandatoryFieldValues = true)
    {
        ...
        foreach ($fieldData as $fieldName => $fieldValue) {
            ...
            if ($fieldDataType == 'owner') {
                ...
                if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase) && !empty($picklistValueInLowerCase)) {
                    if ($moduleName != 'Calendar') {
                        ...
                        $wsFieldDetails[] = ['label' => $fieldValue, 'value' => $fieldValue];
                    ...
                    $fieldData[$fieldName] = $picklistDetails[$picklistValueInLowerCase] ?? null;
                }
            ...
            } elseif ($fieldDataType == 'ownergroup') {
                ...
                if ($fieldDataType == 'date' && !empty($fieldValue)) {
                    ...
                    if ($userDateFormat === 'dd.mm.yyyy') {
                        $dateFormat = 'd.m.Y';
                    } elseif ($userDateFormat === 'mm.dd.yyyy') {
                        $dateFormat = 'm.d.Y';
                    } elseif ($userDateFormat === 'yyyy.mm.dd') {
                        $dateFormat = 'Y.m.d';
                    } elseif ($userDateFormat === 'dd/mm/yyyy') {
                        $dateFormat = 'd/m/Y';
                    } elseif ($userDateFormat === 'mm/dd/yyyy') {
                        $dateFormat = 'm/d/Y';
                    } elseif ($userDateFormat === 'yyyy/mm/dd') {
    ...
    public function createEntityRecord($moduleName, $entityLabel)
    {
        ...
        foreach ($mandatoryFields as $mandatoryField) {
            if (empty($fieldData[$mandatoryField])) {
                ...
                } elseif (!in_array($mandatoryField, $entityNameFields) && $fieldInstance->getFieldDataType() != 'reference') {
    ...
    public function getImportStatusCount()
    {
        ...
        $statusCount = ['TOTAL' => 0, 'IMPORTED' => 0, 'FAILED' => 0, 'PENDING' => 0, 'CREATED' => 0, 'SKIPPED' => 0, 'UPDATED' => 0, 'MERGED' => 0];
    ...
    public function getNumberOfRecordsToImport($user)
    {
        ...
        $result = $db->pquery($query, [Import_Data_Action::$IMPORT_RECORD_NONE]);
    ...
    {
        $adb = PearDatabase::getInstance();
        $result = $adb->pquery('SELECT count(*) AS count FROM vtiger_import_queue WHERE status = ?', [Import_Queue_Action::$IMPORT_STATUS_SCHEDULED]);
    ...
    public static function getImportDetails($user, $moduleName)
    {
        ...
        $result = $adb->pquery("SELECT * FROM {$tableName} where status IN (?,?)", [self::$IMPORT_RECORD_SKIPPED, self::$IMPORT_RECORD_FAILED]);
        $importRecords = [];
        if ($result) {
            $moduleModel = Vtiger_Module_Model::getInstance($moduleName);
    ...
    public function getImportRecordStatus($value)
    {
        $status = '';
        switch ($value) {
            case 'created': $status = self::$IMPORT_RECORD_CREATED;
                break;
            case 'skipped': $status = self::$IMPORT_RECORD_SKIPPED;
                break;
            case 'updated': $status = self::$IMPORT_RECORD_UPDATED;
                break;
            case 'merged': $status = self::$IMPORT_RECORD_MERGED;
                break;
            case 'failed': $status = self::$IMPORT_RECORD_FAILED;
                break;
            case 'none': $status = self::$IMPORT_RECORD_NONE;
                break;
        }
            ...
            if ($recordData['activitytype']) {
                ...
                foreach ($recordData as $fieldName => $fieldValue) {
                    if ($fieldValue) {
                        ...
                        if (in_array($fieldDataType, ['reference', 'owner'])) {
                        ...
                        } elseif (!in_array($fieldName, ['date_start', 'due_date'])) {
                            if ($fieldModel) {
                ...
                foreach ($recordData as $fieldName => $fieldValue) {
                    if (in_array($fieldName, ['date_start', 'due_date'])) {
                        ...
                        [$fieldValue, $timeValue] = explode(' ', $dateTime);
                ...
                foreach ($recordData as $fieldName => $fieldValue) {
                    ...
                    if ($fieldDataType == 'picklist') {
                        ...
                        if (!in_array($fieldValueInLowerCase, $picklistValuesInLowerCase)
                            ...
                            $fieldModel->setPicklistValues([$fieldValue]);
                        }
                    }
                }
                ...
                try {
                    ...
                    if ($entityInfo['id']) {
                        switch ($operation) {
                            case 'create': $entityInfo['status'] = self::$IMPORT_RECORD_CREATED;
                                break;
                            case 'update': $entityInfo['status'] = self::$IMPORT_RECORD_UPDATED;
                                break;
                            case 'revise': $entityInfo['status'] = self::$IMPORT_RECORD_MERGED;
                                break;
                        }
                    }
                } catch (Exception $e) {
