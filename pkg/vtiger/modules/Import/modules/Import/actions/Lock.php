...
class Import_Lock_Action extends Vtiger_Action_Controller
{
    ...
    public static function lock($importId, $module, $user)
    {
        ...
        $adb->pquery(
            ...
            [$adb->getUniqueID('vtiger_import_locks'), $user->id, getTabid($module), $importId, date('Y-m-d H:i:s')],
        );
    }
    ...
    public static function unLock($user, $module = false)
    {
        ...
        if (Vtiger_Utils::CheckTable('vtiger_import_locks')) {
            ...
            $params = [method_exists($user, 'get') ? $user->get('id') : $user->id];
            if ($module != false) {
                $query .= ' AND tabid=?';
    ...
    public static function isLockedForModule($module)
    {
        ...
        if (Vtiger_Utils::CheckTable('vtiger_import_locks')) {
            $lockResult = $adb->pquery('SELECT * FROM vtiger_import_locks WHERE tabid=?', [getTabid($module)]);
