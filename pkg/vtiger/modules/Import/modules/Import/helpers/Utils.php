...
class Import_Utils_Helper
{
    ...
    public static $supportedFileEncoding = ['UTF-8' => 'UTF-8', 'ISO-8859-1' => 'ISO-8859-1'];

    public static $supportedDelimiters = [',' => 'comma', ';' => 'semicolon', '|' => 'Pipe', '^' => 'Caret'];

    public static $supportedFileExtensions = ['csv', 'vcf'];
    ...
    {
        $mergeTypes = [self::$AUTO_MERGE_IGNORE => 'Skip'];
    ...
    public static function isUserImportBlocked($user)
    {
        ...
        if (Vtiger_Utils::CheckTable($tableName)) {
            $result = $adb->pquery('SELECT 1 FROM ' . $tableName . ' WHERE status = ?', [Import_Data_Action::$IMPORT_RECORD_NONE]);
    ...
    public static function clearUserImportInfo($user)
    {
        ...
        $adb->pquery('DROP TABLE IF EXISTS ' . $tableName, []);
