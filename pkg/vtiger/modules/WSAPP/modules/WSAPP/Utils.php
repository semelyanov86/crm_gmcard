...
function wsapp_getHandler($appType)
{
    ...
    $result = $db->pquery('SELECT * FROM vtiger_wsapp_handlerdetails WHERE type=?', [$appType]);
...
function wsapp_getApplicationName($key)
{
    ...
    $result = $db->pquery('SELECT name from vtiger_wsapp WHERE appkey=?', [$key]);
    $name = false;
...
function wsapp_RegisterHandler($type, $handlerClass, $handlerPath)
{
    ...
    if ($db->num_rows($result) > 0) {
        ...
        $parameters = [$handlerClass, $handlerPath, $type];
    } else {
        $saveQuery = 'INSERT INTO vtiger_wsapp_handlerdetails VALUES(?,?,?)';
        $parameters = [$type, $handlerClass, $handlerPath];
    }
