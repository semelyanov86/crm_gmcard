...
class WSAPP_Logs
{
    public const basetable = 'vtiger_wsapp_logs_basic';
    public const detailstable = 'vtiger_wsapp_logs_details';
     ...
     */
    public static function add($syncRecords)
    {
        global $adb;
        ...
        $params1 = [$id, $extensiontabid, $extensionModule, $syncTime, $recordsCount['app']['create'],
        ...
        $params2 = [$id, $app_create, $app_update, $app_delete, $app_skipped, $vt_create, $vt_update, $vt_delete, $vt_skipped];
     ...
     * @return $syncCounts
     */
    public static function getSyncCounts($pagingModel, $extension, $module = false)
    {
        global $adb;
     ...
     */
    public static function getTotalSyncCount($extension, $module = false)
    {
        global $adb;
        $user = Users_Record_Model::getCurrentUserModel();
        $tabid = getTabid($extension);
     ...
     */
    public static function getSyncCountDetails($logId)
    {
        global $adb;
        $syncRecordDetails = [];

        $query = 'SELECT * from ' . self::detailstable . ' WHERE id=?';
        $result = $adb->pquery($query, [$logId]);
     ...
     * @return type
     */
    public function getSyncRecordsDetails($syncRecords)
    {
        $a = $b = $c = $d = $i = $j = $k = $l = 0;
        $recordCount = ['vtiger'	=> ['update' => 0, 'create' => 0, 'delete' => 0, 'skipped' => 0],
            'app'		=> ['update' => 0, 'create' => 0, 'delete' => 0, 'skipped' => 0]];
        $recordDetails = ['vt_record'	=> ['update' => [], 'create' => [], 'delete' => [], 'skipped' => []],
            'app_record' => ['update' => [], 'create' => [], 'delete' => [], 'skipped' => []]];
                        ...
                        if ($type == 'source') {
                            ...
                            switch ($data->getMode()) {
                                ...
                                case 'skipped':	if (empty($recordInfo['messageidentifier'])) {
                                    ...
                                    $recordDetails['vt_record']['skipped'][$l++] = [$recordName => $recordInfo['message']];
                            }
                        }
                    }
                }
                        ...
                        if ($type == 'target') {
                            ...
                            switch ($data->getMode()) {
                                ...
                                case 'skipped':	if (empty($recordInfo['messageidentifier'])) {
                                    ...
                                    $recordDetails['app_record']['skipped'][$d++] = [$recordName => $recordInfo['message']];
                            }
                        }
                    }
                }
        ...
        return [$recordCount, $recordDetails, $extensionModule];
    }
     ...
     */
    public static function getModuleFromLogId($logId)
    {
        global $adb;
