        ...
        if ($allFilters) {
            foreach ($allFilters as $group => $filters) {
                ...
                foreach ($filters as $filter) {
                    $result[$group][] = ['id' => $filter->get('cvid'), 'name' => $filter->get('viewname'), 'default' => $filter->isDefault()];
                }
            }
        }
        $response = new Mobile_API_Response();
        $response->setResult(['filters' => $result, 'moduleLabel' => vtranslate($module, $module)]);
