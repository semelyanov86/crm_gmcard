...
class Mobile_WS_RelatedRecordsWithGrouping extends Mobile_WS_QueryWithGrouping
{
    public function process(Mobile_API_Request $request)
    {
        ...
        // Input validation
        if (empty($record)) {
            $response->setError(1_001, 'Record id is empty');
        ...
        if ($functionHandler) {
            ...
            $relationResult = call_user_func_array([$sourceFocus, $functionHandler], [$recordid, getTabid($module), getTabid($relatedmodule)]);
