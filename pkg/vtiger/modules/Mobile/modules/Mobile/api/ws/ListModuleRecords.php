...
class Mobile_WS_ListModuleRecords extends Mobile_WS_Controller
{
    ...
    public function process(Mobile_API_Request $request)
    {
        ...
        if (is_string($nameFields)) {
            ...
            $fields = ['name' => $nameFieldModel->get('name'), 'label' => $nameFieldModel->get('label'), 'fieldType' => $nameFieldModel->getFieldDataType()];
        ...
        foreach ($headerFieldModels as $fieldName => $fieldModel) {
            ...
            $fields[] = ['name' => $fieldName, 'label' => $fieldModel->get('label'), 'fieldType' => $fieldModel->getFieldDataType()];
        ...
        if ($listViewEntries) {
            foreach ($listViewEntries as $index => $listViewEntryModel) {
                ...
                $record = ['id' => $listViewEntryModel->getId()];
    ...
    public function processSearchRecordLabelForCalendar(Mobile_API_Request $request, $pagingModel = false)
    {
        ...
        $moreMetaFields = ['date_start', 'time_start', 'activitytype', 'location'];
        ...
        $response = new Mobile_API_Response();
        $response->setResult(['records' => $modifiedRecords, 'module' => 'Calendar']);

        return $response;
    }

    public function fetchRecordLabelsForModule($module, $user, $morefields = [], $filterOrAlertInstance = false, $pagingModel = false)
    {
