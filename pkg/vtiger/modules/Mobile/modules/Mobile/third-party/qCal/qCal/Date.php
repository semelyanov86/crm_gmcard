...
class qCal_Date
{
     ...
     * @return int A number between 1 and 12 inclusively
     */
     ...
     * @return int A number between 1 and 31 inclusively
     */
     ...
     * @return int A number between 0 and 365 inclusively
     */
    public function getYearDay($startFromOne = false)
    {

        $yearDay = $this->dateArray['yday'] + (int) $startFromOne;
    ...
    /**
     * Get the year.
     * @return int The year of this date, for example 1999
     */
     ...
     * @return int a number between 0 (for Sunday) and 6 (for Saturday)
     */
     ...
     * @return int The week of the year (0-51 I think)
    ...
    public function isLeapYear()
    {
        ...
        return (bool) $this->dateArray['L'];

    }

    /**
     * Get a unix timestamp for the date.
     * @return int The amount of seconds since unix epoch (January 1, 1970 UTC)
     */
     ...
     * @param int $xth A positive or negative number that determines which weekday of the month we want
     * @param string|int $weekday Either Sunday-Saturday or 0-6 to specify the weekday we want
     * @param string|int $month Either January-December or 1-12 to specify the month we want
     * @param int $year A valid year to specify which year we want
     */
    public function getXthWeekdayOfMonth($xth, $weekday = null, $month = null, $year = null)
    {
        ...
        if ($negpos == '+') {
            ...
            while ($day <= $numdaysinmonth) {
                ...
                if ($wday == 6) {
                    $wday = 0;
                } // reset to Sunday after Saturday
                else {
                    $wday++;
                }
                ++$day;
            ...
            while ($day >= 1) {
                ...
                if ($wday == 0) {
                    $wday = 6;
                } // reset to Saturday after Sunday
                else {
                    $wday--;
                }
                --$day;
     ...
     * @param int The timestamp to use to create the date
     */
