...
class Settings_Webforms_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function getSelectedFieldsList($mode = '')
    {
        if (!$this->selectedFields) {
            ...
            if ($targetModule) {
                ...
                $result = $db->pquery('SELECT * FROM vtiger_webforms_field WHERE webformid = ? ORDER BY sequence', [$this->getId()]);
    ...
    public function save()
    {
        ...
        if ($mode) {
            ...
            $params = [$this->get('description'), $this->get('returnurl'), $this->get('ownerid'), $this->get('enabled'), $this->get('captcha'), $this->get('roundrobin'), $roundrobinUsersList, 0, $this->get('targetmodule'), $this->getId()];
            ...
            $params = [$this->getName(), $this->get('targetmodule'), $this->generatePublicId(), $this->get('enabled'), $this->get('description'), $this->get('ownerid'), $this->get('returnurl'), $this->get('captcha'), $this->get('roundrobin'), $roundrobinUsersList, 0];
        ...
        $db->pquery('DELETE FROM vtiger_webforms_field WHERE webformid = ?', [$this->getId()]);
        ...
        $db->pquery('DELETE FROM vtiger_webform_file_fields WHERE webformid = ?', [$this->getId()]);
        ...
        foreach ($fileFields as $fileField) {
            ...
            $params = [$this->getId(), $fileFieldName, $fileField['fieldlabel'], $required];
    ...
    public function getFileFields()
    {
        ...
        if (!$fileFields) {
            ...
            $result = $db->pquery('SELECT * FROM vtiger_webform_file_fields WHERE webformid = ?', [$this->getId()]);
    ...
    public static function getInstanceById($recordId, $qualifiedModuleName)
    {
        ...
        $result = $db->pquery('SELECT * FROM vtiger_webforms WHERE id = ?', [$recordId]);
        if ($db->num_rows($result)) {
