...
class Settings_CustomerPortal_Module_Model extends Settings_Vtiger_Module_Model
{
    ...
    public function getCurrentPortalUser()
    {
        ...
        $result = $db->pquery("SELECT prefvalue FROM vtiger_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", []);
    ...
    public function getCurrentDefaultAssignee()
    {
        ...
        $result = $db->pquery('SELECT default_assignee FROM vtiger_customerportal_settings', []);
        if ($db->num_rows($result)) {
    ...
    public function getContactRelatedModulesList()
    {
        ...
        $restrictedModules = ['ModComments', 'Calendar', 'Potentials', 'Emails', 'PurchaseOrder', 'SalesOrder', 'Campaigns', 'Vendors'];
        $contactRelatedModules = [];
    ...
    public function save()
    {
        ...
        foreach ($enableModules as $moduleId => $visibility) {
            $disable = [getTabid('Accounts'), getTabid('Contacts')];
            if (in_array($moduleId, $disable)) {
                throw new Exception('Trying to access restricted module');
                exit;
            }
            $tabid = getTabid($moduleId);
            $db->pquery('INSERT INTO vtiger_customerportal_tabs(tabid,visible) VALUES(?,?) ON DUPLICATE KEY UPDATE visible = ?', [$tabid, $visibility, $visibility]);
        }
        ...
        foreach ($portalModulesInfo as $tabId => $moduleDetails) {
            $db->pquery($updateSequenceQuery, [$moduleDetails['sequence'], $tabId]);
        }
        ...
        if ($dashboardWidgets) {
            $db->pquery('UPDATE vtiger_customerportal_settings SET default_assignee=?, support_notification=?, announcement=?, widgets=?', [$defaultAssignee, $renewalPeriod, $announcement, $dashboardWidgets]);
        }
        ...
        if (!empty($relatedModuleList)) {
            foreach ($relatedModuleList as $module => $info) {
                $tabid = getTabid($module);
                $db->pquery('INSERT INTO vtiger_customerportal_relatedmoduleinfo(tabid, relatedmodules) VALUES(?,?) ON DUPLICATE KEY UPDATE relatedmodules = ?', [$tabid, $info, $info]);
            }
        }
        ...
        if (!empty($recordsVisible)) {
            foreach ($recordsVisible as $module => $info) {
                $tabid = getTabid($module);
                if ($info == 'all') {
                    $db->pquery('UPDATE vtiger_customerportal_fields SET records_visible = ? WHERE tabid = ?', [1, $tabid]);
                } elseif ($info == 'onlymine') {
                    $db->pquery('UPDATE vtiger_customerportal_fields SET records_visible = ? WHERE tabid = ?', [0, $tabid]);
                }
            }
        }
        ...
        if (!empty($recordPermissions)) {
            ...
            $db->pquery('UPDATE vtiger_customerportal_tabs SET createrecord=?,editrecord=? WHERE tabid=?', [$updatedPermissions['create'], $updatedPermissions['edit'], getTabid($updatedPermissions['module'])]);
        }
    }
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT relatedmodules FROM vtiger_customerportal_relatedmoduleinfo WHERE tabid = ? ', [getTabid($sourceModule)]);
        $relatedModules = [];
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT * FROM vtiger_customerportal_settings', []);
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT fieldinfo FROM vtiger_customerportal_fields WHERE tabid = ?', [$tabId]);
        if ($db->num_rows($result) > 0) {
    ...
    {
        $db = PearDatabase::getInstance();
        $db->pquery('INSERT INTO vtiger_customerportal_fields(tabid, fieldinfo) VALUES(?,?) ON DUPLICATE KEY UPDATE fieldinfo = ?', [$tabId, $fieldJson, $fieldJson]);
    }

    public function getRecordVisiblity($tabId)
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT records_visible FROM vtiger_customerportal_fields WHERE tabid= ?', [$tabId]);
        if ($db->num_rows($result)) {
            ...
            } elseif ($visibilityResult == 1) {
                $visibilityInfo['all'] = 1;
                $visibilityInfo['onlymine'] = 0;
                $visibilityInfo['onlypublished'] = 0;
            } elseif ($visibilityResult == 2) {
                $visibilityInfo['all'] = 0;
                $visibilityInfo['onlymine'] = 0;
    ...
    {
        $db = PearDatabase::getInstance();
        $permissionResult = $db->pquery('SELECT createrecord,editrecord FROM vtiger_customerportal_tabs WHERE tabid=?', [$tabid]);
    ...
    public static function isFieldCustomerPortalEditable($crmStatus, $value, $module)
    {
        ...
        if ($crmStatus && $value->name !== 'assigned_user_id' && $value->name !== 'contact_id') {
            $isFieldEditable = 1;
            switch ($module) {
                case 'HelpDesk':	if (in_array($value->name, ['contact_id', 'parent_id'])) {
                    $isFieldEditable = 0;
                }
                    break;
                case 'Assets':	if (in_array($value->name, ['account', 'contact', 'datesold', 'serialnumber'])) {
                    $isFieldEditable = 0;
                }
                    break;
            }
