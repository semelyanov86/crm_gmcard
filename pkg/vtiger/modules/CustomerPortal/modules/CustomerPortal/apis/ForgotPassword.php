...
class CustomerPortal_ForgotPassword extends CustomerPortal_API_Abstract
{
    public function process(CustomerPortal_API_Request $request)
    {
        ...
        $res = $adb->pquery($sql, [$mailid, '0', $current_date]);
        ...
        if ($num_rows > 0) {
            ...
            if ($isActive && ($support_end_date >= $current_date || $support_end_date == null)) {
                ...
                if (!empty($adb->query_result($res, 0, 'cryptmode'))) {
                    ...
                    $params = [$enc_password, 'CRYPT', $contactId];
            ...
            } elseif ($isActive && $support_end_date <= $current_date) {
                if (!$neutralResponse) {
                    throw new Exception('Access to the portal was disabled on ' . $support_end_date, 1_413);
                }
            } elseif ($isActive == 0) {
                if (!$neutralResponse) {
                    throw new Exception('Portal access has not been enabled for this account.', 1_414);
                }
            }
        } else {
    ...
    public function authenticatePortalUser($username, $password)
    {
        // always return true
        return true;
    }
