...
class Tracker
{
    ...
    public function track_view($user_id, $current_module, $item_id, $item_summary)
    {
        ...
        if ($current_module != '') {
            ...
            $result = $adb->pquery($query, [$current_module]);
            ...
            $result = $adb->pquery($query1, [$item_id]);
        ...
        $qparams = [$user_id, $current_module, $item_id, $item_summary];
    ...
    public function get_recently_viewed($user_id, $module_name = '')
    {
        ...
        $result = $this->db->pquery($query, [$user_id], true);
        $list = [];
    ...
    public function delete_history($user_id, $item_id)
    {
        ...
        $this->db->pquery($query, [$user_id, $item_id], true);
    }
    ...
    public function delete_item_history($item_id)
    {
        ...
        $this->db->pquery($query, [$item_id], true);
    ...
    public function prune_history($user_id)
    {
        ...
        while ($count > $history_max_viewed) {
            ...
            $result = $this->db->pquery($query, [$oldest_item['id']], true);
