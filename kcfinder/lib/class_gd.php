...
class gd
{
    ...
    protected function build_image($image)
    {
        ...
        } elseif (false !== ([$width, $height, $type] = @getimagesize($image))) {
            $image
    ...
    public function __construct($image, $bigger_size = null)
    {
        ...
        if ($image_details !== false) {
            [$this->image, $this->width, $this->height] = $image_details;
        } else {
        ...
        if (!is_null($this->image)
            ...
            $image = $this->image;
            [$width, $height] = $this->get_prop_size($bigger_size);
    ...
    public function resize_crop($src)
    {
        ...
        if ($image_details !== false) {
            [$src, $src_width, $src_height] = $image_details;
    ...
    public function gd_color()
    {
        ...
        if (count($args[0]) == 3) {
            ...
            [$r, $g, $b] = explode(' ', preg_replace($expr_rgb, '$1 $2 $3', $args[0]));

        } elseif (preg_match($expr_hex1, $args[0])) {
            [$r, $g, $b] = explode(' ', preg_replace($expr_hex1, '$1 $2 $3', $args[0]));
            $r = hexdec($r);
            $g = hexdec($g);
            $b = hexdec($b);

        } elseif (preg_match($expr_hex2, $args[0])) {
            [$r, $g, $b] = explode(' ', preg_replace($expr_hex2, '$1$1 $2$2 $3$3', $args[0]));
            $r = hexdec($r);
            $g = hexdec($g);
            $b = hexdec($b);
