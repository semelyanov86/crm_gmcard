...
class uploader
{
    ...
    protected $typeSettings = ['disabled', 'theme', 'dirPerms', 'filePerms', 'denyZipDownload', 'maxImageWidth', 'maxImageHeight', 'thumbWidth', 'thumbHeight', 'jpegQuality'];
    ...
    public function __construct()
    {
        ...
        $this->type = (
            isset($this->get['type'], $this->types[$this->get['type']])
        ...
        if (is_array($this->types[$this->type])) {
            ...
            $this->types[$this->type] = $this->types[$this->type]['type'] ?? '';
        }
    ...
    protected function localize($langCode)
    {
        ...
        unset($lang['_locale'], $lang['_charset'], $lang['_dateTimeFull'], $lang['_dateTimeMid'], $lang['_dateTimeSmall']);
    ...
    {
        $return = $this->labels[$string] ?? $string;
        if (is_array($data)) {
    ...
    protected function callBack($url, $message = '')
    {
        ...
        $CKfuncNum = $this->opener['CKEditor']['funcNum'] ?? 0;
        if (!$CKfuncNum) {
            $CKfuncNum = 0;
        }
