...
class SMSNotifier_ClickATell_Provider implements SMSNotifier_ISMSProvider_Model
{
    ...
    private $parameters = [];
    public const SERVICE_URI = 'http://api.clickatell.com';

    private static $REQUIRED_PARAMETERS = ['api_id', 'from', 'mo', 'unicode'];
    ...
    public function query($messageId)
    {
        ...
        $result = ['error' => false, 'needlookup' => 1, 'statusmessage' => ''];
        ...
        if (preg_match('/ERR: (.*)/', $response, $matches)) {
            ...
            if ($status == '002' || $status == '008' || $status == '011') {
                ...
                $statusMessage = '';
                switch ($status) {
                    case '001': $statusMessage = 'Message unknown';
                        break;
                    case '005': $statusMessage = 'Error with message';
                        break;
                    case '006': $statusMessage = 'User cancelled message delivery';
                        break;
                    case '007': $statusMessage = 'Error delivering message';
                        break;
                    case '009': $statusMessage = 'Routing error';
                        break;
                    case '010': $statusMessage = 'Message expired';
                        break;
                    case '012': $statusMessage = 'Out of credit';
                        break;
                }
