    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if (!empty($record)) {
            ...
            foreach ($split_amount as $_amount) {
                ...
                $amount_tooltip[$_amount * 100 . "%"] = array("total" => $_total_tooltip->getDisplayValue(NULL, true), "balance" => $_balance_tooltip->getDisplayValue(NULL, true));
            }
        ...
        $res = $db->pquery($sql, array($invoiceId));
        ...
        while ($row = $db->fetch_row($res)) {
            ...
            if ($an_integrate_status) {
                ...
                $result = $db->pquery($query, array($row["paymentid"]));
                $transactions = array();
            ...
            if (!empty($modulePaymentCredits) && ($row["payment_status"] == "*Credit" || $row["payment_status"] == "Credit Applied" || $row["payment_status"] == "*Credit - Used" || $row["payment_status"] == "*Credit Used" || $row["payment_status"] == "Paid")) {
                ...
                $res_credits = $db->pquery($query_credits, array($row["paymentid"], $row["paymentid"]));
                $list_credits = array();
        ...
        $res = $db->pquery($query, array($invoiceId));
