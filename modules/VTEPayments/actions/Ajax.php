...
class VTEPayments_Ajax_Action extends Vtiger_IndexAjax_View
{
    ...
    public function updateGrandTotal(Vtiger_Request $request)
    {
        ...
        $res = $db->pquery($sql, array($invoice_id));
        $total_amount = 0;
        $total = 0;
        $balance = 0;
        if (0 < $db->num_rows($res)) {
        ...
        $res = $db->pquery($query, array($invoice_id));
        if (0 < $db->num_rows($res)) {
            while ($row = $db->fetch_row($res)) {
                $queryCredit = "SELECT SUM(credit_amount) credit_amount FROM vtiger_credits WHERE vtepaymentid = ?";
                $resCredit = $db->pquery($queryCredit, array($row["paymentid"]));
                $creditAmount = 0;
                if (0 < $db->num_rows($resCredit)) {
        ...
        $result = array("total_amount" => $total_amount->getDisplayValue(), "total" => $total->getDisplayValue(), "balance" => $balance->getDisplayValue(), "credit_available_amount" => $credit_available_amount->getDisplayValue());
        $response = new Vtiger_Response();
        $response->setEmitType(Vtiger_Response::$EMIT_JSON);
    ...
    public function updateTooltipArea(Vtiger_Request $request)
    {
        ...
        $split_amount = array(25 / 100, 33 / 100, 50 / 100, 1);
        foreach ($split_amount as $_amount) {
            ...
            $amount_tooltip[$_amount * 100 . "%"] = array("total" => $_total_tooltip->getDisplayValue(), "balance" => $_balance_tooltip->getDisplayValue());
        }
    ...
    public function applyCredit(Vtiger_Request $request)
    {
        ...
        $res = $adb->pquery($query, array($paymentid));
        ...
        if (0 < $accountid || 0 < $contactid) {
            ...
            while ($row = $adb->fetchByAssoc($res)) {
                ...
                $resCredit = $adb->pquery($queryCredit, array($row["paymentid"]));
                if (0 < $adb->num_rows($resCredit)) {
    ...
    public function saveCredit(Vtiger_Request $request)
    {
        ...
        $query = "SELECT * FROM vtiger_payments WHERE paymentid = ?";
        $res = $adb->pquery($query, array($current_paymentid));
        if (0 < $adb->num_rows($res)) {
            $invoiceid2 = $adb->query_result($res, 0, "invoice");
        }
        $query = "SELECT * FROM vtiger_payments WHERE paymentid = ?";
        $res = $adb->pquery($query, array($credit_paymentid));
        if (0 < $adb->num_rows($res)) {
            $accountid = $adb->query_result($res, 0, "organization");
            $contactid = $adb->query_result($res, 0, "contact");
        ...
        if (0 < floatval($remaining_credit)) {
            $obj_payment = Vtiger_Record_Model::getInstanceById($credit_paymentid, "VTEPayments");
            $obj_payment->set("mode", "edit");
