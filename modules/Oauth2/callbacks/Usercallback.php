...
class Oauth2_Usercallback_Callbacks
{
    ...
    public static function handleRequest($config, $req)
    {
        ...
        $authfor = (isset($req['authfor'])) ? $req['authfor'] : ($_SESSION['oauth2for'] ?? '');
        switch ($authfor) {
            case 'OutgoingServer':
        ...
        $authsvc = (isset($req['authservice'])) ? $req['authservice'] : ($_SESSION['oauth2svc'] ?? '');
        ...
        $provider = new GenericProvider($authcfg);
        ...
        if (isset($req['state'], $_SESSION['oauth2state'])   && $req['state'] != $_SESSION['oauth2state']) {
            // something wrong
        ...
        try {
            ...
            $oauth2for = $_SESSION['oauth2for'] ?? '';
            $oauth2svc = $_SESSION['oauth2svc'] ?? '';

            if ($userinfo['email'] && $userinfo['email_verified']) {
                $tokens = ['access_token' => $accessTokenValue, 'refresh_token' => $refreshTokenValue];
                static::updateTokensFor($config, $oauth2for, $oauth2svc, $userinfo, $tokens, $accessTokenExpiresOn);
            }


            unset($_SESSION['oauth2for'], $_SESSION['oauth2state'], $_SESSION['oauth2svc']);
    ...
    protected static function updateTokensFor($config, $oauth2for, $oauth2svc, $userinfo, $tokens, $expireson)
    {
        ...
        if ($oauth2for == 'OutgoingServer') {
            $checkRs = $db->pquery('select 1 from vtiger_systems where server_type = ? limit 1', ['email']);
        ...
        } elseif ($oauth2for == 'MailConverter') {
