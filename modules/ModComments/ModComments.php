...
class ModComments extends ModCommentsCore
{
    ...
    public function vtlib_handler($modulename, $event_type)
    {
        ...
        if ($event_type == 'module.postinstall') {
            self::addWidgetTo(['Leads', 'Contacts', 'Accounts', 'Potentials', 'Project', 'ProjectTask']);
            global $adb;
            // Mark the module as Standard module
            $adb->pquery('UPDATE vtiger_tab SET customized=0 WHERE name=?', [$modulename]);

        } elseif ($event_type == 'module.postupdate') {
            self::addWidgetTo(['Potentials']);
        }
    }
    ...
    {
        global $adb;
        $adb->pquery('UPDATE vtiger_modcomments SET related_to=? WHERE related_to=?', [$targetParentId, $currentParentId]);
    }
    ...
    public static function copyCommentsToRelatedRecord($currentParentId, $targetParentId)
    {
        ...
        $relatedIdMap = [];
        $result = $db->pquery('SELECT *FROM vtiger_modcomments WHERE related_to=?', [$currentParentId]);
        ...
        for ($i = 0; $i < $count; ++$i) {
            ...
            $crmEntityResult = $db->pquery('SELECT *FROM vtiger_crmentity where crmid = ?', [$commentId]);
            ...
            $crmentityParams = [$commentCrmId, $smcreatorId,
            ...
            $modcommentsParams = [$commentCrmId, $commentContent,
