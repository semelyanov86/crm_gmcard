...
class Users extends CRMEntity
{
    ...
    public $tab_name = ['vtiger_users', 'vtiger_attachments', 'vtiger_user2role', 'vtiger_asteriskextensions'];

    public $tab_name_index = ['vtiger_users' => 'id', 'vtiger_attachments' => 'attachmentsid', 'vtiger_user2role' => 'userid', 'vtiger_asteriskextensions' => 'userid'];
    ...
    public $homeorder_array = ['HDB', 'ALVT', 'PLVT', 'QLTQ', 'CVLVT', 'HLT', 'GRT', 'OLTSO', 'ILTI', 'MNL', 'OLTPO', 'LTFAQ', 'UA', 'PA'];
    ...
    public $emailTemplate_defaultFields = ['first_name', 'last_name', 'userlabel', 'title', 'department', 'phone_home', 'phone_mobile', 'signature', 'email1', 'email2', 'address_street', 'address_city', 'address_state', 'address_country', 'address_postalcode'];
    ...
    // Default Widgests
    public $default_widgets = ['PLVT', 'CVLVT', 'UA'];
     ...
     * @return bool - If passwords pass verification and query succeeds, return true, else return false
    ...
    public function change_password($user_password, $new_password, $dieOnError = true)
    {
        ...
        if (!is_admin($current_user)) {
            ...
            if ($this->db->hasFailedTransaction()) {
                if ($dieOnError) {
                    exit('error verifying old transaction[' . $this->db->database->ErrorNo() . '] '
        ...
        $query = "UPDATE {$this->table_name} SET user_password=?, confirm_password=?, "
                . 'crypt_type=? where id=?';
        $this->db->pquery($query, [$encrypted_new_password, $encrypted_new_password,
            $crypt_type, $this->id]);
        if ($this->db->hasFailedTransaction()) {
            if ($dieOnError) {
                exit('error setting new password: [' . $this->db->database->ErrorNo() . '] '
                        . $this->db->database->ErrorMsg());
            }
    ...
    public function retrieve_user_id($user_name)
    {
        ...
        $result  = $adb->pquery($query, [$user_name]);
    ...
    public function verify_data()
    {
        ...
        $result = $this->db->pquery($query, [$usr_name, $this->id], true, 'Error selecting possible duplicate users: ');
        $dup_users = $this->db->fetchByAssoc($result);

        $query = "SELECT user_name from vtiger_users where is_admin = 'on' AND deleted=0";
        $result = $this->db->pquery($query, [], true, 'Error selecting possible duplicate vtiger_users: ');
    ...
    public function getColumnNames_User()
    {
        ...
        $mergeflds = ['FIRSTNAME', 'LASTNAME', 'USERNAME', 'SECONDARYEMAIL', 'TITLE', 'OFFICEPHONE', 'DEPARTMENT',
    ...
    public function fill_in_additional_detail_fields()
    {
        ...
        $result = $this->db->pquery($query, [$this->id], true, 'Error filling in additional detail vtiger_fields');
    ...
    public function saveentity($module, $fileid = '')
    {
        ...
        require_once 'modules/Users/CreateUserPrivilegeFile.php';
        createUserPrivilegesfile($this->id);
        unset($_SESSION['next_reminder_interval'], $_SESSION['next_reminder_time']);
    ...
    public function createAccessKey()
    {
        ...
        $adb->pquery($updateQuery, [vtws_generateRandomAccessKey(16), $this->id]);
    }
    ...
    public function insertIntoEntityTable($table_name, $module, $fileid = '')
    {
        ...
        if ($insertion_mode == 'edit') {
            ...
            $check_result = $this->db->pquery($check_query, [$this->id]);
    ...
    public function retrieve_entity_info($record, $module, $allowDeleted = false)
    {
        ...
        foreach ($this->tab_name_index as $table_name => $index) {
            $result[$table_name] = $adb->pquery('select * from ' . $table_name . ' where ' . $index . '=?', [$record]);
        }
        $tabid = getTabid($module);
        ...
        $currency_result = $adb->pquery($currency_query, [$this->column_fields['currency_id']]);
        if ($adb->num_rows($currency_result) == 0) {
            $currency_query = 'select * from vtiger_currency_info where id =1';
            $currency_result = $adb->pquery($currency_query, []);
        }
        $currency_array = ['$' => '&#36;', '&euro;' => '&#8364;', '&pound;' => '&#163;', '&yen;' => '&#165;'];
        $currency_symbol = $adb->query_result($currency_result, 0, 'currency_symbol');
        $ui_curr = $currency_array[$currency_symbol] ?? '';
        if ($ui_curr == '') {
    ...
    public function uploadAndSaveFile($id, $module, $file_details, $attachmentType = 'Attachment')
    {
        ...
        if ($save_file) {
            ...
            $params1 = [$current_id, $current_user->id, $ownerid, $module . ' Image', $this->column_fields['description'], $this->db->formatString('vtiger_crmentity', 'createdtime', $date_var), $this->db->formatDate($date_var, true)];
    ...
    public function save($module_name, $fileid = '')
    {
        ...
        $query_prev_interval = $adb->pquery(
            'SELECT reminder_interval from vtiger_users where id=?',
            [$this->id],
        );
    ...
    public function getHomeStuffOrder($id)
    {
        ...
        if (!is_array($this->homeorder_array)) {
            $this->homeorder_array = ['UA', 'PA', 'ALVT', 'HDB', 'PLVT', 'QLTQ', 'CVLVT', 'HLT',
    ...
    public function resetReminderInterval($prev_reminder_interval)
    {
        ...
        if ($prev_reminder_interval != $this->column_fields['reminder_interval']) {
            ...
            $adb->pquery('UPDATE vtiger_users SET reminder_next_time=?, date_entered=? WHERE id=?', [$set_reminder_next, $this->column_fields['date_entered'], $this->id]);
        }
    }
    ...
    public function transformOwnerShipAndDelete($userId, $transformToUserId)
    {
        ...
        $adb->pquery($sql, ['Inactive', true, $userId]);
    ...
    public function uploadOrgLogo($requestArray, $fileArray)
    {
        ...
        $validFileFormats =  ['jpeg', 'png', 'jpg', 'pjpeg', 'x-png', 'gif'];
        ...
        if ($file_size != 0 && in_array($file_type_val, $validFileFormats)) {
            ...
            $params = [decode_html($logo_name), '1'];
    ...
    public function updateBaseCurrency($requestArray)
    {
        ...
        if (isset($requestArray['currency_name'])) {
            ...
            $result = $adb->pquery('SELECT currency_code, currency_symbol FROM vtiger_currencies WHERE currency_name = ?', [$currency_name]);
            ...
            $params = [$currency_name, $currency_code, $currency_symbol, '1'];
    ...
    public function getAccountOwnerId()
    {
        ...
        return $adb->query_result($adb->pquery($query, [1]), 0, 'id');
    }
    ...
    public function transferAccountOwnerShipAndSave()
    {
        ...
        $adb->pquery('UPDATE vtiger_users SET is_owner=? WHERE id = ?', [1, $current_user->id]);
     ...
     */
    public function transferOwnership($recordId)
    {
        if ($recordId) {
            $activeAdminId = $this->getActiveAdminId();
            $recordModel = Users_Record_Model::getInstanceById($recordId, 'Users');
            $db = PearDatabase::getInstance();
            $db->pquery('UPDATE vtiger_users SET is_owner = ? WHERE id = ?', ['', $activeAdminId]);
            $db->pquery('UPDATE vtiger_users SET is_owner = ? WHERE id = ?', [1, $recordId]);
            require_once 'modules/Users/CreateUserPrivilegeFile.php';
    ...
    {
        $nonImportableFields = ['is_owner', 'imagename', 'status', 'end_hour', 'theme'];
    ...
    public function createRecords($obj)
    {
        ...
        $result = $adb->pquery($sql, [Import_Data_Action::$IMPORT_RECORD_NONE]);
        ...
        for ($i = 0; $i < $numberOfRecords; ++$i) {
            ...
            foreach ($fieldInstances as $blockInstance) {
                foreach ($blockInstance as $fieldInstance) {
                    ...
                    if ($fieldName == 'status') {
                        $fieldValue = 'Active';
                    } elseif ($fieldName == 'theme') {
                        $fieldValue = 'softed';
                    } elseif ($fieldName == 'confirm_password') {
                        if ($fieldValue != $data['user_password']) {
                            $mandatoryFlag = false;
                        }
                    } elseif ($fieldName == 'roleid') {
                        ...
                        $fieldValue = $roleId;
                        unset($roleId);
                    } elseif ($fieldName == 'is_owner') {
                        $fieldValue = null;
                    } elseif ($fieldName == 'currency_id') {
                        ...
                        $fieldValue = $lang;
                        unset($lang);
                    } elseif ($fieldName == 'reports_to_id') {
                        ...
                        $emptyValuedPicklistFields = ['defaulteventstatus', 'defaultactivitytype', 'reminder_interval'];
                    ...
                    } elseif ($dataType == 'email' && !empty($fieldValue)) {
            ...
            if ($mandatoryFlag) {
                ...
                foreach ($modelData as $fieldName => $fieldValue) {
                    $recordModel->set($fieldName, $record[$fieldName] ?? null);
                }
                $recordModel->save();
...
class Users_CRMSetup
{
    ...
    {
        $restrictedModulesList = ['Emails', 'ModComments', 'Rss', 'Portal', 'Integration', 'PBXManager', 'Dashboard', 'Home'];
        ...
        global $adb;
        $result = $adb->pquery('SELECT parent, name, tablabel FROM vtiger_tab', []);
            ...
            if (!$moduleExists) {
                ...
                if ($parentName) {
                    if (array_key_exists($parentName, $packagesList)) {
                        ...
                        $packagesList[$parentName] = ['label' => $parentName,
                            'modules' => [$moduleName => $adb->query_result($result, $i, 'tablabel')]];
                    }
                }
            }
        }
