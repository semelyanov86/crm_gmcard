...
function createUserSharingPrivilegesfile($userid)
{
    ...
    if ($handle) {
        ...
        for ($idx = 0; $idx < php7_count($custom_modules); ++$idx) {
            ...
            $mod_share_perm_array = getUserModuleSharingObjects(
                $module_name,
                $userid,
                $def_org_share,
                $current_user_roles,
...
function getUserModuleSharingObjects($module, $userid, $def_org_share, $current_user_roles, $parent_roles, $current_user_groups)
{
    ...
    if ($def_org_share[$mod_tabid] == 3 || $def_org_share[$mod_tabid] == 0) {
        ...
        $result = $adb->pquery($query, [$mod_tabid, $current_user_roles]);
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; ++$i) {
        ...
        $result = $adb->pquery($query, [$mod_tabid, $parRoleList]);
        $num_rows = $adb->num_rows($result);
        for ($i = 0; $i < $num_rows; ++$i) {
            $share_roleid = $adb->query_result($result, $i, 'share_roleid');
...
function getRelatedModuleSharingArray($par_mod, $share_mod, $mod_sharingrule_members, $mod_share_read_per, $mod_share_write_per, $def_org_share)
{
    ...
    if ($def_org_share[$share_mod_id] == 3 || $def_org_share[$share_mod_id] == 0) {
        ...
        foreach ($mod_sharingrule_members as $sharingid => $sharingInfoArr) {
            ...
            $result = $adb->pquery($query, [$sharingid, $par_mod_id, $share_mod_id]);
...
function constructTwoDimensionalArray($var)
{
    if (is_array($var)) {
        ...
        foreach ($var as $key => $secarr) {
            $code .= $key . '=>array(';
            if ($secarr == null || !is_iterable($secarr)) {
                continue;
            }
...
function populateSharingtmptables($userid)
{
    ...
    $table_arr = ['vtiger_tmp_read_user_sharing_per', 'vtiger_tmp_write_user_sharing_per', 'vtiger_tmp_read_group_sharing_per', 'vtiger_tmp_write_group_sharing_per', 'vtiger_tmp_read_user_rel_sharing_per', 'vtiger_tmp_write_user_rel_sharing_per', 'vtiger_tmp_read_group_rel_sharing_per', 'vtiger_tmp_write_group_rel_sharing_per'];
    foreach ($table_arr as $tabname) {
        $query = 'delete from ' . $tabname . ' where userid=?';
        $adb->pquery($query, [$userid]);
    }
            ...
            foreach ($var_name_arr['ROLE'] as $roleid => $roleusers) {
                ...
                foreach ($roleusers as $user_id) {
                    if (! in_array($user_id, $user_arr)) {
                        ...
                        $adb->pquery($query, [$userid, $tabid, $user_id]);
                        $user_arr[] = $user_id;
                    }
                }
            }
        ...
        if (php7_sizeof($var_name_arr['ROLE']) > 0) {
            foreach ($var_name_arr['ROLE'] as $roleid => $roleusers) {
                ...
                foreach ($roleusers as $user_id) {
                    if (! in_array($user_id, $user_arr)) {
                        $query = 'insert into ' . $table_name . ' values(?,?,?,?)';
                        $adb->pquery($query, [$userid, $tabid, $reltabid, $user_id]);
                        $user_arr[] = $user_id;
                    }
                }
            }
