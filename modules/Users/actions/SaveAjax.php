...
class Users_SaveAjax_Action extends Vtiger_SaveAjax_Action
{
    ...
    public function checkPermission(Vtiger_Request $request)
    {
        ...
        if (!$currentUserModel->isAdminUser()) {
            ...
            if (in_array($mode, ['userExists', 'restoreUser', 'transferOwner', 'changeUsername'])) {
                throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
            }
            if ($mode != 'savePassword' && ($currentUserModel->getId() != $request->get('record'))) {
                throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
            }
        }
    }
    ...
    protected function checkRestrictedValueChange(Vtiger_Request $request)
    {
        ...
        if ($request->has('field') && in_array($request->get('field'), ['user_name', 'user_password', 'accesskey'])) {
            // should use separate actions.
            throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
        }
    ...
    public function getRecordModelFromRequest(Vtiger_Request $request)
    {
        ...
        foreach ($modelData as $fieldName => $value) {
            ...
            if ($fieldName == 'is_owner' || (in_array($fieldName, ['user_type', 'roleid']) && !$currentUserModel->isAdminUser())) {
                $fieldValue = null;
            }
    ...
    public function restoreUser(Vtiger_Request $request)
    {
        ...
        $response = new Vtiger_Response();
        $response->setResult(['message' => vtranslate('LBL_USER_RESTORED_SUCCESSFULLY', $moduleName), 'listViewUrl' => $listViewUrl]);
        $response->emit();
    }
    ...
    public function transferOwner(Vtiger_Request $request)
    {
        ...
        $response = new Vtiger_Response();
        if ($status) {
            $response->setResult(['message' => vtranslate('LBL_OWNERSHIP_TRANSFERRED_SUCCESSFULLY', $moduleName)]);
        } else {
    ...
    public function changeAccessKey(Vtiger_Request $request)
    {
        ...
        try {
            ...
            if ($newAccessKey != $oldAccessKey) {
                $response->setResult(['success' => true, 'message' => vtranslate('LBL_ACCESS_KEY_UPDATED_SUCCESSFULLY', $moduleName), 'accessKey' => $newAccessKey]);
            } else {
