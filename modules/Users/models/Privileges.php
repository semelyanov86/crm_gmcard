...
class Users_Privileges_Model extends Users_Record_Model
{
    ...
    public static function getInstanceById($userId, $module = null)
    {
        ...
        $valueMap['roleid'] = $acl->current_user_roles ?? null;
        $valueMap['parent_role_seq'] = $acl->current_user_parent_role_seq ?? null;
        $valueMap['profiles'] = $acl->current_user_profiles ?? null;
        $valueMap['profile_global_permission'] = $acl->profileGlobalPermission ?? null;
        $valueMap['profile_tabs_permission'] = $acl->profileTabsPermission ?? null;
        $valueMap['profile_action_permission'] = $acl->profileActionPermission ?? null;
        $valueMap['groups'] = $acl->current_user_groups ?? null;
        $valueMap['subordinate_roles'] = $acl->subordinate_roles ?? null;
        $valueMap['parent_roles'] = $acl->parent_roles ?? null;
        $valueMap['subordinate_roles_users'] = $acl->subordinate_roles_users ?? null;
        $valueMap['defaultOrgSharingPermission'] = $defaultOrgSharingPermission ?? null;
        $valueMap['related_module_share'] = $related_module_share ?? null;
