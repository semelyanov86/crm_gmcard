...
{
    public const moduletableName = 'vtiger_pbxmanager';
    public const lookuptableName = 'vtiger_pbxmanager_phonelookup';
    public const entitytableName = 'vtiger_crmentity';
    ...
    public static function lookUpRelatedWithNumber($from)
    {
        ...
        $rnumber = strrev($fnumber);
        $result = $db->pquery('SELECT crmid, fieldname FROM ' . self::lookuptableName . ' WHERE fnumber LIKE "' . $fnumber . '%" OR rnumber LIKE "' . $rnumber . '%" ', []);
        if ($db->num_rows($result)) {
            $crmid = $db->query_result($result, 0, 'crmid');
            $fieldname = $db->query_result($result, 0, 'fieldname');
            $contact = $db->pquery('SELECT label,setype FROM ' . self::entitytableName . ' WHERE crmid=? AND deleted=0', [$crmid]);
            if ($db->num_rows($result)) {
                $data['id'] = $crmid;
