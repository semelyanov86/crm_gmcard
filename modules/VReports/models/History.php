...
class VReports_History_Model extends Vtiger_Module_Model
{
    ...
    public static function getModuleNameForHistory($relId)
    {
        ...
        $modId = $modId[0];
        $query2 = $db->pquery('SELECT * FROM vtiger_crmentity WHERE crmid = ?', [$modId]);
    ...
    public static function getRelatedRecordForHistory($data)
    {
        ...
        $query = $db->pquery($sql, [$data->get('id')]);
        ...
        if ($result) {
            ...
            if ($countEmailBodyPlain) {
                ...
                $query3 = $db->pquery($sql2, [$data->get('id')]);
                $relation['email_body_plain'] = $db->query_result($query3, 0, 'email_body_plain');
            }

            return $relation;
        }
        $result = $db->pquery('SELECT * FROM vtiger_modtracker_relations WHERE id = ?', [$data->get('id')]);
        ...
        $query3 = $db->pquery($sql3, [$data->get('id')]);
        $emailList = '';
        $relation['from_email'] = '';
        ...
        if ($countEmailBodyPlain) {
            ...
            $query4 = $db->pquery($sql4, [$data->get('id')]);
