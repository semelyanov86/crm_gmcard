...
class VReports_Chart_Model extends Vtiger_Base_Model
{
    ...
    private $colorChart = ['bgBarColors' => ['window.bgBarColors.red', 'window.bgBarColors.orange', 'window.bgBarColors.yellow', 'window.bgBarColors.green', 'window.bgBarColors.blue', 'window.bgBarColors.purple', 'window.bgBarColors.grey'], 'borderBarColors' => ['window.borderBarColors.red', 'window.borderBarColors.orange', 'window.borderBarColors.yellow', 'window.borderBarColors.green', 'window.borderBarColors.blue', 'window.borderBarColors.purple', 'window.borderBarColors.grey'], 'bgPieColors' => ['window.bgPieColors.red', 'window.bgPieColors.orange', 'window.bgPieColors.yellow', 'window.bgPieColors.green', 'window.bgPieColors.blue', 'window.bgPieColors.purple', 'window.bgPieColors.grey']];
    ...
    public function generateChartGroupBar($data)
    {
        ...
        if ($typeChart == 'stacked') {
            ...
            if ($typeChart == 'horizontalBar') {
                $this->colorChart['bgBarColors'] = ['window.bgBarColors.green', 'window.bgBarColors.blue', 'window.bgBarColors.purple'];
                $this->colorChart['borderBarColors'] = ['window.borderBarColors.green', 'window.borderBarColors.blue', 'window.borderBarColors.purple'];
            } else {
                if ($typeChart == 'bar') {
                    $this->colorChart['bgBarColors'] = ['window.bgBarColors.blue', 'window.bgBarColors.green', 'window.bgBarColors.purple'];
                    $this->colorChart['borderBarColors'] = ['window.borderBarColors.blue', 'window.borderBarColors.green', 'window.borderBarColors.purple'];
                }
            }
        }
        $html = 'var color = Chart.helpers.color;';
        $drawline = $this->get('drawline');
        if (!empty($drawline) && $drawline > 0) {
        ...
        $max = round($max, 0);
        if ($max > 2_147_483_647 || $max < $oldmax) {
            $max += $stepValue;
...
abstract class Base_Chart extends Vtiger_Base_Model
{
    ...
    public function getModuleNameRecord($crmId)
    {
        global $adb;
        if (trim($crmId) != '') {
            $rs = $adb->pquery('SELECT * FROM `vtiger_crmentity` WHERE crmid =? AND deleted=0', [$crmId]);
            if ($adb->num_rows($rs) > 0) {
...
class PieChart extends Base_Chart
{
    public function generateData()
    {
        ...
        for ($i = 0; $i < $rows; ++$i) {
            ...
            $value = (float) round($row[$sector], $currentUserModel->no_of_currency_decimals);
        ...
        $data = ['labels' => $labels, 'values' => $values, 'links' => $links, 'graph_label' => $this->getGraphLabel(), 'chart_group_type' => 'pie'];
...
class BarChart extends Base_Chart
{
    public function generateData()
    {
                    ...
                    if ($queryColumnsByFieldModel) {
                        foreach ($queryColumnsByFieldModel as $fieldModel) {
                            if ($fieldModel->get('uitype') == '71' || $fieldModel->get('uitype') == '72') {
                                ...
                                $value = (float) round($row[$reportLabel], $currentUserModel->no_of_currency_decimals);
                                $values[$j][] = CurrencyField::convertFromDollar($value, $currencyRateAndSymbol['rate']);
                            } else {
                                if ($fieldModel->getFieldDataType() == 'double') {
                                    $reportLabel = trim(strtolower($fieldModel->get('reportlabel')), '`');
                                    $values[$j][] = (float) round($row[$reportLabel], $currentUserModel->no_of_currency_decimals);
                                } else {
                                    $reportLabel = trim(strtolower($fieldModel->get('reportlabel')), '`');
        ...
        $data = ['labels' => $labels, 'values' => $values, 'links' => $links, 'type' => count($values[0]) == 1 ? 'singleBar' : 'multiBar', 'data_labels' => $this->getDataLabels(), 'data_type' => $this->getDataTypes(), 'graph_label' => $this->getGraphLabel(), 'chart_group_type' => 'bar'];
        if ($groupByColumnsByFieldModel[0]->getFieldDataType() == 'multipicklist') {
            $picklistArray = $groupByColumnsByFieldModel[0]->getPicklistValues();
            $data = $this->generateDataMultiplePicklist($data, $picklistArray);
        } else {
    ...
    {
        global $adb;
        $rename_field_result = $adb->pquery('SELECT rename_field_chart  FROM vtiger_vreporttype WHERE reportid = ?', [$recordId]);
