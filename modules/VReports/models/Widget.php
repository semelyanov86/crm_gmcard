...
class VReports_Widget_Model extends Vtiger_Base_Model
{
    ...
    {
        $db = PearDatabase::getInstance();
        $query = $db->pquery('SELECT `history_type` FROM `vtiger_module_vreportdashboard_widgets` WHERE id = ? AND linkid = ?', [$widgetid, $linkId]);
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT * FROM vtiger_module_vreportdashboard_widgets\n\t\t\tINNER JOIN vtiger_links ON vtiger_links.linkid = vtiger_module_vreportdashboard_widgets.linkid\n\t\t\tWHERE linktype = ? AND vtiger_links.linkid = ? AND userid = ?", ['DASHBOARDWIDGET', $linkId, $userId]);
        $self = new self();
        if ($db->num_rows($result)) {
            $row = $db->query_result_rowdata($result, 0);
            $self->setData($row);
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT * FROM vtiger_module_vreportdashboard_widgets\n\t\t\tINNER JOIN vtiger_links ON vtiger_links.linkid = vtiger_module_vreportdashboard_widgets.linkid\n\t\t\tWHERE linktype = ? AND vtiger_module_vreportdashboard_widgets.id = ?", ['DASHBOARDWIDGET', $widgetId]);
        $self = new self();
        if ($db->num_rows($result)) {
            $row = $db->query_result_rowdata($result, 0);
    ...
    public static function getLinkId($moduleName, $componentName)
    {
        ...
        $result = $db->pquery('SELECT linkid FROM `vtiger_links` WHERE tabid = ? AND linktype =? AND linklabel = ?', [$moduleId, 'DASHBOARDWIDGET', $componentName]);
    ...
    public function setHistoryType($type, $historyType, $widgetid, $tabid, $sortandgroup = '')
    {
        ...
        $db->pquery('UPDATE `vtiger_module_vreportdashboard_widgets` SET history_type = ?,history_type_radio=?,group_and_sort=? WHERE id = ? AND dashboardtabid = ?', [$historyType, $type, $sortandgroup, $widgetid, $tabid]);
    }
    ...
    public function add($modeWidget = '')
    {
        ...
        $avoidInsertDupicateWidget = ['KeyMetrics', 'Gauge'];
        if (!$db->num_rows($result) || $this->get('data') && !in_array($this->get('widget_name'), $avoidInsertDupicateWidget)) {
            $db->pquery('INSERT INTO vtiger_module_vreportdashboard_widgets(linkid, userid, filterid, title, data, dashboardtabid, pick_color, refresh_time) VALUES(?,?,?,?,?,?,?,?)', [$this->get('linkid'), $this->get('userid'), $this->get('filterid'), $this->get('title'), Zend_Json::encode($this->get('data')), $tabid, $this->get('color'), $this->get('refresh_time')]);
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT id FROM `vtiger_module_vreportdashboard_widgets` ORDER BY id DESC LIMIT 1', []);
    ...
    {
        $db = PearDatabase::getInstance();
        $db->pquery('DELETE FROM vtiger_module_vreportdashboard_widgets WHERE id = ? AND userid = ?', [$this->get('id'), $this->get('userid')]);
    }
    ...
    {
        $db = PearDatabase::getInstance();
        $db->pquery('DELETE FROM vtiger_module_vreportdashboard_widgets WHERE reportid = ?', [$reportId]);
    }
