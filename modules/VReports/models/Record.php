...
class VReports_Record_Model extends Vtiger_Record_Model
{
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT * FROM vtiger_vreportdatefilter WHERE datefilterid = ? AND startdate != ? AND enddate != ?", array($this->getId(), "0000-00-00", "0000-00-00"));
    ...
    public function save()
    {
        ...
        if (empty($reportId)) {
            ...
            $secondaryModule = $this->getSecondaryModules();
            $db->pquery("INSERT INTO vtiger_vreportmodules(reportmodulesid, primarymodule, secondarymodules) VALUES(?,?,?)", array($reportId, $this->getPrimaryModule(), $secondaryModule));
            ...
            $this->saveSelectedFields();
            $db->pquery("DELETE FROM vtiger_vreportsharing WHERE reportid = ?", array($reportId));
            $this->saveSharingInformation();
            if ($this->get("reporttype") == "sql") {
                $this->set("description", $_REQUEST["description"]);
            }
            $db->pquery("UPDATE vtiger_vreportmodules SET primarymodule = ?,secondarymodules = ? WHERE reportmodulesid = ?", array($this->getPrimaryModule(), $this->getSecondaryModules(), $reportId));
            ...
            $db->pquery("DELETE FROM vtiger_vreportsortcol WHERE reportid = ?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreportgroupbycolumn WHERE reportid = ?", array($reportId));
            $this->saveSortFields();
            $db->pquery("DELETE FROM vtiger_vreportsummary WHERE reportsummaryid = ?", array($reportId));
            $this->saveCalculationFields();
            $db->pquery("DELETE FROM vtiger_vreportdatefilter WHERE datefilterid = ?", array($reportId));
            $this->saveStandardFilter();
            $this->saveReportType();
            $this->saveAdvancedFilters();
    ...
    public function saveSortFields()
    {
        ...
        if (!empty($sortFields)) {
            ...
            foreach ($sortFields as $fieldInfo) {
                $db->pquery("INSERT INTO vtiger_vreportsortcol(sortcolid, reportid, columnname, sortorder) VALUES (?,?,?,?)", array($i, $this->getId(), $fieldInfo[0], $fieldInfo[1]));
    ...
    public function saveCalculationFields()
    {
        ...
        for ($i = 0; $i < $count_calculationFields; $i++) {
            ...
            $db->pquery("INSERT INTO vtiger_vreportsummary (reportsummaryid, summarytype, columnname,column_rename,column_rename_status) VALUES (?,?,?,?,?)", array($this->getId(), $i, $calculationFields[$i], $renameField, $renameFieldStatus));
        }
    }
    /**
    ...
    public function saveSharingInformation()
    {
        if (empty($_REQUEST["view"])) {
            ...
            for ($i = 0; $i < $count_sharingInfo; $i++) {
                $db->pquery("INSERT INTO vtiger_vreportsharing(reportid, shareid, setype) VALUES (?,?,?)", array($reportId, $sharingInfo[$i]["id"], $sharingInfo[$i]["type"]));
            }
            $db->pquery("DELETE FROM vtiger_vreport_shareusers WHERE reportid=?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreport_sharegroups WHERE reportid=?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreport_sharerole WHERE reportid=?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreport_sharers WHERE reportid=?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreport_shareall WHERE reportid=?", array($reportId));
            $members = $this->get("members", array());
            if (!empty($members)) {
                ...
                for ($i = 0; $i < $noOfMembers; $i++) {
                    ...
                    if ($idComponents && count($idComponents) == 2) {
                        ...
                        if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
                            $db->pquery("INSERT INTO vtiger_vreport_shareusers(userid, reportid) VALUES (?,?)", array($memberId, $reportId));
                        }
                        if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
                            $db->pquery("INSERT INTO vtiger_vreport_sharegroups(groupid, reportid) VALUES (?,?)", array($memberId, $reportId));
                        }
                        if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
                            $db->pquery("INSERT INTO vtiger_vreport_sharerole(roleid, reportid) VALUES (?,?)", array($memberId, $reportId));
                        }
                        if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
                            $db->pquery("INSERT INTO vtiger_vreport_sharers(rsid, reportid) VALUES (?,?)", array($memberId, $reportId));
                        }
                        if ($memberType == "ShareAll") {
                            $db->pquery("INSERT INTO vtiger_vreport_shareall(is_shareall, reportid) VALUES (?,?)", array($memberId, $reportId));
                        }
                    }
                }
            } else {
                $db->pquery("INSERT INTO vtiger_vreport_shareusers(userid, reportid) VALUES (?,?)", array($currentUser->getId(), $reportId));
            }
        }
    }
    /**
    ...
    public function saveAdvancedFilters()
    {
        ...
        if (!empty($advancedFilters)) {
            $db->pquery("DELETE FROM vtiger_vreport_relcriteria WHERE queryid = ?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreport_relcriteria_grouping WHERE queryid = ?", array($reportId));
            $db->pquery("DELETE FROM vtiger_vreport_relcriteria_grouping_parent WHERE queryid = ?", array($reportId));
            foreach ($advancedFilters as $groupParentIndex => $advancedFilter) {
                foreach ($advancedFilter as $groupIndex => $groupInfo) {
                    ...
                    $db->pquery("INSERT INTO vtiger_vreport_relcriteria_grouping(groupid, queryid, groupparentid, group_condition, condition_expression) VALUES (?,?,?,?,?)", array($groupIndex, $reportId, $groupParentIndex, $groupCondition, $groupConditionExpression));
                }
                $db->pquery("INSERT INTO vtiger_vreport_relcriteria_grouping_parent(groupparentid,queryid,group_parent_condition) VALUES(?,?,?)", array($groupParentIndex, $reportId, $advancedFilter["groupParentCondition"]));
            }
        }
    }
    /**
    ...
    public function getReportCalulationData($filterQuery = false)
    {
        ...
        $data[] = array("count_record" => $dataDetail["count"]);
        return $data;
    }
    /**
    ...
    {
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE vtiger_vreport SET folderid = ? WHERE reportid = ?", array($folderId, $this->getId()));
    }
    /**
    ...
    public function saveReportType()
    {
        ...
        if ($_REQUEST["record"] != "" && $_REQUEST["isDuplicate"] != "1") {
            if (isset($_REQUEST["step"])) {
                $db->pquery("UPDATE vtiger_vreporttype SET `data` = ? ,`rename_field` = ?,rename_field_chart=? WHERE reportid =?", array($data, $renameData, $renameChart, $this->get("reportid")));
            } else {
                $db->pquery("UPDATE vtiger_vreporttype SET `data` = ? ,`rename_field` = ?,rename_field_chart=?,sort_by=?,`limit`=?,`order_by`=? WHERE reportid =?", array($data, $renameData, $renameChart, json_encode($this->get("sort_by")), $this->get("limit"), $this->get("order_by"), $this->get("reportid")));
            }
        } else {
            $db->pquery("DELETE FROM vtiger_vreporttype WHERE reportid = ?", array($this->getId()));
            $db->pquery("INSERT INTO vtiger_vreporttype(reportid, data,rename_field,rename_field_chart,sort_by,`limit`,order_by) VALUES (?,?,?,?,?,?,?)", array($this->getId(), $data, $renameData, $renameChart, json_encode($this->get("sort_by")), $this->get("limit"), $this->get("order_by")));
        }
    }
    public function getReportTypeInfo()
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT data FROM vtiger_vreporttype WHERE reportid = ?", array($this->getId()));
        $dataFields = "";
    ...
    public function isPinnedToDashboard($tabid)
    {
        $db = PearDatabase::getInstance();
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery("SELECT 1 FROM vtiger_module_vreportdashboard_widgets WHERE reportid = ? AND userid = ? AND dashboardtabid = ?", array($this->getId(), $currentUser->getId(), $tabid));
        if ($db->num_rows($result)) {
            return true;
        }
        return false;
    ...
    public function isEditableBySharing()
    {
        $db = PearDatabase::getInstance();
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $ownerResult = $db->pquery("SELECT `owner` FROM vtiger_vreport WHERE reportid = ?", array($this->getId()));
        ...
        $rsUserSharing = $db->pquery("SELECT 1 FROM vtiger_vreport_shareusers WHERE reportid = ? AND  userid = ?", array($reportId, $currentUser->getId()));
        if (0 < $db->num_rows($rsUserSharing)) {
            return true;
        }
        $rsRoleSharing = $db->pquery("SELECT 1 FROM vtiger_vreport_sharerole WHERE reportid = ? AND  roleid = ?", array($reportId, $currentUser->getRole()));
        if (0 < $db->num_rows($rsRoleSharing)) {
            return true;
        }
        $rsGroupsSharing = $db->pquery("SELECT * FROM vtiger_vreport_sharegroups WHERE reportid = ?", array($reportId));
        ...
        $rsRoleSharing = $db->pquery("SELECT * FROM vtiger_vreport_sharers WHERE reportid = ?", array($reportId));
    ...
    public static function getLinkId($recordId)
    {
        global $adb;
        $linkid = 0;
        $result = $adb->pquery("SELECT linkid FROM vtiger_links WHERE tabid =? AND linkurl LIKE '%reportid=" . $recordId . "%'", array(getTabid("VReports")));
