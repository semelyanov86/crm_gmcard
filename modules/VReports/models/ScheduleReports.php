...
class VReports_ScheduleReports_Model extends Vtiger_Base_Model
{
    ...
    public static function getInstanceById($recordId)
    {
        ...
        if (!empty($recordId)) {
            $scheduledReportResult = $db->pquery('SELECT * FROM vtiger_schedulevreports WHERE reportid = ?', [$recordId]);
    ...
    public function saveScheduleReport()
    {
        ...
        if (!preg_match('/^[0-2]\\d(:[0-5]\\d){1,2}$/', $schtime) || substr($schtime, 0, 2) > 23) {
            $schtime = '00:00';
        }
        $schtime .= ':00';
        ...
        if ($scheduleid == self::$SCHEDULED_ON_SPECIFIC_DATE) {
            ...
            $schdate = Zend_Json::encode([$dateDBFormat]);
        } else {
        ...
        if ($isReportScheduled == '0' || $isReportScheduled == '' || $isReportScheduled == false) {
            ...
            $checkScheduledResult = $adb->pquery('SELECT next_trigger_time FROM vtiger_schedulevreports WHERE reportid=?', [$reportid]);
            if ($adb->num_rows($checkScheduledResult) > 0) {
                ...
                $adb->pquery($scheduledReportSql, [$scheduleid, $recipients, $schdate, $schtime, $schdayoftheweek, $schdayofthemonth, $schannualdates, $specificemails, $from_address, $subject, $body, $nextTriggerTime, $fileFormat, $signature, $signature_user, $reportid]);
            } else {
                ...
                $adb->pquery($scheduleReportSql, [$reportid, $scheduleid, $recipients, $schdate, $schtime, $schdayoftheweek, $schdayofthemonth, $schannualdates, $nextTriggerTime, $specificemails, $from_address, $subject, $body, $fileFormat, $signature, $signature_user]);
            }
        }
    }
    ...
    public function sendEmail()
    {
        ...
        if ($this->get('signature') == 'Yes') {
            ...
            $userSignature = nl2br($db->query_result($db->pquery('select signature from vtiger_users where id=?', [$userId]), 0, 'signature'));
        ...
        $params = [$reportRecordModel->get('reportid'), $reportRecordModel->get('reportname'), $currentTimeSendMail, $this->get('from_address'), implode(' |##| ', $this->getRecipientEmails()), '', '', $this->get('subject'), $this->get('body'), $status];
    ...
    {
        $db = PearDatabase::getInstance();
        $smtpFromResult = $db->pquery('SELECT from_email_field FROM vtiger_systems WHERE server_type=?', ['email']);
    ...
    public function getNextTriggerTime()
    {
        ...
        if ($scheduleType == self::$SCHEDULED_DAILY) {
            ...
            $adminDateTime = new DateTimeImmutable('now', new DateTimeZone($adminTimeZone));
            $adminDate = $adminDateTime->format('Y-m-d');
            $schtime = $this->get('schtime');
            $timeZone = new DateTimeZone($default_timezone);
            $userTZ = new DateTimeZone($adminTimeZone);
            $datetime = new DateTimeImmutable($adminDate . ' ' . $schtime, $userTZ);
    ...
    public function updateNextTriggerTime()
    {
        ...
        $adb->pquery('UPDATE vtiger_schedulevreports SET next_trigger_time=? WHERE reportid=?', [$nextTriggerTime, $this->get('reportid')]);
        if (defined('ALLOW_MODULE_LOGGING')) {
    ...
    public static function getScheduledVReports()
    {
        ...
        $result = $adb->pquery($query, [$currentTimestamp, $currentTimestamp]);
        $scheduledReports = [];
    ...
    public static function runScheduledVReports()
    {
        ...
        foreach ($scheduledReports as $reportId => $scheduledReport) {
            ...
            if ($reportType == 'chart') {
                ...
                if ($reportRecordModel->getVReportsCount($countQuery) > 0) {
                    $status = $scheduledReport->sendEmail();
                }
            }
            if (defined('ALLOW_MODULE_LOGGING')) {
