...
class VReports_DashBoard_Model extends Vtiger_Base_Model
{
        ...
        for ($i = 0; $i < $db->num_rows($result); $i++) {
            ...
            if ($this->checkModulePermission($row)) {
                ...
                if ($typeReport == "Tabular") {
                    ...
                    if (in_array($row["linklabel"], array("Mini List VReports", "Key Metrics", "History", "Gauge"))) {
    ...
    public function getDashboards($moduleDashboard)
    {
        ...
        $moduleModel = $this->getModule();
        $rsCheck = $db->pquery("select boardname from vtiger_vreportdashboard_boards \n INNER JOIN vtiger_vreportdashboard_tabs on vtiger_vreportdashboard_tabs.boardid = vtiger_vreportdashboard_boards.id where vtiger_vreportdashboard_tabs.id = ?", array($this->get("tabid")));
    ...
    public function getAllBoards($mode = "", $boardId = 0)
    {
        ...
        if ($mode == "getAll") {
            ...
            $result = $db->pquery("select * from vtiger_vreportdashboard_boards WHERE id = ?", array($boardId));
    ...
    public function checkBoardIsShared($boardId)
    {
        ...
        $db = PearDatabase::getInstance();
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery("SELECT * FROM vtiger_vreportdashboard_boards WHERE id = ? and userid = ?", array($boardId, $currentUser->getId()));
    ...
    public function addTab($tabName, $boardId)
    {
        $db = PearDatabase::getInstance();
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery("SELECT MAX(sequence)+1 AS sequence FROM vtiger_vreportdashboard_tabs where boardid = ?", array($boardId));
    ...
    public function checkTabExist($tabName, $boardId, $mode = "")
    {
        ...
        $currentUser = Users_Record_Model::getCurrentUserModel();
        if ($mode == "check") {
            $result = $db->pquery("SELECT * FROM `vtiger_vreportdashboard_tabs` WHERE id = ? AND tabname = ? AND  isdefault = ?", array(1, "Default", 1));
        } else {
            $result = $db->pquery("SELECT * FROM vtiger_vreportdashboard_tabs WHERE tabname=? AND boardid = ? AND userid=?", array($tabName, $boardId, $currentUser->getId()));
        }
    ...
    public function addTabDefault()
    {
        global $site_URL;
        global $adb;
        $result = $adb->pquery("SELECT * FROM vtiger_vreportdashboard_tabs WHERE id = ?", array(1));
        if (0 < $adb->num_rows($result)) {
            $tabname = $adb->query_result($result, 0, "tabname");
            if ($tabname != "Default") {
                $adb->pquery("UPDATE vtiger_vreportdashboard_tabs SET tabname = ? WHERE id = ?", array("Default", 1));
            }
        } else {
            $adb->pquery("INSERT INTO `vtiger_vreportdashboard_tabs` (`id`,`tabname`,`isdefault`,`sequence`,`userid`,`boardid`) VALUES (?, ?, ?, ?, ?, ?)", array(1, "Default", 1, 1, 0, 1));
        }
    ...
    {
        global $adb;
        $query = $adb->pquery("SELECT `dynamic_filter_account` FROM `vtiger_vreportdashboard_tabs` WHERE id = ? LIMIT 1", array($tabid));
    ...
    public function findMissingWidget($mode)
    {
        ...
        while ($result = $adb->fetch_array($errorFromWidget)) {
            ...
            if (!$typeWidget) {
                ...
                $errorWidgetLink[$result["reportid"]]["query"] = $adb->convert2Sql("UPDATE vtiger_module_vreportdashboard_widgets SET title = ? WHERE linkid = ?", array($typeWidget, $result["linkid"]));
                if ($mode == "fix") {
