...
class VReports_ListView_Model extends Vtiger_ListView_Model
{
    ...
    public function getListViewLinks($linkParams = false)
    {
        ...
        $links = Vtiger_Link_Model::getAllByType($moduleModel->getId(), ['LISTVIEWBASIC']);
    ...
    public function getListViewMassActions($linkParams = false)
    {
        ...
        if ($currentUserModel->hasModulePermission($this->getModule()->getId())) {
            $massActionLinks[] = ['linktype' => 'LISTVIEWMASSACTION', 'linklabel' => 'LBL_DELETE', 'linkurl' => 'javascript:VReports_List_Js.massDelete("index.php?module=' . $this->getModule()->get('name') . '&action=MassDelete");', 'linkicon' => ''];
            $massActionLinks[] = ['linktype' => 'LISTVIEWMASSACTION', 'linklabel' => 'LBL_MOVE_REPORT', 'linkurl' => 'javascript:VReports_List_Js.massMove("index.php?module=' . $this->getModule()->get('name') . '&view=MoveReports");', 'linkicon' => ''];
        }
    ...
    {
        $headers = ['reporttype' => ['label' => 'Report Type', 'type' => 'picklist'], 'reportname' => ['label' => 'Report Name', 'type' => 'string'], 'primarymodule' => ['label' => 'Primary Module', 'type' => 'picklist'], 'foldername' => ['label' => 'LBL_FOLDER_NAME', 'type' => 'picklist'], 'modifiedtime' => ['label' => 'Modified Time', 'type' => 'picklist'], 'sharingtype' => ['label' => 'Sharing Type'], 'schedule' => ['label' => 'Schedule']];
        if ($folderId == 'shared') {
    ...
    {
        $headers = ['reportname' => ['label' => 'LBL_REPORT_NAME', 'type' => 'string'], 'description' => ['label' => 'LBL_DESCRIPTION', 'type' => 'string'], 'reporttype' => ['label' => 'Report Type', 'type' => 'picklist'], 'sharingtype' => ['label' => 'Sharing Type'], 'schedule' => ['label' => 'Schedule']];
        if ($folderId == 'All' || $folderId == 'Public') {
            $headers['foldername'] = ['label' => 'LBL_FOLDER_NAME', 'type' => 'picklist'];
        }
