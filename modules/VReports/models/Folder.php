...
class VReports_Folder_Model extends Vtiger_Base_Model
{
    ...
    public function save()
    {
        ...
        $folderId = $this->getId();
        if (!empty($folderId)) {
            $db->pquery('UPDATE vtiger_vreportfolder SET foldername = ?, description = ? WHERE folderid = ?', [$this->getName(), $this->getDescription(), $folderId]);
        } else {
            $result = $db->pquery('SELECT MAX(folderid) AS folderid FROM vtiger_vreportfolder', []);
            $folderId = (int) $db->query_result($result, 0, 'folderid') + 1;
            $db->pquery('INSERT INTO vtiger_vreportfolder(folderid, foldername, description, state) VALUES(?, ?, ?, ?)', [$folderId, $this->getName(), $this->getDescription(), 'CUSTOMIZED']);
    ...
    {
        $db = PearDatabase::getInstance();
        $db->pquery('DELETE FROM vtiger_vreportfolder WHERE folderid = ?', [$this->getId()]);
    }
    ...
    {
        $paramsList = ['startIndex' => $pagingModel->getStartIndex(), 'pageLimit' => $pagingModel->getPageLimit(), 'orderBy' => $this->get('orderby'), 'sortBy' => $this->get('sortby')];
    ...
    public static function getInstanceById($folderId)
    {
        ...
        if (!$folderModel) {
            ...
            $result = $db->pquery('SELECT * FROM vtiger_vreportfolder WHERE folderid = ?', [$folderId]);
    ...
    public static function getAll()
    {
        ...
        if (!$folders) {
            $folders = [];
            $result = $db->pquery('SELECT * FROM vtiger_vreportfolder ORDER BY foldername ASC', []);
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT 1 FROM vtiger_vreport WHERE folderid = ?', [$this->getId()]);
        if ($db->num_rows($result) > 0) {
            return true;
        }
    ...
    {
        return ['folderId' => $this->getId(), 'folderName' => $this->getName(), 'editURL' => $this->getEditUrl(), 'deleteURL' => $this->getDeleteUrl(), 'isEditable' => $this->isEditable(), 'isDeletable' => $this->isDeletable()];
    }
     ...
     */
    public function getRecordIds($skipRecords, $module, $searchParams = [])
    {
        $db = PearDatabase::getInstance();
     ...
     * @return <Reports_Record_Model>
     */
    public function getListViewQuery($folderId, $searchParams = [])
    {
