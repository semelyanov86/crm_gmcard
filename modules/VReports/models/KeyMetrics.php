...
class VReports_KeyMetrics_Model extends Vtiger_MiniList_Model
{
    public function getShareIdForUser($current_user)
    {
        ...
        $cvid = "";
        $query = $db->pquery("SELECT cvid FROM vtiger_cv2users WHERE userid = ?", array($id));
        ...
        $userRole = trim($userRole, "\"");
        $query2 = $db->pquery("SELECT cvid FROM vtiger_cv2role WHERE roleid in (" . $userRole . ")", array());
        if (0 < $db->num_rows($query2)) {
