...
class VReports_MiniList_Model extends Vtiger_MiniList_Model
{
    ...
    public function getRecords($mode = '', $limit = '')
    {
        ...
        if (!$this->listviewRecords || $mode) {
            ...
            $data = $this->extraData;
            if ((count($this->dynamicFilterAssignedTo) > 0 || count($this->dynamicFilterCreatedBy) > 0) && $data['orderField'] == '' && $data['orderField1'] == '') {
                if (count($this->dynamicFilterAssignedTo) > 0) {
                    if ($this->isDeletedRecord($this->dynamicFilterAssignedTo['recordId'])) {
                        return false;
                    }
                    $query .= $this->dynamicFilterAssignedTo['query'];
                }
                if (count($this->dynamicFilterCreatedBy) > 0) {
    ...
    public function moreRecordExists()
    {
        ...
        if ($adb->num_rows($resultDynamicFilter) > 0 || $widgetFilterAssignedto != '' && $widgetFilterAssignedto != null || $checkAccountUrl == 'organization') {
            ...
            $glue = '';
            if (count($this->queryGenerator->getWhereFields()) > 0 && count($searchParams) > 0) {
        ...
        if ($dynamicFilterAssignedTo) {
            ...
            if (!in_array($conditionDynamicFilterAssignedTo, $assign_to)) {
                $this->dynamicFilterAssignedTo = ['query' => ' AND vtiger_crmentity.smownerid = ' . $dynamicFilterAssignedTo];
                $query .= $this->dynamicFilterAssignedTo['query'];
            }
        }
        if ($dynamicFilterCreatedBy) {
            ...
            foreach ($assign_to as $assign) {
                if (strpos($assign, 'smcreatorid') > 0) {
                    $checkDynamicFilter = true;
                }
            }
            if (!$checkDynamicFilter) {
                $this->dynamicFilterCreatedBy = ['query' => ' AND vtiger_crmentity.smcreatorid = ' . $dynamicFilterCreatedBy];
                $query .= $this->dynamicFilterCreatedBy['query'];
            }
        }
        if ($dynamicFilterDate) {
            ...
            $this->dynamicFilterAssignedTo .= ['query' => ' AND ' . $advfiltergroupsql];
        }
    ...
    {
        global $adb;
        $result = $adb->pquery('SELECT 1 FROM vtiger_crmentity WHERE crmid = ? AND deleted = 1', [$id]);
        if ($adb->num_rows($result) > 0) {
            return true;
        }
