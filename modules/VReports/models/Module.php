...
class VReports_Module_Model extends Vtiger_Module_Model
{
    ...
    public function deleteRecord($reportModel)
    {
        ...
        if ($currentUser->isAdminUser() || in_array($owner, $subOrdinates) || $owner == $currentUser->getId()) {
            ...
            $db->pquery('DELETE FROM vtiger_vreport WHERE reportid = ?', [$reportId]);
            $db->pquery('DELETE FROM vtiger_schedulevreports WHERE reportid = ?', [$reportId]);
            $db->pquery('DELETE FROM vtiger_vreporttype WHERE reportid = ?', [$reportId]);
            $result = $db->pquery('SELECT * FROM vtiger_homevreportchart WHERE reportid = ?', [$reportId]);
    ...
    public function getSettingLinks()
    {
        ...
        $settingsLinks[] = ['linktype' => 'MODULESETTING', 'linklabel' => 'Settings', 'linkurl' => 'index.php?module=VReports&parent=Settings&view=Settings', 'linkicon' => ''];
        $settingsLinks[] = ['linktype' => 'MODULESETTING', 'linklabel' => 'Uninstall', 'linkurl' => 'index.php?module=VReports&parent=Settings&view=Uninstall', 'linkicon' => ''];
    ...
    {
        $quickLinks = [['linktype' => 'SIDEBARLINK', 'linklabel' => 'LBL_VREPORTS', 'linkurl' => $this->getListViewUrl(), 'linkicon' => '']];
        foreach ($quickLinks as $quickLink) {
            $links['SIDEBARLINK'][] = Vtiger_Link_Model::getInstanceFromValues($quickLink);
        }
        $quickWidgets = [['linktype' => 'SIDEBARWIDGET', 'linklabel' => 'LBL_RECENTLY_MODIFIED', 'linkurl' => 'module=' . $this->get('name') . '&view=IndexAjax&mode=showActiveRecords', 'linkicon' => '']];
