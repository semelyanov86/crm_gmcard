...
class VReports_Gauge_Model extends Vtiger_MiniList_Model
{
    public function getListDetailReport()
    {
        ...
        $result = $adb->pquery("SELECT * FROM vtiger_vreport WHERE reporttype = ?", array("tabular"));
    ...
    {
        global $adb;
        $adb->pquery("UPDATE vtiger_module_vreportdashboard_widgets SET sizeHeight = ? , sizeWidth = ? WHERE id = ?", array(2, 2, $widgetId));
    }
}
