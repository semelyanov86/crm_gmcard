...
{
    static $options = ['101', '116', '117', '26', '357', '50', '51', '52', '53', '57', '58', '59', '66', '68', '73', '75', '76', '77', '78', '80', '81'];
...
{
    [$tablename, $colname, $module_field, $fieldname, $typeOfData] = explode(':', $reportColDetails);
    if ($typeOfData == 'D') {
        return true;
    }
...
function getVReportFieldValue($report, $picklistArray, $dbField, $valueArray, $fieldName, $operation = false)
{
    ...
    [$module, $fieldLabel] = explode('_', $dbField->name, 2);
    ...
    if ($report->primarymodule == 'PriceBooks' && $fieldLabel == 'List_Price') {
        $fieldInfo = ['tabid' => getTabid($module), 'fieldid' => '', 'fieldname' => 'listprice', 'fieldlabel' => 'List Price', 'columnname' => 'listprice', 'tablename' => 'vtiger_pricebookproductrel', 'uitype' => 72, 'typeofdata' => 'Currency', 'presence' => 0];
        $field = WebserviceField::fromArray($db, $fieldInfo);
        $fieldType = $field->getFieldDataType();
    }
    if (is_object($field) && $field->getUIType() == 401) {
        ...
        if ($fieldType == 'currency' && $value != '') {
            if ($field->getUIType() == '72') {
                ...
                if (count($curid_value) > 1) {
                    [$currency_id, $currency_value] = $curid_value;
                } else {
    ...
    if ($operation == 'ExcelExport') {
        return ['value' => htmlentities($fieldvalue, ENT_QUOTES, $default_charset), 'type' => $fieldType];
    }
...
function getVReportSearchCondition($searchParams, $filterId)
{
    if (!empty($searchParams)) {
        ...
        foreach ($searchParams as $i => $condition) {
            [$fieldName, $searchValue] = $condition;
