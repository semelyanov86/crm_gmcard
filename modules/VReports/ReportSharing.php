...
function getVisibleCriteria($recordid = "")
{
    ...
    $selcriteria = "";
    if ($recordid != "") {
        $result = $adb->pquery("select sharingtype from vtiger_vreport where reportid=?", array($recordid));
...
{
    global $adb;
    $member_query = $adb->pquery("SELECT vtiger_vreportsharing.setype,vtiger_users.id,vtiger_users.user_name FROM vtiger_vreportsharing INNER JOIN vtiger_users on vtiger_users.id = vtiger_vreportsharing.shareid WHERE vtiger_vreportsharing.setype='users' AND vtiger_vreportsharing.reportid = ?", array($recordid));
    ...
    $member_query = $adb->pquery("SELECT vtiger_vreportsharing.setype,vtiger_groups.groupid,vtiger_groups.groupname FROM vtiger_vreportsharing INNER JOIN vtiger_groups on vtiger_groups.groupid = vtiger_vreportsharing.shareid WHERE vtiger_vreportsharing.setype='groups' AND vtiger_vreportsharing.reportid = ?", array($recordid));
    $noofrows = $adb->num_rows($member_query);
    if (0 < $noofrows) {
        for ($i = 0; $i < $noofrows; $i++) {
