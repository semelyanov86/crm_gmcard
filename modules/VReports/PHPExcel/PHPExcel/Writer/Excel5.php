...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    private function buildWorksheetEschers()
    {
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                // set the BLIP index
                $spContainer->setOPT(0x41_04, $blipIndex);
            ...
            if (!empty($filterRange)) {
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    $spContainer->setOPT(0x00_7F, 0x01_04_01_04); // Protection -> fLockAgainstGrouping
                    $spContainer->setOPT(0x00_BF, 0x00_08_00_08); // Text -> fFitTextToShape
                    $spContainer->setOPT(0x01_BF, 0x00_01_00_00); // Fill Style -> fNoFillHitTest
                    $spContainer->setOPT(0x01_FF, 0x00_08_00_00); // Line Style -> fNoLineDrawDash
                    $spContainer->setOPT(0x03_BF, 0x00_0A_00_00); // Group Shape -> fPrint
    ...
    private function writeDocumentSummaryInformation()
    {
        ...
        $data .= pack('vvvvvvvv', 0xD5_02, 0xD5_CD, 0x2E_9C, 0x10_1B, 0x97_93, 0x00_08, 0x2C_2B, 0xAE_F9);
        ...
        $dataSection[] = ['summary' => ['pack' => 'V', 'data' => 0x17],
            ...
            'data'    => ['pack' => 'V', 'data' => 0x00_0C_00_00]];
        ++$dataSection_NumProps;
        ...
        // cElements
        $dataProp = pack('v', 0x00_01);
        $dataProp .= pack('v', 0x00_00);
        // array of UnalignedLpstr
        // cch
        $dataProp .= pack('v', 0x00_0A);
        $dataProp .= pack('v', 0x00_00);
        // value
        ...
        foreach ($dataSection as $dataProp) {
            ...
            if ($dataProp['type']['data'] == 0x02) { // 2 byte signed integer
                ...
                if ($dataProp['data']['data'] == false) {
                    $dataSection_Content .= pack('V', 0x00_00);
                } else {
                    $dataSection_Content .= pack('V', 0x00_01);
                }
                $dataSection_Content_Offset += 4 + 4;
    ...
    private function writeSummaryInformation()
    {
        ...
        $data .= pack('vvvvvvvv', 0x85_E0, 0xF2_9F, 0x4F_F9, 0x10_68, 0x91_AB, 0x00_08, 0x27_2B, 0xD9_B3);
        // offset: 44; size: 4; offset of the start
        $data .= pack('V', 0x30);
