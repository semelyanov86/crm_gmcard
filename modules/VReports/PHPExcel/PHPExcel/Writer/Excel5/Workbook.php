...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    private function writeAllDefinedNamesBiff8()
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
                ...
                $formulaData .= pack('Cvvvvv', 0x3B, $i, 0, 65_535, $colmin, $colmax); // tArea3d
    ...
    {
        $record = 0x00_42;             // Record identifier
    ...
    {
        $record = 0x00_3D;   // Record identifier
        $length = 0x00_12;   // Number of bytes to follow

        $xWn  = 0x00_00;     // Horizontal position of window
        $yWn  = 0x00_00;     // Vertical position of window
        $dxWn = 0x25_BC;     // Width of window
        $dyWn = 0x15_72;     // Height of window

        $grbit = 0x00_38;    // Option flags

        // not supported by PHPExcel, so there is only one selected sheet, the active
        $ctabsel = 1;       // Number of workbook tabs selected

        $wTabRatio = 0x02_58;    // Tab to scrollbar ratio
    ...
    private function writeBoundSheet($sheet, $offset)
    {
        ...
        $record    = 0x00_85;                    // Record identifier
        ...
        $grbit = 0x00_00;                    // Visibility and sheet type
    ...
    {
        $record    = 0x01_AE;   // Record identifier
        $length    = 0x00_04;   // Bytes to follow

        $header    = pack('vv', $record, $length);
        $data      = pack('vv', $this->phpExcel->getSheetCount(), 0x04_01);
    ...
    {
        $record = 0x02_93;   // Record identifier
        $length = 0x00_04;   // Bytes to follow

        $ixfe    = 0x80_00;  // Index to cell style XF
    ...
    {
        $record = 0x04_1E;    // Record identifier
    ...
    {
        $record = 0x00_22;   // Record identifier
    ...
    {
        $record = 0x00_18;       // Record identifier
        $length = 0x00_24;       // Number of bytes to follow

        $grbit  = 0x00_20;       // Option flags
        $chKey  = 0x00;         // Keyboard shortcut
        $cch    = 0x01;         // Length of text name
        $cce    = 0x00_15;       // Length of text definition
    ...
    {
        $record          = 0x00_18;       // Record identifier
        $length          = 0x00_3D;       // Number of bytes to follow
        $grbit           = 0x00_20;       // Option flags
        $chKey           = 0x00;         // Keyboard shortcut
        $cch             = 0x01;         // Length of text name
        $cce             = 0x00_2E;       // Length of text definition
        $ixals           = $index + 1;   // Sheet index
        $itab            = $ixals;       // Equal to ixals
        $cchCustMenu     = 0x00;         // Length of cust menu text
        $cchDescription  = 0x00;         // Length of description text
    ...
    {
        $record = 0x00_8C;    // Record identifier
    ...
    {
        $record = 0x01_C1;    // Record identifier
        $length = 8;         // Number of bytes to follow

        $header = pack('vv', $record, $length);

        // by inspection of real Excel files, MS Office Excel 2007 writes this
        $data = pack('VV', 0x00_00_01_C1, 0x0_00_01_E6_67);
    ...
    private function writePalette()
    {
        ...
        $record = 0x00_92;                // Record identifier
