...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    public function __construct(&$str_total, &$str_unique, &$str_table, &$colors, $parser, $preCalculateFormulas, $phpSheet)
    {
        ...
        $this->lastRowIndex = ($maxR > 65_535) ? 65_535 : $maxR;
    ...
    {
        $record    = 0x02_03;                 // Record identifier
        $length    = 0x00_0E;                 // Number of bytes to follow
    ...
    {
        $record    = 0x00_FD;                   // Record identifier
        $length    = 0x00_0A;                   // Bytes to follow
    ...
    {
        $strlen    = strlen($str);
        $record    = 0x02_04;                   // Record identifier
        $length    = 0x00_08 + $strlen;         // Bytes to follow

        $str_error = 0;

        if ($strlen > $this->xlsStringMaxLength) { // LABEL must be < 255 chars
            $str    = substr($str, 0, $this->xlsStringMaxLength);
            $length    = 0x00_08 + $this->xlsStringMaxLength;
    ...
    private function writeNote($row, $col, $note)
    {
        ...
        $record            = 0x00_1C;            // Record identifier
        $max_length        = 2_048;                // Maximun length for a NOTE record

        // Length for this record is no more than 2048 + 6
        $length    = 0x00_06 + min($note_length, 2_048);
        $header    = pack('vv', $record, $length);
        $data    = pack('vvv', $row, $col, $note_length);
        $this->append($header . $data . substr($note, 0, 2_048));

        for ($i = $max_length; $i < $note_length; $i += $max_length) {
            $chunk  = substr($note, $i, $max_length);
            $length = 0x00_06 + strlen($chunk);
            $header = pack('vv', $record, $length);
    ...
    {
        $record    = 0x02_01;                 // Record identifier
        $length    = 0x00_06;                 // Number of bytes to follow
    ...
    {
        $record    = 0x00_06;     // Record identifier
        ...
        if (isset($calculatedValue)) {
            ...
            if (is_bool($calculatedValue)) {
                // Boolean value
                $num = pack('CCCvCv', 0x01, 0x00, (int) $calculatedValue, 0x00, 0x00, 0xFF_FF);
                ...
                if (array_key_exists($calculatedValue, PHPExcel_Cell_DataType::getErrorCodes())) {
                    // Error value
                    $num = pack('CCCvCv', 0x02, 0x00, self::mapErrorCode($calculatedValue), 0x00, 0x00, 0xFF_FF);
                } elseif ($calculatedValue === '') {
                    // Empty string (and BIFF8)
                    $num = pack('CCCvCv', 0x03, 0x00, 0x00, 0x00, 0x00, 0xFF_FF);
                } else {
                    // Non-empty string value (or empty string BIFF5)
                    $stringValue = $calculatedValue;
                    $num = pack('CCCvCv', 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF_FF);
                }
            } else {
    ...
    {
        $record = 0x02_07;     // Record identifier
    ...
    {
        $record      = 0x01_B8;                       // Record identifier
        $length      = 0x0_00_00;                      // Bytes to follow
    ...
    {
        $record      = 0x02_08;               // Record identifier
        $length      = 0x00_10;               // Number of bytes to follow

        $colMic      = 0x00_00;               // First defined column
        $colMac      = 0x00_00;               // Last defined column
        $irwMac      = 0x00_00;               // Used by Excel to optimise loading
        $reserved    = 0x00_00;               // Reserved
        $grbit       = 0x00_00;               // Option flags
        $ixfe        = $xfIndex;
    ...
    {
        $record = 0x02_00; // Record identifier

        $length = 0x00_0E;
        $data = pack('VVvvv', $this->firstRowIndex, $this->lastRowIndex + 1, $this->firstColumnIndex, $this->lastColumnIndex + 1, 0x00_00); // reserved
    ...
    {
        $record         = 0x02_3E;     // Record identifier
        $length         = 0x00_12;

        $grbit          = 0x00_B6;     // Option flags
        $rwTop          = 0x00_00;     // Top row visible in window
        $colLeft        = 0x00_00;     // Leftmost column visible in window
        ...
        // FIXME !!!
        $rgbHdr       = 0x00_40; // Row/column heading and gridline color index
        $zoom_factor_page_break = ($fPageBreakPreview ? $this->phpSheet->getSheetView()->getZoomScale() : 0x00_00);
        $zoom_factor_normal =  $this->phpSheet->getSheetView()->getZoomScaleNormal();

        $data .= pack('vvvvV', $rgbHdr, 0x00_00, $zoom_factor_page_break, $zoom_factor_normal, 0x00_00_00_00);
    ...
    private function writeDefaultRowHeight()
    {
        ...
        $record   = 0x02_25;      // Record identifier
        $length   = 0x00_04;      // Number of bytes to follow
    ...
    private function writeDefcol()
    {
        ...
        $record   = 0x00_55;      // Record identifier
        $length   = 0x00_02;      // Number of bytes to follow
    ...
    private function writeColinfo($col_array)
    {
        ...
        $record   = 0x00_7D;          // Record identifier
        $length   = 0x00_0C;          // Number of bytes to follow
    ...
    private function writeSelection()
    {
        ...
        $rwFirst = min($rwFirst, 65_535);
        $rwLast  = min($rwLast, 65_535);

        $record   = 0x00_1D;                  // Record identifier
        $length   = 0x00_0F;                  // Number of bytes to follow
    ...
    private function writeMergedCells()
    {
        ...
        $maxCountMergeCellsPerRecord = 1_027;
    ...
    private function writeSheetLayout()
    {
        ...
        $recordData = pack(
            ...
            0x00_00_00_00, // unused
            0x00_00_00_14, // size of record data
        ...
        $record = 0x08_62; // Record identifier
        $header = pack('vv', $record, $length);
        $this->append($header . $recordData);
    }
    ...
    private function writeSheetProtection()
    {
        ...
        $recordData = pack(
            'vVVCVVvv',
            0x08_67, // repeated record identifier
            0x00_00, // not used
            0x00_00, // not used
            0x00, // not used
            0x01_00_02_00, // unknown data
            0xFF_FF_FF_FF, // unknown data
            $options, // options
            0x00_00, // not used
        );
    ...
    private function writeRangeProtection()
    {
        foreach ($this->phpSheet->getProtectedCells() as $range => $password) {
            ...
            $record = 0x08_68;        // Record identifier
            $header = pack('vv', $record, $length);
            $this->append($header . $recordData);
        }
    }
    ...
    {
        $record = 0x00_16;          // Record identifier
        $length = 0x00_02;          // Number of bytes to follow
    ...
    {
        $record    = 0x00_17;         // Record identifier
    ...
    private function writePanes()
    {
        ...
        $record  = 0x00_41;       // Record identifier
        $length  = 0x00_0A;       // Number of bytes to follow
    ...
    {
        $record       = 0x00_A1;                  // Record identifier
        $length       = 0x00_22;                  // Number of bytes to follow
        ...
        $iRes         = 0x02_58;               // Print resolution
        $iVRes        = 0x02_58;               // Vertical print resolution
    ...
    {
        $record  = 0x00_14;               // Record identifier
    ...
    {
        $record  = 0x00_15;               // Record identifier
    ...
    {
        $record   = 0x00_83;              // Record identifier
        $length   = 0x00_02;              // Bytes to follow
    ...
    {
        $record   = 0x00_84;              // Record identifier
        $length   = 0x00_02;              // Bytes to follow
    ...
    {
        $record  = 0x00_26;                   // Record identifier
        $length  = 0x00_08;                   // Bytes to follow
    ...
    {
        $record  = 0x00_27;                   // Record identifier
        $length  = 0x00_08;                   // Bytes to follow
    ...
    {
        $record  = 0x00_28;                   // Record identifier
        $length  = 0x00_08;                   // Bytes to follow
    ...
    {
        $record  = 0x00_29;                   // Record identifier
        $length  = 0x00_08;                   // Bytes to follow
    ...
    {
        $record      = 0x00_2A;                   // Record identifier
        $length      = 0x00_02;                   // Bytes to follow
    ...
    {
        $record      = 0x00_2B;                    // Record identifier
        $length      = 0x00_02;                    // Bytes to follow
    ...
    {
        $record      = 0x00_82;                        // Record identifier
        $length      = 0x00_02;                        // Bytes to follow
    ...
    {
        $record      = 0x00_9D;                        // Record identifier
        $length      = 0x00_02;                        // Bytes to follow
    ...
    {
        $record      = 0x00_80;   // Record identifier
        $length      = 0x00_08;   // Bytes to follow

        $dxRwGut     = 0x00_00;   // Size of row gutter
        $dxColGut    = 0x00_00;   // Size of col gutter
    ...
    {
        $record      = 0x00_81;   // Record identifier
        $length      = 0x00_02;   // Bytes to follow
        $grbit       = 0x00_00;
        ...
        $grbit |= 0x00_01;                           // Auto page breaks visible
        if ($this->outlineStyle) {
            $grbit |= 0x00_20; // Auto outline styles
        }
        if ($this->phpSheet->getShowSummaryBelow()) {
            $grbit |= 0x00_40; // Outline summary below
        }
        if ($this->phpSheet->getShowSummaryRight()) {
            $grbit |= 0x00_80; // Outline summary right
        }
        if ($this->phpSheet->getPageSetup()->getFitToPage()) {
            $grbit |= 0x01_00; // Page setup fit to page
        }
        if ($this->outlineOn) {
            $grbit |= 0x04_00; // Outline symbols displayed
        }
    ...
    private function writeBreaks()
    {
        ...
        if (!empty($hbreaks)) {
            ...
            $record  = 0x00_1B;               // Record identifier
            ...
            foreach ($hbreaks as $hbreak) {
                $data .= pack('vvv', $hbreak, 0x00_00, 0x00_FF);
            }
        ...
        if (!empty($vbreaks)) {
            ...
            $vbreaks = array_slice($vbreaks, 0, 1_000);
            ...
            $record  = 0x00_1A;               // Record identifier
            ...
            foreach ($vbreaks as $vbreak) {
                $data .= pack('vvv', $vbreak, 0x00_00, 0xFF_FF);
            }
    ...
    private function writeProtect()
    {
        ...
        $record      = 0x00_12;             // Record identifier
        $length      = 0x00_02;             // Bytes to follow
    ...
    private function writeScenProtect()
    {
        ...
        $record = 0x00_DD; // Record identifier
        $length = 0x00_02; // Bytes to follow
    ...
    private function writeObjectProtect()
    {
        ...
        $record = 0x00_63; // Record identifier
        $length = 0x00_02; // Bytes to follow
    ...
    private function writePassword()
    {
        ...
        $record      = 0x00_13;               // Record identifier
        $length      = 0x00_02;               // Bytes to follow
    ...
    public function positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        $x1 = $x1     / PHPExcel_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_start))   * 1_024;
        $y1 = $y1     / PHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $row_start + 1)   *  256;
        $x2 = $width  / PHPExcel_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_end))     * 1_024; // Distance to right side of object
    ...
    {
        $record      = 0x00_5D;   // Record identifier
        $length      = 0x00_3C;   // Bytes to follow

        $cObj        = 0x00_01;   // Count of objects in file (set to 1)
        $OT          = 0x00_08;   // Object type. 8 = Picture
        $id          = 0x00_01;   // Object ID
        $grbit       = 0x06_14;   // Option flags

        $cbMacro     = 0x00_00;   // Length of FMLA structure
        $Reserved1   = 0x00_00;   // Reserved
        $Reserved2   = 0x00_00;   // Reserved
        ...
        $frs         = 0x00_00;   // Frame style
        $cf          = 0x00_09;   // Image format, 9 = bitmap
        $Reserved3   = 0x00_00;   // Reserved
        $cbPictFmla  = 0x00_00;   // Length of FMLA structure
        $Reserved4   = 0x00_00;   // Reserved
        $grbit2      = 0x00_01;   // Option flags
        $Reserved5   = 0x00_00;   // Reserved
    ...
    public function processBitmapGd($image)
    {
        ...
        $data = pack('Vvvvv', 0x00_0C, $width, $height, 0x01, 0x18);
        for ($j = $height; --$j;) {
            for ($i = 0; $i < $width; ++$i) {
    ...
    public function processBitmap($bitmap)
    {
        ...
        $header  = pack('Vvvvv', 0x00_0C, $width, $height, 0x01, 0x18);
        $data    = $header . $data;
    ...
    private function writeZoom()
    {
        ...
        $record      = 0x00_A0;               // Record identifier
        $length      = 0x00_04;               // Bytes to follow
    ...
    private function writeMsoDrawing()
    {
        ...
        if (isset($this->escher)) {
            ...
            for ($i = 1; $i <= $nm; ++$i) {
                // MSODRAWING record
                $record = 0x00_EC;            // Record identifier
                ...
                // ftCmo
                if ($spTypes[$i] == 0x00_C9) {
                    // Add ftCmo (common object data) subobject
                    $objData
                        .= pack(
                            'vvvvvVVV',
                            0x00_15, // 0x0015 = ftCmo
                            0x00_12, // length of ftCmo data
                            0x00_14, // object type, 0x0014 = filter
                            $i, // object id number, Excel seems to use 1-based index, local for the sheet
                            0x21_01, // option flags, 0x2001 is what OpenOffice.org uses
                            0, // reserved
                            0, // reserved
                            0,  // reserved
                        );
                    ...
                    $objData .= pack('vv', 0x0_0C, 0x00_14);
                    $objData .= pack('H*', '0000000000000000640001000A00000010000100');
                    // Add ftLbsData (List box data) subobject
                    $objData .= pack('vv', 0x00_13, 0x1F_EE);
                    ...
                    $objData
                        .= pack(
                            ...
                            0x00_15, // 0x0015 = ftCmo
                            0x00_12, // length of ftCmo data
                            0x00_08, // object type, 0x0008 = picture
                            $i, // object id number, Excel seems to use 1-based index, local for the sheet
                            0x60_11, // option flags, 0x6011 is what OpenOffice.org uses
                            0, // reserved
                            0, // reserved
                            0,  // reserved
                        );
                ...
                $objData
                    .= pack(
                        'vv',
                        0x00_00, // 0x0000 = ftEnd
                        0x00_00,  // length of ftEnd data
                    );
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            $record = 0x01_B2;      // Record identifier
            $length = 0x00_12;      // Bytes to follow

            $grbit  = 0x00_00;       // Prompt box at cell, no cached validity data at DV records
            $horPos = 0x00_00_00_00;  // Horizontal position of prompt box, if fixed position
            $verPos = 0x00_00_00_00;  // Vertical position of prompt box, if fixed position
            $objId  = 0xFF_FF_FF_FF;  // Object identifier of drop down arrow object, or -1 if not visible
            ...
            $record = 0x01_BE;              // Record identifier
    ...
    {
        $record      = 0x08_8B;               // Record identifier
        $length      = 0x00_10;               // Bytes to follow

        $rt         = 0x08_8B; // 2
        $grbitFrt   = 0x00_00; // 2
        $reserved   = 0x00_00_00_00_00_00_00_00; // 8
        ...
        $header      = pack('vv', $record, $length);
        $data      = pack('vvVVvv', $rt, $grbitFrt, 0x00_00_00_00, 0x00_00_00_00, $wScalvePLV, $grbit);
        $this->append($header . $data);
    }
    ...
    {
        $record      = 0x01_B1;               // Record identifier
        ...
        // Alignment
        $flags = 0;
        $flags |= ($bAlignHz == 1 ? 0x00_00_00_01 : 0);
        $flags |= ($bAlignVt == 1 ? 0x00_00_00_02 : 0);
        $flags |= ($bAlignWrapTx == 1 ? 0x00_00_00_04 : 0);
        $flags |= ($bTxRotation == 1 ? 0x00_00_00_08 : 0);
        // Justify last line flag
        $flags |= (1 == 1 ? 0x00_00_00_10 : 0);
        $flags |= ($bIndent == 1 ? 0x00_00_00_20 : 0);
        $flags |= ($bShrinkToFit == 1 ? 0x00_00_00_40 : 0);
        // Default
        $flags |= (1 == 1 ? 0x00_00_00_80 : 0);
        // Protection
        $flags |= ($bProtLocked == 1 ? 0x00_00_01_00 : 0);
        $flags |= ($bProtHidden == 1 ? 0x00_00_02_00 : 0);
        // Border
        $flags |= ($bBorderLeft == 1 ? 0x00_00_04_00 : 0);
        $flags |= ($bBorderRight == 1 ? 0x00_00_08_00 : 0);
        $flags |= ($bBorderTop == 1 ? 0x00_00_10_00 : 0);
        $flags |= ($bBorderBottom == 1 ? 0x00_00_20_00 : 0);
        $flags |= (1 == 1 ? 0x00_00_40_00 : 0); // Top left to Bottom right border
        $flags |= (1 == 1 ? 0x00_00_80_00 : 0); // Bottom left to Top right border
        // Pattern
        $flags |= ($bFillStyle == 1 ? 0x00_01_00_00 : 0);
        $flags |= ($bFillColor == 1 ? 0x00_02_00_00 : 0);
        $flags |= ($bFillColorBg == 1 ? 0x00_04_00_00 : 0);
        $flags |= (1 == 1 ? 0x00_38_00_00 : 0);
        // Font
        $flags |= ($bFormatFont == 1 ? 0x04_00_00_00 : 0);
        // Alignment:
        $flags |= ($bFormatAlign == 1 ? 0x08_00_00_00 : 0);
        // Border
        $flags |= ($bFormatBorder == 1 ? 0x10_00_00_00 : 0);
        // Pattern
        $flags |= ($bFormatFill == 1 ? 0x20_00_00_00 : 0);
        // Protection
        $flags |= ($bFormatProt == 1 ? 0x40_00_00_00 : 0);
        // Text direction
        $flags |= (1 == 0 ? 0x80_00_00_00 : 0);

        // Data Blocks
        if ($bFormatFont == 1) {
            // Font Name
            if ($conditional->getStyle()->getFont()->getName() == null) {
                $dataBlockFont =  pack('VVVVVVVV', 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00);
                $dataBlockFont .= pack('VVVVVVVV', 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00, 0x00_00_00_00);
            } else {
            ...
            if ($conditional->getStyle()->getFont()->getBold() == true) {
                $dataBlockFont .= pack('v', 0x02_BC);
            } else {
                $dataBlockFont .= pack('v', 0x01_90);
            }
            // Escapement type
            ...
            // Not used (3)
            $dataBlockFont .= pack('vC', 0x00_00, 0x00);
            // Font color index
            ...
            // Not used (4)
            $dataBlockFont .= pack('V', 0x00_00_00_00);
            // Options flags for modified font attributes
            $optionsFlags = 0;
            $optionsFlagsBold = ($conditional->getStyle()->getFont()->getBold() == null ? 1 : 0);
            $optionsFlags |= ($optionsFlagsBold == 1 ? 0x00_00_00_02 : 0);
            $optionsFlags |= (1 == 1 ? 0x00_00_00_08 : 0);
            $optionsFlags |= (1 == 1 ? 0x00_00_00_10 : 0);
            $optionsFlags |= (1 == 0 ? 0x00_00_00_20 : 0);
            $optionsFlags |= (1 == 1 ? 0x00_00_00_80 : 0);
            ...
            $dataBlockFont .= pack('V', 0x00_00_00_00);
            // Not used (8)
            $dataBlockFont .= pack('VV', 0x00_00_00_00, 0x00_00_00_00);
            // Always
            $dataBlockFont .= pack('v', 0x00_01);
        }
        if ($bFormatAlign == 1) {
            $blockAlign = 0;
            ...
            $dataBlockAlign = pack('CCvvv', $blockAlign, $blockRotation, $blockIndent, $blockIndentRelative, 0x00_00);
        }
        if ($bFormatBorder == 1) {
            $blockLineStyle = 0;
        ...
        $data      = pack('CCvvVv', $type, $operatorType, $szValue1, $szValue2, $flags, 0x00_00);
    ...
    {
        $record      = 0x01_B0;               // Record identifier
        $length      = 0x00_16;               // Bytes to follow
