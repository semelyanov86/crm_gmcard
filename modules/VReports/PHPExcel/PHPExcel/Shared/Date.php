...
{
    /** constants */
    public const CALENDAR_WINDOWS_1900 = 1_900;        //    Base date of 1st Jan 1900 = 1.0
    public const CALENDAR_MAC_1904 = 1_904;            //    Base date of 2nd Jan 1904 = 1.0
    ...
    public static function ExcelToPHP($dateValue = 0, $adjustToTimezone = false, $timezone = null)
    {
        if (self::$excelBaseDate == self::CALENDAR_WINDOWS_1900) {
            $myexcelBaseDate = 25_569;
            //    Adjust for the spurious 29-Feb-1900 (Day 60)
            if ($dateValue < 60) {
                --$myexcelBaseDate;
            }
        } else {
            $myexcelBaseDate = 24_107;
        }

        // Perform conversion
        if ($dateValue >= 1) {
            $utcDays = $dateValue - $myexcelBaseDate;
            $returnValue = round($utcDays * 86_400);
            if (($returnValue <= PHP_INT_MAX) && ($returnValue >= -PHP_INT_MAX)) {
                $returnValue = (int) $returnValue;
            }
        } else {
            $hours = round($dateValue * 24);
            $mins = round($dateValue * 1_440) - round($hours * 60);
            $secs = round($dateValue * 86_400) - round($hours * 3_600) - round($mins * 60);
    ...
    public static function ExcelToPHPObject($dateValue = 0)
    {
        ...
        $days = floor($dateTime / 86_400);
        $time = round((($dateTime / 86_400) - $days) * 86_400);
        $hours = round($time / 3_600);
        $minutes = round($time / 60) - ($hours * 60);
        $seconds = round($time) - ($hours * 3_600) - ($minutes * 60);

        $dateObj = date_create_immutable('1-Jan-1970+' . $days . ' days');
    ...
    public static function FormattedPHPToExcel($year, $month, $day, $hours = 0, $minutes = 0, $seconds = 0)
    {
        ...
        $excelDate = floor((146_097 * $century) / 4) + floor((1_461 * $decade) / 4) + floor((153 * $month + 2) / 5) + $day + 1_721_119 - $myexcelBaseDate + $excel1900isLeapYear;

        $excelTime = (($hours * 3_600) + ($minutes * 60) + $seconds) / 86_400;
