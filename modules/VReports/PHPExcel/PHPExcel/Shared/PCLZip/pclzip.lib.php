...
if (!defined('PCLZIP_READ_BLOCK_SIZE')) {
    define('PCLZIP_READ_BLOCK_SIZE', 2_048);
}
...
// ----- Options values
define('PCLZIP_OPT_PATH', 77_001);
define('PCLZIP_OPT_ADD_PATH', 77_002);
define('PCLZIP_OPT_REMOVE_PATH', 77_003);
define('PCLZIP_OPT_REMOVE_ALL_PATH', 77_004);
define('PCLZIP_OPT_SET_CHMOD', 77_005);
define('PCLZIP_OPT_EXTRACT_AS_STRING', 77_006);
define('PCLZIP_OPT_NO_COMPRESSION', 77_007);
define('PCLZIP_OPT_BY_NAME', 77_008);
define('PCLZIP_OPT_BY_INDEX', 77_009);
define('PCLZIP_OPT_BY_EREG', 77_010);
define('PCLZIP_OPT_BY_PREG', 77_011);
define('PCLZIP_OPT_COMMENT', 77_012);
define('PCLZIP_OPT_ADD_COMMENT', 77_013);
define('PCLZIP_OPT_PREPEND_COMMENT', 77_014);
define('PCLZIP_OPT_EXTRACT_IN_OUTPUT', 77_015);
define('PCLZIP_OPT_REPLACE_NEWER', 77_016);
define('PCLZIP_OPT_STOP_ON_ERROR', 77_017);
// Having big trouble with crypt. Need to multiply 2 long int
// which is not correctly supported by PHP ...
// define('PCLZIP_OPT_CRYPT', 77018);
define('PCLZIP_OPT_EXTRACT_DIR_RESTRICTION', 77_019);
define('PCLZIP_OPT_TEMP_FILE_THRESHOLD', 77_020);
define('PCLZIP_OPT_ADD_TEMP_FILE_THRESHOLD', 77_020); // alias
define('PCLZIP_OPT_TEMP_FILE_ON', 77_021);
define('PCLZIP_OPT_ADD_TEMP_FILE_ON', 77_021); // alias
define('PCLZIP_OPT_TEMP_FILE_OFF', 77_022);
define('PCLZIP_OPT_ADD_TEMP_FILE_OFF', 77_022); // alias

// ----- File description attributes
define('PCLZIP_ATT_FILE_NAME', 79_001);
define('PCLZIP_ATT_FILE_NEW_SHORT_NAME', 79_002);
define('PCLZIP_ATT_FILE_NEW_FULL_NAME', 79_003);
define('PCLZIP_ATT_FILE_MTIME', 79_004);
define('PCLZIP_ATT_FILE_CONTENT', 79_005);
define('PCLZIP_ATT_FILE_COMMENT', 79_006);

// ----- Call backs values
define('PCLZIP_CB_PRE_EXTRACT', 78_001);
define('PCLZIP_CB_POST_EXTRACT', 78_002);
define('PCLZIP_CB_PRE_ADD', 78_003);
define('PCLZIP_CB_POST_ADD', 78_004);
/* For futur use
...
class PclZip
{
    ...
    public function create($p_filelist)
    {
        ...
        if ($v_size > 1) {
            ...
            // ----- Look for first arg
            if (is_integer($v_arg_list[0]) && ($v_arg_list[0] > 77_000)) {
                // ----- Parse the options
    ...
    public function privOptionDefaultThreshold(&$p_options)
    {
        ...
        if ($last == 'k') {
            $v_memory_limit = $v_memory_limit * 1_024;
        }
    ...
    public function privAddFile($p_filedescr, &$p_header, &$p_options)
    {
        ...
        */
        ...
        if ($p_filedescr['type'] == 'file') {
            $p_header['external'] = 0x00_00_00_00;
            $p_header['size'] = filesize($p_filename);
        } elseif ($p_filedescr['type'] == 'folder') {
            // ----- Look for regular folder
            $p_header['external'] = 0x00_00_00_10;
        ...
        if ($p_header['status'] == 'ok') {
            ...
            if ($p_filedescr['type'] == 'file') {
                ...
                $p_header['external'] = 0x00_00_00_10;     // Value for a folder : to be checked
    ...
    public function privWriteFileHeader(&$p_header)
    {
        ...
        $v_mdate = (($v_date['year'] - 1_980) << 9) + ($v_date['mon'] << 5) + $v_date['mday'];

        // ----- Packed data
        $v_binary_data = pack('VvvvvvVVVvv', 0x04_03_4B_50, $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], strlen($p_header['stored_filename']), $p_header['extra_len']);
    ...
    public function privWriteCentralFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack('VvvvvvvVVVvvvvvVV', 0x02_01_4B_50, $p_header['version'], $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], strlen($p_header['stored_filename']), $p_header['extra_len'], $p_header['comment_len'], $p_header['disk'], $p_header['internal'], $p_header['external'], $p_header['offset']);
    ...
    public function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack('VvvvvVVv', 0x06_05_4B_50, 0, 0, $p_nb_entries, $p_nb_entries, $p_size, $p_offset, strlen($p_comment));
    ...
    public function privConvertHeader2FileInfo($p_header, &$p_info)
    {
        ...
        $p_info['folder'] = (($p_header['external'] & 0x00_00_00_10) == 0x00_00_00_10);
    ...
    public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        if ($p_remove_all_path == true) {
            ...
            if (($p_entry['external'] & 0x00_00_00_10) == 0x00_00_00_10) {
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if ((($p_entry['external'] & 0x00_00_00_10) == 0x00_00_00_10) || (substr($p_entry['filename'], -1) == '/')) {
                ...
                if (($v_result = $this->privDirCheck($v_dir_to_check, ($p_entry['external'] & 0x00_00_00_10) == 0x00_00_00_10)) != 1) {
                    // ----- Change the file status
    ...
    public function privExtractFileUsingTempFile(&$p_entry, &$p_options)
    {
        ...
        $v_binary_data = pack('va1a1Va1a1', 0x8B_1F, chr($p_entry['compression']), chr(0x00), time(), chr(0x00), chr(3));
    ...
    public function privReadFileHeader(&$p_header)
    {
        ...
        if ($p_header['mdate'] && $p_header['mtime']) {
            // ----- Extract time
            $v_hour = ($p_header['mtime'] & 0xF8_00) >> 11;
            $v_minute = ($p_header['mtime'] & 0x07_E0) >> 5;
            $v_seconde = ($p_header['mtime'] & 0x00_1F) * 2;

            // ----- Extract date
            $v_year = (($p_header['mdate'] & 0xFE_00) >> 9) + 1_980;
            $v_month = ($p_header['mdate'] & 0x01_E0) >> 5;
            $v_day = $p_header['mdate'] & 0x00_1F;
    ...
    public function privReadEndCentralDir(&$p_central_dir)
    {
        ...
        if (!$v_found) {
            $v_maximum_size = 65_557; // 0xFFFF + 22;
            ...
            while ($v_pos < $v_size) {
                ...
                $v_bytes = (($v_bytes & 0xFF_FF_FF) << 8) | ord($v_byte);

                // ----- Compare the bytes
                if ($v_bytes == 0x50_4B_05_06) {
                    ++$v_pos;
                    break;
                }
                ...
                for ($j = 0; ($j < sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); ++$j) {
                    ...
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == '/') {
                        ...
                        if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) > strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_found = true;
                        } elseif ((($v_header_list[$v_nb_extracted]['external'] & 0x00_00_00_10) == 0x00_00_00_10) /* Indicates a folder */ && ($v_header_list[$v_nb_extracted]['stored_filename'] . '/' == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_found = true;
                        }
