...
class VReports_ChartActions_Action extends Vtiger_Action_Controller
{
    ...
    public function pinToDashboard(Vtiger_Request $request)
    {
        ...
        if ($numOfRows >= 1) {
            ...
            $query = 'INSERT INTO vtiger_module_vreportdashboard_widgets (userid,reportid,linkid,title,dashboardtabid,sizeWidth,sizeHeight) VALUES (?,?,?,?,?,?,?)';
            $param = [$currentuserid, $reportid, $linkid, $widgetTitle, $dashBoardTabId, $width, $height];
            $result = $db->pquery($query, $param);
    ...
    public function savePosition($request)
    {
        ...
        $db->pquery('UPDATE vtiger_vreport set position = ? where reportid = ?', [$position, $reportid]);
        $response = new Vtiger_Response();
