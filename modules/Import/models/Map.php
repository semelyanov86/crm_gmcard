...
class Import_Map_Model extends Vtiger_Base_Model
{
    ...
    public static function markAsDeleted($mapId)
    {
        ...
        $db->pquery('UPDATE vtiger_import_maps SET deleted=1 WHERE id=?', [$mapId]);
    }
    ...
    public function save()
    {
        ...
        if (php7_count($map) > 0) {
            $db->pquery('INSERT INTO ' . self::$tableName . ' (' . implode(',', $columnNames) . ') VALUES (' . generateQuestionMarks($columnValues) . ')', [$columnValues]);
    ...
    public static function getAllByModule($moduleName)
    {
        ...
        $result = $db->pquery('SELECT * FROM ' . self::$tableName . ' WHERE deleted=0 AND module=?', [$moduleName]);
