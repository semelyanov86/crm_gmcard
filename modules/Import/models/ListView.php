...
class Import_ListView_Model extends Vtiger_ListView_Model
{
    ...
    public function getListViewEntries($pagingModel)
    {
        ...
        if (!empty($searchValue)) {
            $queryGenerator->addUserSearchConditions(['search_field' => $searchKey, 'search_text' => $searchValue, 'operator' => 'c']);
        }
     ...
     */
    public static function getInstance($moduleName, $viewId = '0', $listHeader = [])
    {
        $db = PearDatabase::getInstance();
    ...
    public function getLastImportedRecord()
    {
        ...
        $result = $db->pquery('SELECT recordid FROM ' . $userDBTableName . ' WHERE status NOT IN (?,?) AND recordid IS NOT NULL', [Import_Data_Action::$IMPORT_RECORD_FAILED, Import_Data_Action::$IMPORT_RECORD_SKIPPED]);
