...
class Products_Relation_Model extends Vtiger_Relation_Model
{
    ...
    public function deleteRelation($sourceRecordId, $relatedRecordId)
    {
        ...
        } elseif ($sourceModuleName == $relatedModuleName && $this->get('source') != 'custom') {
    ...
    public function deleteProductToProductRelation($sourceRecordId, $relatedRecordId)
    {
        ...
        if (!empty($sourceRecordId) && !empty($relatedRecordId)) {
            $db->pquery('DELETE FROM vtiger_seproductsrel WHERE crmid = ? AND productid = ?', [$relatedRecordId, $sourceRecordId]);
    ...
    public function isDeletable()
    {
        $relatedModuleModel = $this->getRelationModuleModel();
        $relatedModuleName = $relatedModuleModel->get('name');
        $inventoryModulesList = ['Invoice', 'Quotes', 'PurchaseOrder', 'SalesOrder'];
    ...
    public function isSubProduct($subProductId)
    {
        if (!empty($subProductId)) {
            $db = PearDatabase::getInstance();
            $result = $db->pquery('SELECT crmid FROM vtiger_seproductsrel WHERE crmid = ?', [$subProductId]);
