...
class CustomerPortal_Utils
{
    public static function getImageDetails($recordId, $module)
    {
        ...
        $result = $adb->pquery($sql, [$module . ' Image', $recordId]);
    ...
    public static function isModuleActive($module)
    {
        ...
        $defaultAllowedModules = ['ModComments', 'History', 'Contacts', 'Accounts'];

        if (in_array($module, $defaultAllowedModules)) {
            return true;
        }
        if (in_array($module, $activeModules) && !Vtiger_Runtime::isRestricted('modules', $module)) {
            return true;
        }
    ...
    {
        $userTypeFields = ['assigned_user_id', 'creator', 'userid', 'created_user_id', 'modifiedby', 'folderid'];
        ...
        if (!empty($fieldnamesToResolve)) {
            foreach ($fieldnamesToResolve as $resolveFieldname) {
                ...
                if (isset($record[$resolveFieldname]) && !empty($record[$resolveFieldname])) {
                    ...
                    $record[$resolveFieldname] = ['value' => $fieldvalueid, 'label' => $fieldvalue];
                }
            }
        }
    ...
    public static function isViewable($fieldName, $module)
    {
        ...
        $tabidResult = $db->pquery($tabidSql, [$module]);
        ...
        $presenceResult = $db->pquery($presenceSql, [$fieldName, $tabId]);
    ...
    public static function isModuleRecordCreatable($module)
    {
        ...
        $createPermissionResult = $db->pquery($recordPermissionQuery, [getTabid($module)]);
    ...
    public static function isModuleRecordEditable($module)
    {
        ...
        $editPermissionResult = $db->pquery($recordPermissionQuery, [getTabid($module)]);
