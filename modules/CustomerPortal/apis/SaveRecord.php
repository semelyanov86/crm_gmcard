            ...
            if (in_array($module, ['HelpDesk', 'Documents', 'Assets', 'Quotes', 'Contacts', 'Accounts'])) {
                ...
                if (!empty($recordId)) {
                    ...
                    if (!CustomerPortal_Utils::isModuleRecordEditable($module)) {
                        throw new Exception('Module record cannot be edited', 1_412);
                        exit;
                    }
                } else {
                    if (!CustomerPortal_Utils::isModuleRecordCreatable($module)) {
                        throw new Exception('Module record cannot be created', 1_412);
                        exit;
                    }
                }
                    ...
                    if (!empty($values)) {
                        foreach ($values as $key => $value) {
                            if (!in_array($key, $editableFields)) {
                                throw new Exception('Specified fields not editable', 1_412);
                                exit;
                            }
                        }
                    }
                ...
                $response->setError(1_404, 'save operation not supported for this module');
            }
