...
class CustomerPortal_DownloadFile extends CustomerPortal_API_Abstract
{
    public function process(CustomerPortal_API_Request $request)
    {
        ...
        if ($current_user) {
            ...
            if (!CustomerPortal_Utils::isModuleActive($module)) {
                $response->setError(1_404, 'Module is disabled.');
            ...
            if (!empty($parentId)) {
                if ($parentModule === 'Faq') {
                    if (!$this->isFaqPublished($parentId)) {
                        throw new Exception('This Faq is not published', 1_412);
                        exit;
                    }
                } else {
                    if (!$this->isRecordAccessible($parentId)) {
                        throw new Exception('Parent record not Accessible', 1_412);
                        exit;
                    }
                    ...
                    if (!in_array($recordId, $relatedRecordIds)) {
                        throw new Exception('Record not Accessible', 1_412);
                        exit;
                    }
                }
            } else {
                if (!$this->isRecordAccessible($recordId, $module) && $module != 'ModComments') {
                    $response->setError(1_404, 'Record not accessible');
            ...
            if ($module == 'Documents') {
                ...
                $fileres = $adb->pquery($filepathQuery, [$fileid]);
                ...
                throw new Exception('Download not supported.', 1_412);
                exit;
            }
    ...
    public function updateDownloadCount($id)
    {
        ...
        $countres = $adb->pquery($updateDownloadCount, [$id]);
        $log->debug('Entering customer portal function updateDownloadCount');
