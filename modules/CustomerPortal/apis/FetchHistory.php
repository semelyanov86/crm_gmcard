    ...
    public function process(CustomerPortal_API_Request $request)
    {
        ...
        if ($current_user) {
            ...
            if (!CustomerPortal_Utils::isModuleActive($module)) {
                throw new Exception('History not available for this module', 1_412);
                exit;
            }

            if (!ModTracker::isTrackingEnabledForModule($module)) {
                throw new Exception('Module not tracked for changes.', 1_412);
                exit;
            }
            ...
            if (empty($relatedIds)) {
                throw new Exception('No records found', 1_412);
                exit;
            }
            $recordIds = [];
            ...
            $response->setResult(['history' => $historyItems]);
        } else {
            $response->setError(1_404, 'No permission to perform this operation.');
        }
