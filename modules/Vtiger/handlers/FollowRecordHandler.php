...
class FollowRecordHandler extends VTEventHandler
{
    public function handleEvent($eventName, $entityData)
    {
        if ($eventName == 'vtiger.entity.aftersave') {
            ...
            $restrictedModules = ['CustomerPortal', 'Dashboard', 'Emails', 'EmailTemplates', 'ExtensionStore', 'Google', 'Home',
            ...
            if (!in_array($moduleName, $restrictedModules)) {
                ...
                // following users
                $userIdsList = [];
                $result = $db->pquery("SELECT userid FROM {$tableName} WHERE recordid = ? AND starred = ? AND userid != ?", [$recordId, '1', $currentUserId]);
    ...
    public function getChangedFieldString($fieldModels, $changedValues, $userRecordModel)
    {
        ...
        foreach ($fieldModels as $fieldName => $fieldModel) {
            ...
            } elseif ($fieldModel->get('uitype') == 117 && $fieldCurrentValue) {
                $fieldDisplayValue = getCurrencyName($fieldCurrentValue, false);
            } else {
                $fieldDataType = $fieldModel->getFieldDataType();
                switch ($fieldDataType) {
                    case 'boolean':
                    case 'multipicklist':	$fieldDisplayValue = $fieldModel->getDisplayValue($fieldCurrentValue);
                        break;
                    case 'date':	$fieldDisplayValue = DateTimeField::convertToUserFormat($fieldCurrentValue, $userEntity);
                        break;
                    case 'double':	$fieldDisplayValue = CurrencyField::convertToUserFormat(decimalFormat($fieldCurrentValue), $userEntity, true);
                        break;
                    ...
                    default:	$fieldDisplayValue = $fieldModel->getEditViewDisplayValue($fieldCurrentValue);
                        break;
                }
            }
