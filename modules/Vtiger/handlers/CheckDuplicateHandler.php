        ...
        if (!$moduleModel->allowDuplicates && !$skipDuplicateCheck) {
            ...
            foreach ($fields as $fieldName => $fieldModel) {
                if ($fieldModel->isUniqueField() && $fieldModel->isEditable()) {
                    ...
                    if (in_array($moduleName, ['Events', 'Calendar']) && in_array($fieldName, ['date_start', 'due_date'])) {
            ...
            if (php7_count($uniqueFields) > 0) {
                ...
                if ($checkDuplicates) {
                    ...
                    foreach ($uniqueFields as $fieldName => $fieldModel) {
                        ...
                        if (in_array($moduleName, ['Events', 'Calendar']) && in_array($fieldName, ['date_start', 'due_date', 'time_start', 'time_end'])) {
                            ...
                            $params[] = trim(implode(' ', [$uniqueFieldsData[$dateFieldColumnName], $uniqueFieldsData[$timeFieldColumnName]]));
