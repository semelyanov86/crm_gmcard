...
class Vtiger_ComposeEmail_View extends Vtiger_Footer_View
{
    ...
    public function emailPreview($request)
    {
        ...
        if (empty($parentId)) {
            [$parentRecord, $status] = explode('@', reset(array_filter(explode('|', $recordModel->get('parent_id')))));
        ...
        foreach ($parentIds as $index => $parentFieldId) {
            ...
            foreach ($emailFields as $emailField) {
                ...
                if (in_array($emailValue, $TO)) {
                    ...
                    $toMailNamesList[$parentId][] = ['label' => decode_html($parentRecordModel->getName()), 'value' => decode_html($emailValue)];
                }
            }
            if ($emailOptOutFlag) {
            ...
            foreach ($referenceFields as $fieldName) {
                ...
                $refModuleEmailFields = array_keys($refRecordModel->getModule()->getFieldsByType(['email']));
                foreach ($refModuleEmailFields as $fieldName) {
                    $emailValue = $refRecordModel->get($fieldName);
                    if (in_array($emailValue, $TO)) {
                        $toMailInfo[$parentId][] = $emailValue;
                        $toMailNamesList[$parentId][] = ['label' => decode_html($parentRecordModel->getName()) . ':' . vtranslate('SINGLE_' . $refRecordModel->getModuleName(), $refRecordModel->getModuleName()), 'value' => decode_html($emailValue)];
                    }
                }
            }
        }
    ...
    public function emailReply($request)
    {
        $viewer = $this->getViewer($request);
        $this->emailActionsData($request);
    ...
    public function emailReplyAll($request)
    {
        $viewer = $this->getViewer($request);
        $this->emailActionsData($request);
        ...
        $emailFlagsList = ['MailManager', 'MAILSCANNER'];
