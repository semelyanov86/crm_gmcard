...
class Vtiger_Relation_Model extends Vtiger_Base_Model
{
    ...
    public const RELATION_INDIRECT = 2;
    ...
    {
        $db = PearDatabase::getInstance();
        $moduleRelations = Vtiger_Cache::get('moduleRelations', [$parentModuleModel->getName(), $selected, $onlyActive]);
        if ($moduleRelations) {
            return $moduleRelations;
        }

        $skipReltionsList = ['get_history'];
        $query = 'SELECT vtiger_relatedlists.*,vtiger_tab.name as modulename FROM vtiger_relatedlists 
        ...
        $result = $db->pquery($query, [$parentModuleModel->getId(), $skipReltionsList]);
        ...
        Vtiger_Cache::set('moduleRelations', [$parentModuleModel->getName(), $selected, $onlyActive], $relationModels);
    ...
    public static function getInstanceFromRelationFied($relationFieldId)
    {
        ...
        $result = $db->pquery($query, [$relationFieldId]);
        $relationModel = false;
        if ($db->num_rows($result) > 0) {
            $row = $db->query_result_rowdata($result, 0);
            $parentModuleId = $row['tabid'];
    ...
    public function getRelationField()
    {
        ...
        if (!$relationField) {
            ...
            $result = $db->pquery($relationFieldSql, [$this->getId()]);
            if ($db->num_rows($result) > 0) {
