...
class Vtiger_Module_Model extends Vtiger_Module
{
    ...
    public function getQuickCreateFields()
    {
        ...
        foreach ($blocksList as $blockName => $blockModel) {
            ...
            foreach ($fieldList as $fieldName => $fieldModel) {
                ...
                } elseif ($fieldModel->isQuickCreateEnabled() && $fieldModel->isEditable()) {
                    $quickCreateFieldList[$fieldName] = $fieldModel;
                }
            }
        }
    ...
    public function getNameFields()
    {
        ...
        if ($nameFieldObject && $nameFieldObject->fieldname) {
            ...
            switch ($moduleName) {
                case 'HelpDesk': $this->nameFields = ['ticket_title'];
                    $fieldNames = 'ticket_title';
                    break;
                case 'Documents': $this->nameFields = ['notes_title'];
    ...
    {
        $db = PearDatabase::getInstance();
        $query = $db->convert2Sql('SELECT ' . implode(',', $searchFields) . ' FROM vtiger_crmentity WHERE label LIKE ? AND vtiger_crmentity.deleted = 0', ["%{$searchValue}%"]);
    ...
    public function getRelatedModuleRecordIds(Vtiger_Request $request, $recordIds = [], $nonAdminCheck = false)
    {
        ...
        $focus = CRMEntity::getInstance($this->getName());
        $focus->related_module_table_index ??= null;
     ...
     * @return bool - true if module is related
     */
