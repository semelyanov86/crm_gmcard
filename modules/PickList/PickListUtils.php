...
function getUserFldArray($fld_module, $roleid)
{
    ...
    $result = $adb->pquery($query, [$tabid, $tabid]);
 ...
 */
function getAllPickListValues($fieldName, $lang = [])
{
 ...
 */
function getEditablePicklistValues($fieldName, $lang = [], $adb = null)
{
    if ($adb == null) {
 ...
 */
function getNonEditablePicklistValues($fieldName, $lang = [], $adb = null)
{
    if ($adb == null) {
        $adb = PearDatabase::getInstance();
    }
...
function getAssignedPicklistValues($tableName, $roleid, $adb, $lang = [])
{
    ...
    $result = $adb->pquery($sql, [$tableName]);
