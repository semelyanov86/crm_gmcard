...
class EMAILMaker_SendEmails_View extends Vtiger_Index_View
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if (true) {
                ...
                while ($row = $adb->fetchByAssoc($result4)) {
                    ...
                    if ($row["esentid"] == "") {
                        ...
                        $adb->pquery($sql6, array(
                            $row["from_name"], $row["from_email"], $row["subject"], $row["body"], $row["type"], "",
                            $row["pdf_template_ids"], $row["pdf_language"], $row["userid"], $row["attachments"],
                            $row["att_documents"], "0", "0", "0", "0", "", $row["pmodule"]
                        ));
                    ...
                    while ($row7 = $adb->fetchByAssoc($result7)) {
                        $result12 = $adb->pquery("SELECT emailid FROM vtiger_emakertemplates_emails WHERE esentid = ? AND pid = ?",
                            array($row["esentid"], $row7["crmid"]));
                        ...
                        if (!$num_rows12) {
                            ...
                            if ($row7["emailoptout"] == "1") {
                                ...
                                $adb->pquery($sql13, array(
                                    $row["esentid"], $row7["crmid"], "massemail|".$to_email, "", "", "", "", "1",
                                    "unsubscribes"
                                ));
                                $adb->pquery("UPDATE vtiger_emakertemplates_me SET unsubscribes = unsubscribes + 1 WHERE meid = ?",
                                    array($row["meid"]));
                            } else {
                                if ($to_email != "") {
                                    ...
                                    $adb->pquery($sql8, array(
                                        $row["esentid"], $row7["crmid"], "massemail|".$to_email, $to_email, "", "", "",
                                        "", "0", "", ""
                                    ));
                                    $adb->pquery("UPDATE vtiger_emakertemplates_sent SET total_emails = total_emails + 1 WHERE esentid = ?",
                                        array($row["esentid"]));
                                }
                            }
                        }
                    }
                    $adb->pquery("UPDATE vtiger_emakertemplates_me SET status = 'in progress', total_entries = ? WHERE meid = ?",
                        array($total_entries, $row["meid"]));
                }
            }
            ...
            $num_rows10 = $adb->num_rows($result10);
            if ($num_rows10 > 0) {
                $result17 = $adb->pquery("select from_email_field from vtiger_systems where server_type=?",
                    array('email'));
            ...
            $result12 = $adb->pquery("SELECT meid, esentid FROM vtiger_emakertemplates_me WHERE status = ?",
                array('in progress'));
            $num_rows12 = $adb->num_rows($result12);
            if ($num_rows12 > 0) {
            ...
            while ($row2 = $adb->fetchByAssoc($result2)) {
                ...
                if ($is_updated == "1") {
                    ...
                    if ($mycrmid == "email") {
                        ...
                        if ($temp == "-1") {
                            if ($email_address == "") {
                                $email_address = $adb->query_result($adb->pquery("select email1 from vtiger_users where id=?",
                                    array($mycrmid)), 0, 'email1');
                            }
                    ...
                    $EMAILContentModel = EMAILMaker_EMAILContent_Model::getInstance($formodule, $pid, $language,
                        $mycrmid, $rmodule);
                    ...
                    if ($parent_id == "" || $parent_id == "0") {
                        ...
                        $parent_id = $focus->id;
                        $adb->pquery("UPDATE vtiger_emakertemplates_emails SET parent_id = ? WHERE emailid = ?",
                            array($parent_id, $semailid));
                    }
                    if ($attachments == "1") {
                        $result_attch = $adb->pquery("SELECT * FROM vtiger_emakertemplates_attch WHERE esentid = ?",
                            array($esentid));
                        $num_attch = $adb->num_rows($result_attch);
                        if ($num_attch > 0) {
                            while ($row_attch = $adb->fetchByAssoc($result_attch)) {
                                $this->SaveAttachmentIntoEmail($current_user, $parent_id, $row_attch["filename"],
                                    $row_attch["type"], $row_attch["file_desc"]);
                            }
                        }
                    }
                    if ($att_documents != "") {
                    ...
                    if ($temp == "-1") {
                        $rmodule = 'Users';
                        $mailer = $this->send_mail($email_address, $from_name, $from_email, $from_email_field, $subject,
                            $body, $cc, $bcc, $parent_id, $logo, $Email_Images, $phpmailer_version);
                        $mail_status = $mailer->Send(true);
                        $mail_status_str .= $email_address."=".$mail_status."&&&";
                    } else {
                        $emailid = $parent_id;
                        if ($mycrmid == "") {
                            $mailer = $this->send_mail($email_address, $from_name, $from_email, $from_email_field,
                                $subject, $body, $cc, $bcc, $parent_id, $logo, $Email_Images, $phpmailer_version);
                            $mail_status = $mailer->Send(true);
                            $mail_status_str .= $email_address."=".$mail_status."&&&";
                            if ($mail_status != 1) {
                                $errorheader2 = 1;
                            ...
                            if ($email_address != '') {
                                ...
                                if (isPermitted($rmodule, 'DetailView', $mycrmid) == 'yes') {
                                    $mailer = $this->send_mail($email_address, $from_name, $from_email,
                                        $from_email_field, $subject, $body, $cc, $bcc, $parent_id, $logo, $Email_Images,
                                        $phpmailer_version);
                                    $mail_status = $mailer->Send(true);
                                } else {
                    ...
                    $adb->pquery("INSERT INTO vtiger_emakertemplates_contents (activityid,emailid,content) VALUES (?,?,?)",
                        array($parent_id, $semailid, $preview_body));
                    ...
                    $adb->pquery("UPDATE vtiger_crmentity SET description = ? WHERE crmid = ?",
                        array($new_body, $parent_id));
                    ...
                    if ($mail_status) {
                        $adb->pquery("UPDATE vtiger_emaildetails SET email_flag = 'SENT' WHERE emailid = ?",
                            array($parent_id));
                        $adb->pquery("UPDATE vtiger_emakertemplates_sent SET total_sent_emails = total_sent_emails + 1 WHERE esentid = ?",
                            array($esentid));
                    }
    ...
    public function getMELimitForSQL($ED)
    {
        ...
        $esentid = $ED["esentid"];
        if ($limit > 0) {
            $sql = "SELECT count(emailid) as total_emails FROM vtiger_emakertemplates_emails WHERE esentid = ? AND status = 1 AND deleted = '0' AND email_send_date > date_add(now(),interval -45 minute) GROUP BY esentid";
    ...
    public function SaveAttachmentIntoEmail($current_user, $id, $file_name, $filetype, $filetmp_name)
    {
        ...
        if (copy($filetmp_name, $upload_file_path.$current_id."_".$file_name)) {
            ...
            $params1 = array(
                $current_id, $current_user->id, $ownerid, "Email Attachment", "", $adb->formatDate($date_var, true),
