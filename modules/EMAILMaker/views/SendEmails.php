...
class EMAILMaker_SendEmails_View extends Vtiger_Index_View
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if (true) {
            if ($num_rows1 > 0) {
                $adb->pquery('UPDATE vtiger_emakertemplates_delay SET delay_active = ?', ['1']);
            } else {
                $adb->pquery('INSERT INTO vtiger_emakertemplates_delay (delay_active) VALUES  (?)', []);
            }
            $default_language = vglobal('default_language');
            $default_theme = vglobal('default_theme');
            $from_name = $from_email = $cc = $bcc = '';
            $result_s = $adb->pquery('SELECT * FROM vtiger_emakertemplates_settings', []);
            ...
            if ($num_rows4 > 0) {
                while ($row = $adb->fetchByAssoc($result4)) {
                    ...
                    $result10 = $adb->pquery($sql10, [$templateid]);
                    ...
                    if ($row['esentid'] == '') {
                        ...
                        $adb->pquery($sql10, [$row['esentid'], $row['meid']]);
                    }
                    unset($current_user);
                    ...
                    [$sql7a, $sql7b] = explode(' FROM ', $query7);
                    ...
                    $sql7 .= ' FROM ' . $sql7b;
                    $result7 = $adb->pquery($sql7, []);
                    $total_entries = $adb->num_rows($result7);
                    $unsubscribes = 0;

                    while ($row7 = $adb->fetchByAssoc($result7)) {
                        $result12 = $adb->pquery(
                            'SELECT emailid FROM vtiger_emakertemplates_emails WHERE esentid = ? AND pid = ?',
                            [$row['esentid'], $row7['crmid']],
                        );
                        ...
                        if (!$num_rows12) {
                            ...
                            if ($row7['emailoptout'] == '1') {
                                ...
                                $adb->pquery(
                                    'UPDATE vtiger_emakertemplates_me SET unsubscribes = unsubscribes + 1 WHERE meid = ?',
                                    [$row['meid']],
                                );
                            } else {
                                if ($to_email != '') {
                                    ...
                                    $adb->pquery(
                                        'UPDATE vtiger_emakertemplates_sent SET total_emails = total_emails + 1 WHERE esentid = ?',
                                        [$row['esentid']],
                                    );
                                }
                            }
                    ...
                    $adb->pquery(
                        "UPDATE vtiger_emakertemplates_me SET status = 'in progress', total_entries = ? WHERE meid = ?",
                        [$total_entries, $row['meid']],
                    );
                }
            }
            ...
            $result12 = $adb->pquery(
                'SELECT meid, esentid FROM vtiger_emakertemplates_me WHERE status = ?',
                ['in progress'],
            );
            $num_rows12 = $adb->num_rows($result12);
            if ($num_rows12 > 0) {
    ...
    public function sendEMAILMakerEmails($from_email_field, $current_user, $ED, $language)
    {
        ...
        $result2 = $adb->pquery($sql2, [$esentid]);
            ...
            while ($row2 = $adb->fetchByAssoc($result2)) {
                ...
                if ($is_updated == '1') {
                    ...
                    if ($mycrmid == 'email') {
                        ...
                        if ($temp == '-1') {
                            if ($email_address == '') {
                                $email_address = $adb->query_result($adb->pquery(
                    ...
                    if ($parent_id == '' || $parent_id == '0') {
                        ...
                        if ($cc_ids != '') {
                            ...
                            foreach ($CC_IDs as $email_crm_id) {
                                if (!in_array($email_crm_id, $Inserted_Emails)) {
                                    ...
                                    $rel_params_2 = [$email_crm_id, $focus->id];
                        ...
                        if ($bcc_ids != '') {
                            ...
                            foreach ($BCC_IDs as $email_crm_id) {
                                if (!in_array($email_crm_id, $Inserted_Emails)) {
                                    ...
                                    $rel_params_3 = [$email_crm_id, $focus->id];
                        ...
                        $parent_id = $focus->id;
                        $adb->pquery(
                            'UPDATE vtiger_emakertemplates_emails SET parent_id = ? WHERE emailid = ?',
                            [$parent_id, $semailid],
                        );
                    }
                    if ($attachments == '1') {
                        $result_attch = $adb->pquery(
                            'SELECT * FROM vtiger_emakertemplates_attch WHERE esentid = ?',
                            [$esentid],
                        );
                        $num_attch = $adb->num_rows($result_attch);
                        if ($num_attch > 0) {
                            while ($row_attch = $adb->fetchByAssoc($result_attch)) {
                                $this->SaveAttachmentIntoEmail(
                                    $current_user,
                                    $parent_id,
                                    $row_attch['filename'],
                                    $row_attch['type'],
                    ...
                    $adb->pquery(
                        'INSERT INTO vtiger_emakertemplates_contents (activityid,emailid,content) VALUES (?,?,?)',
                        [$parent_id, $semailid, $preview_body],
                    );
                    ...
                    $adb->pquery(
                        'UPDATE vtiger_crmentity SET description = ? WHERE crmid = ?',
                        [$new_body, $parent_id],
                    );
                    ...
                    $adb->pquery($sql_u3, [$semailid]);
                    if ($mail_status) {
                        $adb->pquery(
                            "UPDATE vtiger_emaildetails SET email_flag = 'SENT' WHERE emailid = ?",
                            [$parent_id],
                        );
                        $adb->pquery(
                            'UPDATE vtiger_emakertemplates_sent SET total_sent_emails = total_sent_emails + 1 WHERE esentid = ?',
                            [$esentid],
                        );
                    }
    ...
    public function SaveAttachmentIntoEmail($current_user, $id, $file_name, $filetype, $filetmp_name)
    {
        ...
        if (copy($filetmp_name, $upload_file_path . $current_id . '_' . $file_name)) {
            ...
            if (EMAILMaker_EMAILMaker_Model::isStoredName()) {
                ...
                $params2 = [$current_id, $filename, $filename, $filetype, $upload_file_path];
            } else {
                $sql2 = 'insert into vtiger_attachments(attachmentsid, name, type, path) values(?, ?, ?, ?)';
                $params2 = [$current_id, $filename, $filetype, $upload_file_path];
            }
    ...
    public function saveDocumentsIntoEmail($id, $documents)
    {
        ...
        foreach ($Documents as $document_id) {
            if ($document_id != '') {
                ...
                $adb->pquery($sql1, [$id, $document_id]);
            }
        }
    }
    ...
    public function controlIfIsSent($emailid)
    {
        ...
        $result_email_d = $adb->pquery($sql_email_d, [$emailid]);
