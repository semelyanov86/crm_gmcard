...
class EMAILMaker_Detail_View extends Vtiger_Index_View
{
    ...
    public function showModuleDetailView(Vtiger_Request $request)
    {
        ...
        if ($request->has('record') && !$request->isEmpty('record')) {
            ...
            if (vtlib_isModuleActive("ITS4YouStyles")) {
                ...
                $email_body = $ITS4YouStylesModuleModel->addStyles($email_body, $emailtemplateResult["templateid"],
                    "EMAILMaker");
            }
    ...
    public function preProcess(Vtiger_Request $request, $display = true)
    {
        ...
        if ($request->has('record') && !$request->isEmpty('record')) {
            ...
            if (vtlib_isModuleActive("ITS4YouStyles")) {
                ...
                $Styles_List = $ITS4YouStylesModuleModel->getRelatedRecords($emailtemplateResult["templateid"],
                    "EMAILMaker", "asc", true);
            ...
            if ($userPrivilegesModel->hasModuleActionPermission($documentsInstance->getId(), 'DetailView')) {
                ...
                $Documents_Header = array(
                    "title" => 'Title', "folder" => 'Folder Name', "assigned_to" => 'Assigned To', "name" => 'File Name'
                );
    ...
    public function getHeaderScripts(Vtiger_Request $request)
    {
        ...
        $moduleName = $request->getModule();
        $jsFileNames = array(
            "layouts.v7.modules.Vtiger.resources.List", "layouts.v7.modules.Vtiger.resources.Detail",
            "layouts.v7.modules.EMAILMaker.resources.Detail", "layouts.v7.modules.Vtiger.resources.RelatedList"
        );
        if (vtlib_isModuleActive("ITS4YouStyles")) {
