...
class EMAILMaker_MassSaveAjax_View extends Vtiger_Footer_View
{
    ...
    public function massSave(Vtiger_Request $request)
    {
        ...
        $sendEmails = $sendMail = $EmailCCBCC = [];
        $moduleName = $request->getModule();
        ...
        if (!empty($emailValue)) {
            if ($type === 'a') {
                ...
                $result_u = $adb->pquery(sprintf('SELECT first_name, last_name, %s AS email  FROM vtiger_users WHERE id = ?', $type), [$emailValue]);
        ...
        $existingAttachments = $request->get('attachments', []);
        if (empty($recordId)) {
        ...
        if (is_array($existingAttachments)) {
            foreach ($existingAttachments as $index => $existingAttachInfo) {
                ...
                $description = '';
                $params1 = [$current_id, $current_user_id, $ownerId, $moduleName . ' Attachment', $description, $adb->formatDate($date_var, true), $adb->formatDate($date_var, true)];
                ...
                if (EMAILMaker_EMAILMaker_Model::isStoredName()) {
                    $params2 = [$current_id, $filename, $filename, $description, $filetype, $upload_file_path];

                    $adb->pquery('insert into vtiger_attachments(attachmentsid, name, storedname, description, type, path) values(?, ?, ?, ?, ?, ?)', $params2);
                } else {
                    $params2 = [$current_id, $filename, $description, $filetype, $upload_file_path];
    ...
    public function getRecordsListFromRequest(Vtiger_Request $request)
    {
        ...
        if ($selectedIds == 'all') {
            ...
            foreach ($request->getAll() as $requestKey => $requestValue) {
                if (strpos($requestKey, 'emailinfo') !== false) {
    ...
    public function uploadAndSaveFile($file_details)
    {
        ...
        if ($save_file == 'true' && $upload_status == 'true') {
            ...
            $module = 'Email';
            $params1 = [$current_id, $current_user->id, $ownerid, $module . ' Attachment', '', $adb->formatDate($date_var, true), $adb->formatDate($date_var, true)];
            $adb->pquery('insert into vtiger_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?)', $params1);

            if (EMAILMaker_EMAILMaker_Model::isStoredName()) {
                $adb->pquery('insert into vtiger_attachments(attachmentsid, name, storedname, description, type, path) values(?, ?, ?, ?, ?, ?)', [$current_id, $filename, $filename, '', $filetype, $upload_file_path]);
            } else {
                $adb->pquery('insert into vtiger_attachments(attachmentsid, name, description, type, path) values(?, ?, ?, ?, ?)', [$current_id, $filename, '', $filetype, $upload_file_path]);
            }
    ...
    public function saveEmails($Send_Emails, $SendMail, $language, $pdf_template_ids, $pdflanguage, $ids_for_pdf)
    {
        ...
        if (count($Send_Emails) > 0) {
            foreach ($Send_Emails as $SED) {
                ...
                $adb->pquery($sql, [$this->from_name, $this->from_email, $SED['subject'], $SED['description'], '1', $ids_for_pdf, $pdf_template_ids, $pdflanguage, $current_user_id, $SED['att_documents'], '', '', '', '0', $related_to, $pmodule, $language]);
                ...
                foreach ((array) $SendMail['to'] as $sid => $PidsEmails) {
                    ...
                    foreach ($PidsEmails as $to_email_id => $to_email_address) {
                        ...
                        [$pid, $to_email, $fmodule] = explode('|', $to_email_id);
                        ...
                        $adb->pquery($sql2, [$esentid, $sid, $to_email_id, $to_email, $cc, $bcc, $cc_ids, $bcc_ids, '0', $focus->id, $email_send_date]);

                        ++$total_emails;

                        unset($focus, $Inserted_Emails);

                    }
                }
                $sql3 = 'UPDATE vtiger_emakertemplates_sent SET total_emails = ? WHERE esentid = ?';
                $adb->pquery($sql3, [$total_emails, $esentid]);
            }
        }
