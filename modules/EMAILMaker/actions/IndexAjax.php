...
class EMAILMaker_IndexAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function getFormattedFileSize($filesize)
    {
        if ($filesize < 1_024) {
            ...
            if ($filesize > 1_024 && $filesize < 1_048_576) {
                $filesize = sprintf('%0.2f', round($filesize / 1_024, 2)) . 'KB';
            } else {
                if ($filesize > 1_048_576) {
                    $filesize = sprintf('%0.2f', round($filesize / (1_024 * 1_024), 2)) . 'MB';
                }
            }
        }
    ...
    public function SaveEMAILImages(Vtiger_Request $request)
    {
        ...
        foreach ($R_Data as $key => $value) {
            if (strpos($key, 'img_') !== false) {
                ...
                $adb->pquery('INSERT INTO vtiger_emakertemplates_images (crmid, productid, sequence, attachmentid, width, height) VALUES (?, ?, ?, ?, ?, ?)', [$crmid, $productid, $sequence, $value, $width, $height]);
            }
        }
    }
    ...
    public function getModuleFields(Vtiger_Request $request)
    {
        ...
        $response = new Vtiger_Response();
        $response->setResult(['success' => true, 'fields' => $SelectModuleFields, 'related_modules' => $RelatedModules, 'subject_fields' => [vtranslate('LBL_COMMON_EMAILINFO', 'EMAILMaker') => $subject_fields]]);
        $response->emit();
    }
    ...
    public function sendExampleMEmail(Vtiger_Request $request)
    {
        ...
        $response = new Vtiger_Response();
        $response->setResult(['success' => $success, 'status' => $mail_status]);
        $response->emit();
    }
