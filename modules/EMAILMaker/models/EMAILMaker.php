    ...
    public function AddLinks($modulename)
    {
        ...
        if (empty($modulename)) {
            ...
            if ($focus && method_exists($focus, 'get_emails')) {
                ...
                $function = 'get_emails';
                $result = $this->db->pquery('SELECT name FROM vtiger_relatedlists WHERE tabid=? AND related_tabid=? AND name=? AND label=?', Array($link_module->getId(), $relatedModuleInstance->id, $function, $label));
