...
function getVisibleCriteria($recordid = '')
{
    ...
    $selcriteria = '';
    if ($recordid != '') {
        $result = $adb->pquery('select sharingtype from vtiger_report where reportid=?', [$recordid]);
        $selcriteria = $adb->query_result($result, 0, 'sharingtype');
    }
    if ($selcriteria == '') {
        $selcriteria = 'Public';
    }
    $filter_result = $adb->pquery('select * from vtiger_reportfilters', []);
...
{
    global $adb;
    $member_query = $adb->pquery("SELECT vtiger_reportsharing.setype,vtiger_users.id,vtiger_users.user_name FROM vtiger_reportsharing INNER JOIN vtiger_users on vtiger_users.id = vtiger_reportsharing.shareid WHERE vtiger_reportsharing.setype='users' AND vtiger_reportsharing.reportid = ?", [$recordid]);
    ...
    $member_query = $adb->pquery("SELECT vtiger_reportsharing.setype,vtiger_groups.groupid,vtiger_groups.groupname FROM vtiger_reportsharing INNER JOIN vtiger_groups on vtiger_groups.groupid = vtiger_reportsharing.shareid WHERE vtiger_reportsharing.setype='groups' AND vtiger_reportsharing.reportid = ?", [$recordid]);
    $noofrows = $adb->num_rows($member_query);
    if ($noofrows > 0) {
        for ($i = 0; $i < $noofrows; ++$i) {
