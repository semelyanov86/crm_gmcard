...
class Reports_Folder_Model extends Vtiger_Base_Model
{
    ...
    public function save()
    {
        ...
        if (!empty($folderId)) {
            $db->pquery(
                'UPDATE vtiger_reportfolder SET foldername = ?, description = ? WHERE folderid = ?',
                [$this->getName(), $this->getDescription(), $folderId],
            );
        } else {
     ...
     */
    public function getRecordIds($skipRecords = false, $module = null, $searchParams = [])
    {
        $db = PearDatabase::getInstance();
