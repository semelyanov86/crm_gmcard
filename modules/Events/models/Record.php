...
class Events_Record_Model extends Calendar_Record_Model
{
    ...
    public function getRelatedContactInfo()
    {
        ...
        foreach ($contactIdList as $contactId) {
            $relatedContactInfo[] = ['name' => decode_html(Vtiger_Util_Helper::toSafeHTML(Vtiger_Util_Helper::getRecordName($contactId))), 'id' => $contactId];
        }
    ...
    public function getRelatedContactInfoFromIds($eventIds)
    {
        ...
        $result = $adb->pquery($query, [$eventIds]);
        $num_rows = $adb->num_rows($result);

        $contactInfo = [];
        for ($i = 0; $i < $num_rows; ++$i) {
            $row = $adb->fetchByAssoc($result, $i);
            $contactInfo[$row['id']][] = ['name' => Vtiger_Util_Helper::toSafeHTML(Vtiger_Util_Helper::getRecordName($row['contactid'])),
    ...
    public function updateInvitationStatus($activityId, $userId, $status)
    {
        ...
        $adb->pquery($sql, [$status, $activityId, $userId]);
    ...
    public function getInviteUserMailData()
    {
        ...
        $cont_res = $adb->pquery($cont_qry, [$return_id]);
