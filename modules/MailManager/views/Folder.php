...
class MailManager_Folder_View extends MailManager_Abstract_View
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        } elseif ($this->getOperationArg($request) == 'drafts') {
            $moduleName = $request->getModule();
            $q = $request->get('q');
            $type = $request->get('type');
        ...
        } elseif ($this->getOperationArg($request) == 'getFoldersList') {
            $viewer = $this->getViewer($request);
    ...
    {
        $options = ['SUBJECT' => 'SUBJECT', 'TO' => 'TO', 'BODY' => 'BODY', 'BCC' => 'BCC', 'CC' => 'CC', 'FROM' => 'FROM', 'DATE' => 'ON'];
