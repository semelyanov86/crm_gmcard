...
class MailManager_Mail_View extends MailManager_Abstract_View
{
    public function process(Vtiger_Request $request)
    {
        ...
        if ($this->getOperationArg($request) == 'open') {
        ...
        } elseif ($this->getOperationArg($request) == 'mark' && $request->validateWriteAccess()) {
            ...
            $response->isJson(true);
            $response->setResult(['folder' => $foldername, 'unread' => $folder->unreadCount() + 1,
                'status' => true, 'msgno' => $request->get('_msgno')]);

        } elseif ($this->getOperationArg($request) == 'delete' && $request->validateWriteAccess()) {
        ...
        } elseif ($this->getOperationArg($request) == 'move' && $request->validateWriteAccess()) {
        ...
        } elseif ($this->getOperationArg($request) == 'send' && $request->validateWriteAccess()) {
            ...
            if (!empty($to_string)) {
                ...
                foreach ($toArray as $to) {
                    ...
                    $referenceArray = ['Contacts', 'Accounts', 'Leads'];
            ...
            if ($status === true) {
                $response->isJson(true);
                $response->setResult(['sent' => true]);
            } else {
                $response->isJson(true);
                $response->setError(112, 'please verify outgoing server.');
            }

        } elseif ($this->getOperationArg($request) == 'attachment_dld') {
            ...
            flush();
            exit;
        } elseif ($this->getOperationArg($request) == 'getdraftmail' && $request->validateWriteAccess()) {

            $connector = $this->getConnector('__vt_drafts');
            $draftMail = $connector->getDraftMail($request);
            $response->isJson(true);
            $response->setResult([$draftMail]);

        } elseif ($this->getOperationArg($request) == 'save' && $request->validateWriteAccess()) {
            ...
            $response->isJson(true);
            if (!empty($draftId)) {
                $response->setResult(['success' => true, 'emailid' => $draftId]);
            } else {
                $response->setResult(['success' => false, 'error' => 'Draft was not saved']);
            }

        } elseif ($this->getOperationArg($request) == 'deleteAttachment' && $request->validateWriteAccess()) {

            $connector = $this->getConnector('__vt_drafts');
            $deleteResponse = $connector->deleteAttachment($request);

            $response->isJson(true);
            $response->setResult(['success' => $deleteResponse]);

        } elseif ($this->getOperationArg($request) == 'forward' && $request->validateWriteAccess()) {
            ...
            if (!empty($attachments)) {
                foreach ($attachments as $aValue) {
                    ...
                    if (is_array($attachInfo) && !empty($attachInfo) && $attachInfo['size'] > 0) {
                        ...
                        $attachmentInfo[] = ['name' => $attachInfo['name'], 'size' => $attachInfo['size'], 'emailid' => $draftId, 'docid' => $document->id];
                    }
                    unset($aValue['data']);
                }
            }
            $response->isJson(true);
            $response->setResult(['attachments' => $attachmentInfo, 'emailid' => $draftId]);
        }
