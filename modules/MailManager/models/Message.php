...
class MailManager_Message_Model extends Vtiger_MailRecord
{
     ...
     * @param int $msgno - Mail Message Number
     * @param bool $fetchbody - Used to save the mail information to DB
     */
     ...
     * @param int $messageid - Mail Number
     * @param object $p
     * @param int $partno
     */
    ...
    public function __getpart($imap, $messageid, $p, $partno)
    {
        ...
        if (((isset($params['filename']) && $params['filename']) || (isset($params['name']) && $params['name'])) && strtolower($p->disposition) == 'attachment') {
            ...
            $this->_attachments[] = ['filename' => @self::__mime_decode($filename), 'data' => $data]; // For Fixing issue when two files have same name
            ...
            $this->_inline_attachments[] = ['cid' => $id, 'filename' => @self::__mime_decode($filename), 'data' => $data];
        } elseif (((isset($params['filename']) && $params['filename']) || (isset($params['name']) && $params['name'])) && $p->bytes > 0) {
            $filename = ($params['filename']) ? $params['filename'] : $params['name'];
            $this->_attachments[] = ['filename' => @self::__mime_decode($filename), 'data' => $data];
        }
        // TEXT
    ...
    public static function pruneOlderInDB($waybacktime)
    {
		...
		WHERE userid=? AND lastsavedtime < ?', [$currentUserModel->getId(), $waybacktime]);
     ...
     * @global Users Instance $currentUserModel
     * @global PearDataBase Instance $db
     * @param int $waybacktime - timestamp
     */
    ...
    public function readFromDB($uid, $folder = false)
    {
        ...
        $params = [$currentUserModel->getId(), $uid];
        if ($folder) {
            $query .= ' AND mfolder = ?';
     ...
     * @param bool $withContent - Used to load the Attachments with/withoud content
     * @param string $aName - Attachment Name
     * @param int $aId - Attachment Id (to eliminate friction with same Attachment Name)
     */
    protected function loadAttachmentsFromDB($withContent, $aName = false, $aId = false)
    {
        ...
        if (empty($this->_attachments)) {
            ...
            $params = [$currentUserModel->getId(), $this->muid()];
            ...
            if ($db->num_rows($atResult)) {
                for ($atIndex = 0; $atIndex < $db->num_rows($atResult); ++$atIndex) {
                    ...
                    $this->_attachments[] = ['filename' => $atResultRow['aname'], 'data' => $data, 'size' => $fileSize, 'path' => $filePath, 'attachid' => $atResultRow['attachid']];
     ...
     * @global PearDataBase Instance $db
     * @global Users Instance $currentUserModel
     * @param int $uid - Mail Unique Number
     * @return bool
     */
            ...
            foreach ($this->_attachments as $index => $aValue) {
                ...
                if (is_array($attachInfo) && !empty($attachInfo)) {
					...
					(userid, muid, attachid, aname, path, lastsavedtime) VALUES (?, ?, ?, ?, ?, ?)',
                        [$currentUserModel->getId(), $uid, $attachInfo['attachid'], $attachInfo['name'], $attachInfo['path'], $savedtime],
                    );

                    unset($this->_attachments[$index]);					// This is needed first when we save attachment with invalid file extension,
                    $this->_attachments[] = ['filename' => $attachInfo['name'], 'data' => $aValue['data']]; // so the file name has to renamed.
                }
                unset($aValue['data']);
            }
        }
        ...
        if (is_array($this->_inline_attachments)) {
            foreach ($this->_inline_attachments as $index => $info) {
				...
				(userid, muid, attachid, aname, path, lastsavedtime, cid) VALUES (?, ?, ?, ?, ?, ?, ?)',
                        [$currentUserModel->getId(), $uid, $attachInfo['attachid'], @self::__mime_decode($attachInfo['name']), $attachInfo['path'], $savedtime, $info['cid']],
                    );
                    if ($this->_attachments === false) {
                        $this->_attachments = [];
                    }
                    $this->_attachments[] = ['filename' => @self::__mime_decode($info['filename']), 'data' => $info['data']]; // so the file name has to renamed.
                }
                unset($aValue['data']);
            }
        }
    ...
    public function __SaveAttachmentFile($filename, $filecontent)
    {
        ...
        $db->pquery(
            'INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
				...
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [$attachid, $currentUserModel->getId(), $currentUserModel->getId(), $currentUserModel->getId(), 'MailManager Attachment', $binFile, $usetime, $usetime, 1, 0],
        );

        $db->pquery(
            'INSERT INTO vtiger_attachments SET attachmentsid=?, name=?, description=?, type=?, path=?',
            [$attachid, $binFile, $binFile, $mimetype, $dirname],
        );

        $attachInfo = ['attachid' => $attachid, 'path' => $dirname, 'name' => $binFile, 'type' => $mimetype, 'size' => filesize($saveasfile)];
    ...
    public function convertFileSize($size)
    {
        $type = 'Bytes';
        if ($size > 1_048_575) {
            $size = round($size / (1_024 * 1_024), 2);
            $type = 'MB';
        } elseif ($size > 1_023) {
            $size = round($size / 1_024, 2);
            $type = 'KB';
        }
