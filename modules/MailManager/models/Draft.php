...
class MailManager_Draft_Model
{
    ...
    public function saveEmailParentRel($emailId, $parentIds)
    {
        ...
        if (!empty($emailId)) {
            $db->pquery('delete from vtiger_seactivityrel where activityid=?', [$emailId]); // remove all previous relation
        }
        for ($i = 0; $i < php7_count($myids); ++$i) {
            ...
            if (!empty($realid[0]) && !empty($emailId)) {
                ...
                $result = $db->pquery('SELECT * FROM vtiger_seactivityrel WHERE crmid=? AND activityid=?', [$realid[0], $emailId]);
                if (!$db->num_rows($result)) {
                    $db->pquery('INSERT INTO vtiger_seactivityrel(crmid, activityid) VALUES(?,?)', [$realid[0], $emailId]);
                }
            }
        }
    }
