...
{
    public const settings_table_name = 'vtiger_google_sync_settings';
    public const fieldmapping_table_name = 'vtiger_google_sync_fieldmapping';
    ...
    public static function updateSyncTime($sourceModule, $modifiedTime = false, $user = false)
    {
        ...
        if (!self::getSyncTime($sourceModule, $user)) {
            if ($modifiedTime) {
                $db->pquery('INSERT INTO vtiger_google_sync (googlemodule,user,synctime,lastsynctime) VALUES (?,?,?,?)', [$sourceModule, $user->id, $modifiedTime, date('Y-m-d H:i:s')]);
            }
        } else {
            $db->pquery('UPDATE vtiger_google_sync SET synctime = ?,lastsynctime = ? WHERE user=? AND googlemodule=?', [$modifiedTime, date('Y-m-d H:i:s'), $user->id, $sourceModule]);
        }
    }
     ...
     */
    public static function getCallbackUrl($request, $options = [])
    {
        global $site_URL;
    ...
    public static function hasSettingsForUser($userId, $source_module)
    {
        $db = PearDatabase::getInstance();
    ...
    public static function getSyncDirectionForUser($user = false, $module = 'Contacts')
    {
        if (!$user) {
            $user = Users_Record_Model::getCurrentUserModel();
        }
    ...
    public static function toGoogleXml($string)
    {
