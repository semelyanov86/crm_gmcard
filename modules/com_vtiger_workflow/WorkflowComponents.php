...
function vtJsonFields($adb, $request)
{
    ...
    echo Zend_Json::encode(['moduleFields' => $fields]);
}
...
function vtJsonDependentModules($adb, $request)
{
    ...
    $filterModules = ['Invoice', 'Quotes', 'SalesOrder', 'PurchaseOrder', 'Emails', 'Calendar', 'Events', 'Accounts'];
    $skipFieldsList = [];
    for ($i = 0; $i < $noOfFields; ++$i) {
        ...
        if ($referenceModule == $moduleName && $tabModuleName != $moduleName) {
            ...
            $dependentFields[$tabModuleName] = ['fieldname' => $fieldName, 'modulelabel' => getTranslatedString($tabModuleName, $tabModuleName)];
        } else {
    ...
    $returnValue = ['count' => php7_count($dependentFields), 'entities' => $dependentFields];
...
function vtJsonOwnersList($adb)
{
    ...
    foreach ($activeUsersList as $userId => $userName) {
        $ownersList[] = ['label' => $userName, 'value' => getUserName($userId)];
    }
    foreach ($allGroupsList as $groupId => $groupName) {
        $ownersList[] = ['label' => $groupName, 'value' => $groupName];
    }
