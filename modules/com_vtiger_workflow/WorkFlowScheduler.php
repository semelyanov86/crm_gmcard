...
class WorkFlowScheduler
{
                        ...
                        foreach ($tasks as $task) {
                            if ($task->active) {
                                ...
                                if ($taskClassName == 'VTEmailTask') {
                                    if ($data['emailoptout'] == 1) {
                                        continue;
                                    }
                                }
                                $trigger = $task->trigger;
                                if ($trigger != null) {
                                    $delay = strtotime($data[$trigger['field']]) + $trigger['days'] * 86_400;
                                }
    ...
    public function addWorkflowConditionsToQueryGenerator($queryGenerator, $conditions)
    {
        ...
        if ($noOfConditions > 0) {
            ...
            foreach ($conditions as $index => $condition) {
                ...
                $specialDateComparator = ['is today', 'is tomorrow', 'is yesterday'];
     ...
     * @return <String>
     */
    public function _parseValueForDate($condition, $fieldModel = false)
    {
        $value = $condition['value'];
        $operation = $condition['operation'];
