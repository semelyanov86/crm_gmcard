...
class Documents_List_View extends Vtiger_List_View
{
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        if ($request->get('mode') == 'removeAlphabetSearch') {
            Vtiger_ListView_Model::deleteParamsSession($listViewSessionKey, ['search_key', 'search_value', 'operator']);
            $searchKey = '';
            $searchValue = '';
            $operator = '';
        }
        ...
        if (empty($orderBy) && empty($searchValue) && empty($pageNumber)) {
            ...
            $params = ['page' => $pageNumber, 'orderby' => $orderBy, 'sortorder' => $sortOrder, 'search_key' => $searchKey,
