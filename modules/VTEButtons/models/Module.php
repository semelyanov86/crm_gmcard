...
class VTEButtons_Module_Model extends Vtiger_Module_Model
{
    ...
    public function checkIsMembers($condition, $recordid)
    {
        ...
        if (is_array($advanceFilter) && count($advanceFilter)) {
            ...
            if (count($groups)) {
                ...
                $res = $adb->pquery($query, array($currentUser->get("id")));
                ...
                $check_role_group = 0;
                if (0 < $adb->num_rows($res_role_group)) {
    ...
    public function getFieldLabel($moduleName, $fieldName)
    {
        ...
        $results = $adb->pquery("SELECT fieldlabel FROM `vtiger_field` where tabid=? AND fieldname=?", array($tabId, $fieldName));
        if (0 < $adb->num_rows($results)) {
