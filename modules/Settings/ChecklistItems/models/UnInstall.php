...
class UnInstall
{
    ...
    public function removeDataFromDB($queries)
    {
        ...
        global $adb;
        $adb->pquery('DELETE FROM vtiger_settings_field WHERE `name` = ?', ['VTE Check List']);
        $adb->pquery("DELETE FROM `vtiger_links` WHERE linklabel = 'Checklists' AND linkurl LIKE '%javascript:Vtiger_ChecklistItems_Js.showChecklistItems%';", []);
        $adb->pquery("DELETE FROM `vtiger_links` WHERE linklabel = 'ChecklistItemsJS' AND linkurl = 'layouts/v7/modules/ChecklistItems/resources/ChecklistItems.js';", []);
