...
class Settings_UserLogin_Settings_Model extends Vtiger_Base_Model
{
    ...
    public function deleteRecord($request)
    {
        ...
        $this->db->pquery("DELETE FROM vte_user_login WHERE vte_user_login.id IN(" . implode(",", $recordIds) . ")", array());
        return true;
    }
        ...
        if (isset($imageInfo["name"]) && $imageInfo["name"] != "" && $imageInfo["error"] == 0) {
            ...
            if (isset($allowed_image_types[$file_type]) && $allowed_image_types[$file_type] == $file_ext) {
                ...
                if (move_uploaded_file($file_tmp, $newFile) === true) {
                    list($imagewidth, $imageheight, $imageType) = getimagesize($newFile);
                    if ($method == "crop") {
    ...
    public function Generate($request)
    {
        ...
        $pattern = array("\$USER_LOGIN_CUSTOM_LOGO\$", "\$USER_LOGIN_CUSTOM_SLIDE_TYPE\$", "\$USER_LOGIN_CUSTOM_SLIDE_SPEED\$", "\$USER_LOGIN_CUSTOM_SLIDE_EASING\$", "\$USER_LOGIN_CUSTOM_SLIDE_IMAGES\$", "\$USER_LOGIN_CUSTOM_HEADER\$", "\$USER_LOGIN_CUSTOM_DESCRIPTION\$", "\$USER_LOGIN_CUSTOM_SOCIAL_ICONS\$", "\$USER_LOGIN_CUSTOM_COPYRIGHT\$");
        $replace = array($recordDetail["logo"], $recordDetail["slide_type"], $recordDetail["slide_speed"], $recordDetail["slide_easing"], $slide_html, $recordDetail["header"], $recordDetail["description"], $social_icons, $recordDetail["copyright"]);
    ...
    public function SaveImageSettings($data)
    {
        ...
        $logo_result = $this->db->pquery("SELECT * FROM vte_user_login_image_setting WHERE `type`=?", array("logo"));
        $logo_params = array($logo_method, $logo_width, $logo_height, "logo");
        ...
        $slide_result = $this->db->pquery("SELECT * FROM vte_user_login_image_setting WHERE `type`=?", array("slide"));
        $slide_params = array($slide_method, $slide_width, $slide_height, "slide");
