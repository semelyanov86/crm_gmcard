...
class Settings_LayoutEditor_Index_View extends Settings_Vtiger_Index_View
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        switch ($mode) {
            case 'showRelatedListLayout':	$selectedTab = 'relatedListTab';
                break;
            case 'showDuplicationHandling':	$selectedTab = 'duplicationTab';
                break;
    ...
    public function showFieldLayout(Vtiger_Request $request)
    {
        ...
        foreach ($blockModels as $blockLabel => $blockModel) {
            ...
            $fieldModelList = $blockIdFieldMap[$id] ?? null;
        ...
        foreach ($blocks as $blockId => $blockModel) {
            ...
            foreach ($blockFields as $key => $fieldModel) {
                if ($fieldModel->isEditable()
                    ...
                    && !in_array($fieldModel->get('uitype'), [28, 30, 53, 56, 69, 83])
                    && !in_array($fieldModel->getFieldDataType(), ['text', 'multireference'])) {
