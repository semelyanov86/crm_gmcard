...
class Vtiger_MailScannerAction
{
    ...
    public function update($ruleid, $actiontext)
    {
        ...
        $checkmapping = $adb->pquery('SELECT COUNT(*) AS ruleaction_count FROM vtiger_mailscanner_ruleactions
			WHERE ruleid=? AND actionid=?', [$ruleid, $this->actionid]);
     ...
     */
    public function __CreateContact($mailscanner, $mailrecord, $mailscannerrule)
    {
     ...
     * Create Lead action.
     */
    public function __CreateLead($mailscanner, $mailrecord, $mailscannerrule)
    {
     ...
     */
    public function __CreateAccount($mailscanner, $mailrecord, $mailscannerrule)
    {
     ...
     * Create ticket action.
     */
    public function __CreateTicket($mailscanner, $mailrecord, $mailscannerrule)
    {
     ...
     */
    public function linkMail($mailscanner, $mailrecord, $relatedTo)
    {
        $fromemail = $mailrecord->_from[0];
    ...
    public function __SaveAttachements($mailrecord, $basemodule, $basefocus)
    {
        ...
        foreach ($mailrecord->_attachments as $filename => $filecontent) {
            ...
            $adb->pquery(
				...
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                [$attachid, $userid, $userid, $userid, $setype, $description, $usetime, $usetime, 1, 0],
            );
    ...
    public function setDefaultValue($module, $moduleObj)
    {
