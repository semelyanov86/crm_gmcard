...
class Settings_Roles_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function getChildren()
    {
        ...
        if (!$this->children) {
            ...
            $params = [$parentRoleString . '::%', $currentRoleDepth + 1];
    ...
    public function getSameLevelRoles()
    {
        ...
        if (!$this->children) {
            ...
            $sql = 'SELECT * FROM vtiger_role WHERE parentrole LIKE ? AND depth = ?';
            $params = [$parentRoleString . '::%', $currentRoleDepth];
            $result = $db->pquery($sql, $params);
            $noOfRoles = $db->num_rows($result);
            $roles = [];
            for ($i = 0; $i < $noOfRoles; ++$i) {
    ...
    public function save()
    {
        ...
        if ($mode == 'edit') {
            ...
            $params = [$this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'), $roleId];
            $db->pquery($sql, $params);
        } else {
            $sql = 'INSERT INTO vtiger_role(roleid, rolename, parentrole, depth, allowassignedrecordsto) VALUES (?,?,?,?,?)';
            $params = [$roleId, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto')];
            $db->pquery($sql, $params);
            ...
            $db->pquery($picklist2RoleSQL, [$parentRole->getId()]);
        }
        ...
        if (!empty($profileIds)) {
            ...
            if ($noOfProfiles > 0) {
                $db->pquery('DELETE FROM vtiger_role2profile WHERE roleid=?', [$roleId]);

                $sql = 'INSERT INTO vtiger_role2profile(roleid, profileid) VALUES (?,?)';
                for ($i = 0; $i < $noOfProfiles; ++$i) {
                    $params = [$roleId, $profileIds[$i]];
                    $db->pquery($sql, $params);
                }
            }
        }
    ...
    public function delete($transferToRole)
    {
        ...
        $usersResult = $db->pquery('SELECT userid FROM vtiger_user2role WHERE roleid = ?', [$roleId]);
        ...
        $db->pquery('UPDATE vtiger_user2role SET roleid=? WHERE roleid=?', [$transferRoleId, $roleId]);

        $db->pquery('DELETE FROM vtiger_role2profile WHERE roleid=?', [$roleId]);
        $db->pquery('DELETE FROM vtiger_group2role WHERE roleid=?', [$roleId]);
        $db->pquery('DELETE FROM vtiger_group2rs WHERE roleandsubid=?', [$roleId]);

        // delete handling for sharing rules
        deleteRoleRelatedSharingRules($roleId);

        $db->pquery('DELETE FROM vtiger_role WHERE roleid=?', [$roleId]);
    ...
    */
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT userid FROM vtiger_user2role WHERE roleid = ?', [$this->getId()]);
