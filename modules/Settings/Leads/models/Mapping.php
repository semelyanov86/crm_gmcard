...
class Settings_Leads_Mapping_Model extends Settings_Vtiger_Module_Model
{
    ...
    {
        return ['Leads' => 'Leads', 'Type' => 'Type', 'Accounts' => 'Accounts', 'Contacts' => 'Contacts', 'Potentails' => 'Potentials'];
    }
        ...
        if (!property_exists($this, 'mapping') || !$this->mapping) {
            ...
            foreach ($mapping as $mappingId => $mappingDetails) {
                $finalMapping[$mappingId] = [
                    ...
                    'Contacts'	=> $fieldLabelsList[$mappingDetails['contactfid']] ?? null,
                    'Potentials' => $fieldLabelsList[$mappingDetails['potentialfid']] ?? null,
                ];
            }
    ...
    public function save($mapping)
    {
        ...
        foreach ($mapping as $mappingDetails) {
            $mappingId = $mappingDetails['mappingId'] ?? null;
        ...
        if ($updateMappingsList) {
            ...
            $paramArray = $leadParamArray = $accountParamArray = $contactParamArray = $potentialParamArray = [];
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT * FROM vtiger_convertleadmapping WHERE editable = ?', [0]);
        $numOfRows = $db->num_rows($result);
    ...
    {
        return ['Accounts', 'Contacts', 'Potentials'];
    }
