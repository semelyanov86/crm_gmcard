...
class Settings_Groups_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function save()
    {
        ...
        if ($mode == 'edit') {
            ...
            $params = [$this->getName(), $this->getDescription(), $groupId];
        } else {
            $sql = 'INSERT INTO vtiger_groups(groupid, groupname, description) VALUES (?,?,?)';
            $params = [$groupId, $this->getName(), $this->getDescription()];
        }
        $db->pquery($sql, $params);

        $members = $this->get('group_members');
        if (is_array($members)) {
            $db->pquery('DELETE FROM vtiger_users2group WHERE groupid=?', [$groupId]);
            $db->pquery('DELETE FROM vtiger_group2grouprel WHERE groupid=?', [$groupId]);
            $db->pquery('DELETE FROM vtiger_group2role WHERE groupid=?', [$groupId]);
            $db->pquery('DELETE FROM vtiger_group2rs WHERE groupid=?', [$groupId]);
            ...
            for ($i = 0; $i < $noOfMembers; ++$i) {
                ...
                if ($idComponents && php7_count($idComponents) == 2) {
                    ...
                    if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
                        $db->pquery('INSERT INTO vtiger_users2group(userid, groupid) VALUES (?,?)', [$memberId, $groupId]);
                    }
                    if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
                        $db->pquery('INSERT INTO vtiger_group2grouprel(containsgroupid, groupid) VALUES (?,?)', [$memberId, $groupId]);
                    }
                    if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
                        $db->pquery('INSERT INTO vtiger_group2role(roleid, groupid) VALUES (?,?)', [$memberId, $groupId]);
                    }
                    if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
                        $db->pquery('INSERT INTO vtiger_group2rs(roleandsubid, groupid) VALUES (?,?)', [$memberId, $groupId]);
                    }
                }
            }
        }
    ...
    protected function transferOwnership($transferToGroup)
    {
        ...
        $params = [$transferGroupId, $groupId];
        ...
        if (Vtiger_Utils::CheckTable('vtiger_customerportal_prefs')) {
            ...
            $params = [$transferGroupId, 'defaultassignee', $groupId];
            $db->pquery($query, $params);

            $query = 'UPDATE vtiger_customerportal_prefs SET prefvalue = ? WHERE prefkey = ? AND prefvalue = ?';
            $params = [$transferGroupId, 'userid', $groupId];
            $db->pquery($query, $params);
        }
    ...
    public function delete($transferToGroup)
    {
        ...
        $db->pquery('DELETE FROM vtiger_group2rs WHERE groupid=?', [$groupId]);
        $db->pquery('DELETE FROM vtiger_users2group WHERE groupid=?', [$groupId]);
        $db->pquery("DELETE FROM vtiger_reportsharing WHERE shareid=? AND setype='groups'", [$groupId]);

        $db->pquery('DELETE FROM vtiger_groups WHERE groupid=?', [$groupId]);
    }
