...
{
    public const RULE_TYPE_GROUPS = 'GRP';
    public const RULE_TYPE_ROLE = 'ROLE';
    public const RULE_TYPE_ROLE_AND_SUBORDINATES = 'RS';
    public const READ_ONLY_PERMISSION = 0;
    public const READ_WRITE_PERMISSION = 1;
    ...
    public function save()
    {
        ...
        if (!$ruleId) {
            ...
            $db->pquery("DELETE FROM {$tableName} WHERE shareid=?", [$ruleId]);
        }
        ...
        $params = [$ruleId, $sourceIdComponents[1], $targetIdComponents[1], $permission];
        $db->pquery($sql, $params);

        $sql = 'UPDATE vtiger_datashare_module_rel SET relationtype=? WHERE shareid=?';
        $params = [$this->get('relationtype'), $ruleId];
        $db->pquery($sql, $params);
    ...
    public function delete()
    {
        ...
        $db->pquery('DELETE FROM vtiger_datashare_module_rel WHERE shareid=?', [$ruleId]);
        Settings_SharingAccess_Module_Model::recalculateSharingRules();
    }
    ...
    public static function getInstance($moduleModel, $ruleId)
    {
        ...
        $params = [$moduleModel->getId(), $ruleId];
        $result = $db->pquery($sql, $params);
