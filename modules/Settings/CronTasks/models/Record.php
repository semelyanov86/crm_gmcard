...
class Settings_CronTasks_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function getDisplayValue($fieldName)
    {
        ...
        switch ($fieldName) {
            ...
            case 'status': $fieldValue = intval($fieldValue);
                ...
                } elseif ($fieldValue === Settings_CronTasks_Record_Model::$STATUS_RUNNING) {
                    $fieldLabel = 'LBL_RUNNING';
                } elseif ($fieldValue === Settings_CronTasks_Record_Model::$STATUS_ENABLED) {
                    $fieldLabel = 'LBL_ACTIVE';
                } elseif ($fieldValue === Vtiger_Cron::$STATUS_ERROR) {
    ...
    public function save()
    {
        ...
        $params = [$this->get('frequency'), $this->get('status'), $this->getId()];
    ...
    public static function getInstanceById($recordId, $qualifiedModuleName)
    {
        ...
        $result = $db->pquery('SELECT * FROM vtiger_cron_task WHERE id = ?', [$recordId]);
    ...
    public static function getInstanceByName($name)
    {
        ...
        $result = $db->pquery('SELECT * FROM vtiger_cron_task WHERE name = ?', [$name]);
        if ($db->num_rows($result)) {
