...
class Settings_Workflows_Record_Model extends Settings_Vtiger_Record_Model
{
    ...
    public function getV7executionConditionAsLabel($executionCondition = null, $module_name = null)
    {
        if ($executionCondition == null) {
            $executionCondition = $this->get('execution_condition');
        }
    ...
    public function transformToAdvancedFilterCondition()
    {
        ...
        if (!empty($conditions)) {
            foreach ($conditions as $index => $info) {
                ...
                if (!$info['groupid']) {
                    $firstGroup[] = ['columnname' => $columnName, 'comparator' => $info['operation'], 'value' => $value,
                        'column_condition' => $info['joincondition'], 'valuetype' => $info['valuetype'], 'groupid' => $info['groupid']];
                } else {
                    $secondGroup[] = ['columnname' => $columnName, 'comparator' => $info['operation'], 'value' => $value,
                        'column_condition' => $info['joincondition'], 'valuetype' => $info['valuetype'], 'groupid' => $info['groupid']];
                }
            }
        }
