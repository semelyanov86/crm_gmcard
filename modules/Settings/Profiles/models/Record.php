...
{
    public const PROFILE_FIELD_INACTIVE = 0;
    public const PROFILE_FIELD_READONLY = 1;
    public const PROFILE_FIELD_READWRITE = 2;
    ...
    public function delete($transferToRecord)
    {
        ...
        $db->pquery('DELETE FROM vtiger_profile2globalpermissions WHERE profileid=?', [$profileId]);
        $db->pquery('DELETE FROM vtiger_profile2tab WHERE profileid=?', [$profileId]);
        $db->pquery('DELETE FROM vtiger_profile2standardpermissions WHERE profileid=?', [$profileId]);
        $db->pquery('DELETE FROM vtiger_profile2utility WHERE profileid=?', [$profileId]);
        $db->pquery('DELETE FROM vtiger_profile2field WHERE profileid=?', [$profileId]);
        ...
        for ($i = 0; $i < $noOfRoles; ++$i) {
            ...
            if ($profileCount > 1) {
                ...
                $params = [$roleId, $profileId];
            } else {
                $sql = 'UPDATE vtiger_role2profile SET profileid=? WHERE roleid=? AND profileid=?';
                $params = [$transferProfileId, $roleId, $profileId];
            }
            $db->pquery($sql, $params);
        }

        $db->pquery('DELETE FROM vtiger_profile WHERE profileid=?', [$profileId]);
    }
    ...
    public function save()
    {
        ...
        if (!$profileId) {
            ...
            $params = [$profileId, $profileName, $description, $isProfileDirectlyRelatedToRole];
            $isNewProfile = true;
        } else {
            $sql = 'UPDATE vtiger_profile SET profilename=?, description=?, directly_related_to_role=? WHERE profileid=?';
            $params = [$profileName, $description, $isProfileDirectlyRelatedToRole, $profileId];
        ...
        $params = [$profileId, Settings_Profiles_Module_Model::GLOBAL_ACTION_VIEW, $this->tranformInputPermissionValue($this->get('viewall'))];
        $db->pquery($sql, $params);

        $sql = 'INSERT INTO vtiger_profile2globalpermissions(profileid, globalactionid, globalactionpermission) VALUES (?,?,?)';
        $params = [$profileId, Settings_Profiles_Module_Model::GLOBAL_ACTION_EDIT, $this->tranformInputPermissionValue($this->get('editall'))];
        $db->pquery($sql, $params);
    ...
    protected function saveModulePermissions($moduleModel, $permissions)
    {
        ...
        $db->pquery('DELETE FROM vtiger_profile2tab WHERE profileid=? AND tabid=?', [$profileId, $tabId]);
            ...
            if ($actionPermissions || $moduleModel->isUtilityActionEnabled()) {
                ...
                foreach ($availableActionIds as $actionId) {
                    if ($actionId === 0) {
                        ...
                        $actionPermissions[$actionId] ??= null;
                    }
                }
        ...
        $params = [$profileId, $tabId, $isModulePermitted];
        $db->pquery($sql, $params);
        ...
        if (is_array($fieldPermissions)) {
            foreach ($fieldPermissions as $fieldId => $stateValue) {
                $db->pquery(
                    'DELETE FROM vtiger_profile2field WHERE profileid=? AND tabid=? AND fieldid=?',
                    [$profileId, $tabId, $fieldId],
                );
                ...
                $params = [$profileId, $tabId, $fieldId, $visible, $readOnly];
                $db->pquery($sql, $params);
            }
        }
    }
    ...
    public function saveUserAccessbleFieldsIntoProfile2Field()
    {
        ...
        if (!empty($profileId)) {
            ...
            $userAccessbleFields = [];
            $skipFields = [115, 116, 31, 32];
            $allowedFields = ['roleid', 'currency_id', 'reports_to_id'];
            foreach ($moduleFields as $fieldName => $fieldModel) {
            ...
            foreach ($userAccessbleFields as $fieldId => $fieldName) {
                ...
                $db->pquery($insertQuery, [$profileId, $tabId, $fieldId, Settings_Profiles_Module_Model::FIELD_ACTIVE, Settings_Profiles_Module_Model::FIELD_READWRITE]);
            }

            $sql = 'SELECT fieldid FROM vtiger_def_org_field WHERE tabid = ?';
            $result1 = $db->pquery($sql, [$tabId]);
            $def_org_fields = [];
            ...
            foreach ($userAccessbleFields as $fieldId => $fieldName) {
                if (!in_array($fieldId, $def_org_fields)) {
                    ...
                    $db->pquery($insertQuery, [$tabId, $fieldId, 0, 0]);
                }
            }
        }
    }
