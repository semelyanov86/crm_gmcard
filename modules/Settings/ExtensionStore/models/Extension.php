...
class Settings_ExtensionStore_Extension_Model extends Vtiger_Base_Model
{
    ...
    protected $EXTENSIONSTORE_LOOKUP_URL;
    ...
    public function isAlreadyExists()
    {
        ...
        if (self::getLanguageInstance($moduleName)) {
            return true;
        }
    ...
    public static function getLanguageInstance($lang)
    {
        ...
        $result = $db->pquery($sql, [$lang, $lang]);
    ...
    public function getInstanceById($extensionId, $trial = false, $fileName = false)
    {
        ...
        if ($fileName) {
            ...
            if ($response['success']) {
                ...
                return ['success' => true, 'result' => $extensionModel];
            }
    ...
    public function getListings($id = null, $type = 'Extension')
    {
        ...
        if ($connector) {
            ...
            if ($listings['success']) {
                ...
                return ['success' => false, 'message' => $listings['error']];
            }
        }
    ...
    public function checkRegistration()
    {
        ...
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT 1 FROM ' . $tableName, []);
    ...
    public function passwordStatus()
    {
        $tableName = $this->getExtensionTable();
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT password FROM ' . $tableName, []);
    ...
    public function getRegisteredUser()
    {
        $tableName = $this->getExtensionTable();
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT username FROM ' . $tableName, []);
    ...
    public function forgotPassword($options)
    {
        ...
        return ['success' => false, 'error' => 'Invalid EmailAddress!'];
    }
