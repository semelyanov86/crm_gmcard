...
class Settings_ExtensionStore_Basic_Action extends Settings_Vtiger_IndexAjax_View
{
    ...
    protected function uninstallExtension(Vtiger_Request $request)
    {
        ...
        if ($extnType == 'module') {
            ...
            $response->setResult(['success' => true, 'message' => 'extension deleted']);
        } elseif ($extnType == 'language') {
    ...
    protected function deleteRecursively($dirName)
    {
        ...
        foreach ($files as $file) {
            ...
            $afterDeletefiles = array_diff(scandir($dirName), ['.', '..']);
    ...
    protected function updateTrialMode(Vtiger_Request $request)
    {
        ...
        $db->pquery('UPDATE vtiger_tab SET trial = ? WHERE name = ?', [$trial, $importedModuleName]);
        $response->setResult(['success' => true, 'message' => 'Extension Store Installed']);
        $response->emit();
    }
    ...
    public function forgotPassword(Vtiger_Request $request)
    {
        ...
        if ($forgotPasswordRes['success']) {
            $response->setResult(['message' => vtranslate('LBL_RESET_PASSWORD_LINK_EMAILED', $qualifiedModuleName)]);
        } else {
