...
if (defined('VTIGER_UPGRADE')) {
    ...
    $accounts = 'Accounts';
    $db->pquery('UPDATE vtiger_field SET isunique=? WHERE fieldname=? AND tabid=(SELECT tabid FROM vtiger_tab WHERE name=?)', [1, 'accountname', $accounts]);
    ...
    $restrictedModules = ['Dashboard', 'Home', 'Rss', 'Portal', 'Webmails', 'Import'];
    foreach ($moduleModels as $moduleModel) {
        ...
        $baseTableId = $moduleClass->table_index ?? null;
        if ($baseTableId) {
            $baseTableName = $moduleClass->table_name;
            $customTable = $moduleClass->customFieldTable ?? null;
            if (!$customTable) {
                continue;
            }
            ...
            if (!empty($customTableColumns) && in_array($fieldName, $customTableColumns)) {
                ...
                $db->pquery("UPDATE {$baseTableName}, {$customTableName} SET {$baseTableName}.tags={$customTableName}.tags WHERE {$baseTableName}.{$baseTableId}={$customTableName}.{$customTableId}", []);
    ...
    $packageModules = ['Project', 'ProjectTask', 'ProjectMilestone']; /* Projects zip is bundle */
    ...
    if (!Vtiger_Utils::CheckTable('vtiger_google_event_calendar_mapping')) {
        $db->pquery('CREATE TABLE vtiger_google_event_calendar_mapping (event_id VARCHAR(255) DEFAULT NULL, calendar_id VARCHAR(255) DEFAULT NULL, user_id INT(11) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8', []);
    ...
    if (Vtiger_Utils::CheckTable($generalUserFieldTable)) {
        ...
        $checkUserFieldConstraintExists = $db->pquery('SELECT DISTINCT 1 FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE table_name=? AND CONSTRAINT_SCHEMA=?', [$generalUserFieldTable, $db->dbName]);
        if ($db->num_rows($checkUserFieldConstraintExists) < 1) {
            $db->pquery('ALTER TABLE vtiger_crmentity_user_field ADD CONSTRAINT `fk_vtiger_crmentity_user_field_recordid` FOREIGN KEY (`recordid`) REFERENCES `vtiger_crmentity`(`crmid`) ON DELETE CASCADE', []);
        }

    }

    $migratedTables = [];
    $userTableResult = $db->pquery('SELECT vtiger_tab.tabid, vtiger_tab.name, tablename, fieldid FROM vtiger_field INNER JOIN vtiger_tab ON vtiger_tab.tabid=vtiger_field.tabid WHERE fieldname=?', ['starred']);
        ...
        if (Vtiger_Utils::CheckTable($oldTableName)) {
            if (!in_array($oldTableName, $migratedTables)) {
                if ($oldTableName != $generalUserFieldTable) {
                    ...
                    $db->pquery("INSERT INTO {$generalUserFieldTable} (recordid, userid, starred) (SELECT recordid,userid,starred FROM {$oldTableName} INNER JOIN vtiger_crmentity ON {$oldTableName}.recordid = vtiger_crmentity.crmid)", []);
    ...
    $appsList = ['Tools' => ['Rss', 'Portal', 'EmailTemplates', 'RecycleBin']];
    ...
    $tabResult2 = $db->pquery('SELECT tabid, name FROM vtiger_tab', []);
    $moduleTabIds = [];
    ...
    $defSequenceList = [
        ...
        'TOOLS' => [$moduleTabIds['EmailTemplates'],
            $moduleTabIds['Rss'],
            $moduleTabIds['Portal'],
    ...
    $db->pquery('DELETE FROM vtiger_app2tab WHERE appname=? AND tabid IN (?, ?, ?)', ['SUPPORT', $moduleTabIds['Project'], $moduleTabIds['ProjectTask'], $moduleTabIds['ProjectMilestone']]);
    $db->pquery('DELETE FROM vtiger_app2tab WHERE appname=? AND tabid=?', ['INVENTORY', $moduleTabIds['Assets']]);

    foreach ($defSequenceList as $appName => $tabIdsList) {
        $appTabResult1 = $db->pquery('SELECT tabid FROM vtiger_app2tab WHERE appname=? AND tabid NOT IN (' . generateQuestionMarks($tabIdsList) . ')', [$appName, $tabIdsList]);
    ...
    foreach ($defSequenceList as $appName => $tabIdsList) {
        foreach ($tabIdsList as $seq => $tabId) {
            $appTabResult2 = $db->pquery('SELECT 1 FROM vtiger_app2tab WHERE tabid=? AND appname=?', [$tabId, $appName]);

            $params = [$seq + 1, $tabId, $appName];
    ...
    $tabIdsList = [getTabid('Reports')];
    $actionIdPerms = [0 => 1, // Save
        1 => 1, // EditView
        2 => 1, // Delete
        3 => 0, // Index
        4 => 0, // DetailView
    ...
    for ($i = 0; $i < $adb->num_rows($result); ++$i) {
        ...
        foreach ($tabIdsList as $tabId) {
            foreach ($actionIdPerms as $actionId => $permission) {
                $isExist = $adb->pquery('SELECT 1 FROM vtiger_profile2standardpermissions WHERE profileid=? AND tabid=? AND operation=?', [$profileId, $tabId, $actionId]);
                ...
                $db->pquery($query, [$actionIdPerms[$actionId], $profileId, $tabId, $actionId]);
            }
        }
    }
    ...
    foreach ($cvTables as $tableName) {
        $updatedColumnsList = [];
        $result = $db->pquery("SELECT columnname FROM {$tableName} WHERE columnname LIKE ? OR columnname LIKE ?", ['vtiger_crmentity:createdtime%:T', 'vtiger_crmentity:modifiedtime%:T']);
    ...
    $reportTables = ['vtiger_selectcolumn', 'vtiger_relcriteria'];
