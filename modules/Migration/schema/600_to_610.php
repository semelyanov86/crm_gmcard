...
if ($adb->num_rows($result) <= 0) {
    ...
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_currencies (currencyid,currency_name,currency_code,currency_symbol) VALUES (' . $adb->getUniqueID('vtiger_currencies') . ',"Sudanese Pound","SDG","£")', []);
...
$fieldInstance->setPicklistValues(['Mobile Call']);
// 76 ends
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_mailscanner_rules ADD assigned_to INT(10), ADD cc VARCHAR(255), ADD bcc VARCHAR(255)', []);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_troubletickets MODIFY hours decimal(25,8)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_troubletickets MODIFY days decimal(25,8)', []);

Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? and fieldname=?', ['1', 'vtiger_pricebook', 'active']);
...
// Schema changes for vtiger_troubletickets hours & days column
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field set typeofdata=? WHERE fieldname IN(?,?) AND tablename = ?', ['N~O', 'hours', 'days', 'vtiger_troubletickets']);

// 79 ends

// 80 starts
// Added recurring enddate column for events,to vtiger_recurringevents table
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_recurringevents ADD COLUMN recurringenddate date', []);
...
// 82 starts
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_mailscanner CHANGE timezone time_zone VARCHAR(10)', []);
...
$result = $adb->pquery('SELECT task_id FROM com_vtiger_workflowtasks WHERE workflow_id IN
                        ...
                        AND task LIKE ?', ['Calendar', 'Events', '%VTSendNotificationTask%']);
$numOfRows = $adb->num_rows($result);
for ($i = 0; $i < $numOfRows; ++$i) {
    $tm = new VTTaskManager($adb);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_cron_task set description = ?  where name = "ScheduleReports" ', ['Recommended frequency for ScheduleReports is 15 mins']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_cron_task set module = ? where name = "ScheduleReports" ', ['Reports']);
...
if ($module) {
    ...
    if ($blockInstance) {
        ...
        if (!$desField) {
            ...
            $fieldInstance->setPicklistValues(['Planned', 'Held', 'Not Held']);
        }
        ...
        if (!$datField) {
            ...
            $fieldInstance1->setPicklistValues(['Call', 'Meeting']);
        }
    }
}
...
$userFields = ['user_name', 'email1', 'first_name', 'last_name', 'user_password', 'confirm_password', 'is_admin', 'roleid',
...
if (!$exist) {
    ...
    $result = Migration_Index_View::ExecuteQuery($shareaccessquery, [$ruleid, $EmailsTabId, 2, 0]);
...
// 92 starts
$result = $adb->pquery('SELECT max(templateid) AS maxtemplateid FROM vtiger_emailtemplates', []);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_emailtemplates_seq SET id = ?', [1 + ((int) $adb->query_result($result, 0, 'maxtemplateid'))]);
...
$fieldResult = $adb->pquery('SELECT fieldname, name, typeofdata FROM vtiger_field
                                INNER JOIN vtiger_tab ON vtiger_tab.tabid = vtiger_field.tabid WHERE typeofdata LIKE ?', ['D%']);
...
$dtresult = Migration_Index_View::ExecuteQuery($datatypeQuery, []);
if ($dtresult) {
...
$indexResult = Migration_Index_View::ExecuteQuery($indexQuery, []);
if ($indexResult) {
...
$module->enableTools(['Print', 'Export']);
echo '<br> enabled Print and export';

// 94 ends

// 95 starts
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_webforms MODIFY COLUMN description TEXT', []);
require_once 'vtlib/Vtiger/Module.php';
$module = Vtiger_Module::getInstance('Users');
if ($module) {
    $blockInstance = Vtiger_Block::getInstance('LBL_CALENDAR_SETTINGS', $module);
...
foreach ($modules as $module) {
    ...
    if ($moduleInstance) {
        $result = Migration_Index_View::ExecuteQuery('select blocklabel from vtiger_blocks where tabid=? and sequence = ?', [$moduleInstance->id, 1]);
...
$result = Migration_Index_View::ExecuteQuery('SELECT * FROM vtiger_calendar_default_activitytypes', []);
if ($adb->num_rows($result) <= 0) {
    $calendarViewTypes = ['Events' => ['Events' => '#17309A'],
        'Calendar' => ['Tasks' => '#3A87AD'],
        'Potentials' => ['Potentials' => '#AA6705'],
        'Contacts' => ['support_end_date' => '#953B39',
            'birthday' => '#545252'],
        'Invoice' => ['Invoice' => '#87865D'],
        'Project' => ['Project' => '#C71585'],
        'ProjectTask' => ['Project Task' => '#006400'],
    ];

    foreach ($calendarViewTypes as $module => $viewInfo) {
        foreach ($viewInfo as $fieldname => $color) {
            Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_calendar_default_activitytypes (id, module, fieldname, defaultcolor) VALUES (?,?,?,?)', [$adb->getUniqueID('vtiger_calendar_default_activitytypes'), $module, $fieldname, $color]);
        }
    }
...
if ($adb->num_rows($result) <= 0) {
    ...
    foreach ($allUsers as $userId => $userModel) {
        foreach ($activityIds as $activityId => $color) {
            Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', [$adb->getUniqueID('vtiger_calendar_user_activitytypes'), $activityId, $userId, $color]);
        }
    }
    echo '<br>Default Calendar view types added to the table for all existing users';
}

Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET quickcreate = ? WHERE tabid = 8 AND (fieldname = ? OR fieldname = ?);', [0, 'filename', 'filelocationtype']);
...
if (!defined('INSTALLATION_MODE')) {
    ...
    if (!$moduleInstance) {
        ...
        for ($i = 0; $i < $rowCount; ++$i) {
            ...
            // Update query
            $adb->pquery('UPDATE vtiger_pbxmanager SET customer = ? AND user = ? AND totalduration = ? AND callstatus = ? WHERE pbxmanagerid = ?', [$customer, $user, $timeofcall, $callstatus, $pbxmanagerid]);
        }
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;', [1, 'notes_title']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;', [1, 'assigned_user_id']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;', [1, 'notecontent']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;', [1, 'fileversion']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;', [1, 'filestatus']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;', [1, 'folderid']);
...
$result = $adb->pquery('SHOW COLUMNS FROM vtiger_tab LIKE ?', ['trial']);
if (!$adb->num_rows($result)) {
...
$moduleArray = ['Accounts', 'Leads', 'HelpDesk', 'Campaigns', 'Potentials', 'PurchaseOrder', 'SalesOrder', 'Quotes', 'Invoice'];
...
$contactsResult = $adb->pquery($contactsRelatedToQuery, [$tabId, 'contact_id']);
...
$module = 'Contacts';
$adb->pquery($insertContactsQuery, [$contactsFieldId, $module, $relModule, null, null]);
