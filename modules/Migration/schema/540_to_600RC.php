...
if (!defined('INSTALLATION_MODE')) {
    Migration_Index_View::ExecuteQuery('ALTER TABLE com_vtiger_workflows ADD COLUMN filtersavedinnew int(1)', []);
}

Migration_Index_View::ExecuteQuery('UPDATE com_vtiger_workflows SET filtersavedinnew = 5', []);

// Core workflow schema dependecy introduced in 6.1.0
$adb = PearDatabase::getInstance();
$result = $adb->pquery('show columns from com_vtiger_workflows like ?', ['schtypeid']);
if (!$adb->num_rows($result)) {
    $adb->pquery('ALTER TABLE com_vtiger_workflows ADD schtypeid INT(10)', []);
}
$result = $adb->pquery('show columns from com_vtiger_workflows like ?', ['schtime']);
if (!$adb->num_rows($result)) {
    $adb->pquery('ALTER TABLE com_vtiger_workflows ADD schtime TIME', []);
}
$result = $adb->pquery('show columns from com_vtiger_workflows like ?', ['schdayofmonth']);
if (!$adb->num_rows($result)) {
    $adb->pquery('ALTER TABLE com_vtiger_workflows ADD schdayofmonth VARCHAR(100)', []);
}
$result = $adb->pquery('show columns from com_vtiger_workflows like ?', ['schdayofweek']);
if (!$adb->num_rows($result)) {
    $adb->pquery('ALTER TABLE com_vtiger_workflows ADD schdayofweek VARCHAR(100)', []);
}
$result = $adb->pquery('show columns from com_vtiger_workflows like ?', ['schannualdates']);
if (!$adb->num_rows($result)) {
    $adb->pquery('ALTER TABLE com_vtiger_workflows ADD schannualdates VARCHAR(100)', []);
}
$result = $adb->pquery('show columns from com_vtiger_workflows like ?', ['nexttrigger_time']);
if (!$adb->num_rows($result)) {
    $adb->pquery('ALTER TABLE com_vtiger_workflows ADD nexttrigger_time DATETIME', []);
}
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_role SET rolename=? WHERE rolename=? and roleid=?', ['Sales Person', 'Sales Man', 'H5']);

if (!defined('INSTALLATION_MODE')) {
    $picklistResult = $adb->pquery('SELECT distinct fieldname FROM vtiger_field WHERE uitype IN (15,33)', []);
    ...
    for ($i = 0; $i < $numRows; ++$i) {
        ...
        Migration_Index_View::ExecuteQuery($query, []);
    }
}
...
$fieldInstance->setPicklistValues(['Cancel']);

// Email Reporting - added default email reports.

$sql = 'INSERT INTO vtiger_reportfolder (FOLDERNAME,DESCRIPTION,STATE) VALUES(?,?,?)';
$params = ['Email Reports', 'Email Reports', 'SAVED'];
Migration_Index_View::ExecuteQuery($sql, $params);

$reportmodules = [
    ['primarymodule' => 'Contacts', 'secondarymodule' => 'Emails'],
    ['primarymodule' => 'Accounts', 'secondarymodule' => 'Emails'],
    ['primarymodule' => 'Leads', 'secondarymodule' => 'Emails'],
    ['primarymodule' => 'Vendors', 'secondarymodule' => 'Emails'],
];

$reports = [
    ['reportname' => 'Contacts Email Report',
        'reportfolder' => 'Email Reports',
        'description' => 'Emails sent to Contacts',
        'reporttype' => 'tabular',
        'sortid' => '', 'stdfilterid' => '', 'advfilterid' => '0'],
    ['reportname' => 'Accounts Email Report',
        'reportfolder' => 'Email Reports',
        'description' => 'Emails sent to Organizations',
        'reporttype' => 'tabular',
        'sortid' => '', 'stdfilterid' => '', 'advfilterid' => '0'],
    ['reportname' => 'Leads Email Report',
        'reportfolder' => 'Email Reports',
        'description' => 'Emails sent to Leads',
        'reporttype' => 'tabular',
        'sortid' => '', 'stdfilterid' => '', 'advfilterid' => '0'],
    ['reportname' => 'Vendors Email Report',
        'reportfolder' => 'Email Reports',
...
$selectcolumns = [
    ...
    ['vtiger_account:accountname:Accounts_Account_Name:accountname:V',
    ...
    ['vtiger_leaddetails:lastname:Leads_Last_Name:lastname:V',
        ...
        'vtiger_activity:subject:Emails_Subject:subject:V',
        'vtiger_email_track:access_count:Emails_Access_Count:access_count:V'],
    ['vtiger_vendor:vendorname:Vendors_Vendor_Name:vendorname:V',
...
Migration_Index_View::ExecuteQuery("UPDATE vtiger_report SET sharingtype='Public'", []);
// End.

// Currency Decimal places handling
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_account MODIFY COLUMN annualrevenue decimal(25,5)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_leaddetails MODIFY COLUMN annualrevenue decimal(25,5)', []);
Migration_Index_View::ExecuteQuery("UPDATE vtiger_field SET typeofdata='N~O' WHERE fieldlabel='Annual Revenue' and typeofdata='I~O'", []);

Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_currency_info MODIFY COLUMN conversion_rate decimal(12,5)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_productcurrencyrel MODIFY COLUMN actual_price decimal(28,5)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_productcurrencyrel MODIFY COLUMN converted_price decimal(28,5)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_pricebookproductrel MODIFY COLUMN listprice decimal(27,5)', []);
...
$result = $adb->pquery('SELECT fieldname,tablename,columnname FROM vtiger_field WHERE uitype IN (?,?)', ['71', '72']);
...
for ($i = 0; $i < $count; ++$i) {
    ...
    if (!empty($tableAndColumnSize[$tableName][$columnName]['scale'])) {
        ...
        if ($decimalsToChange != 0) {
            ...
            Migration_Index_View::ExecuteQuery('ALTER TABLE ' . $tableName . ' MODIFY COLUMN ' . $columnName . ' decimal(?,?)', [$maxlength, $decimalDigits]);
        }
    }
}
...
$currency_decimals_field->setPicklistValues(['1', '2', '3', '4', '5']);
// Currency Decimal places handling - END

$inventoryModules = ['Invoice', 'SalesOrder', 'PurchaseOrder', 'Quotes'];
...
$itemFieldsName = ['productid', 'quantity', 'listprice', 'comment', 'discount_amount', 'discount_percent', 'tax1', 'tax2', 'tax3'];
$itemFieldsLabel = ['Item Name', 'Quantity', 'List Price', 'Item Comment', 'Item Discount Amount', 'Item Discount Percent', 'Tax1', 'Tax2', 'Tax3'];
$itemFieldsTypeOfData = ['V~M', 'V~M', 'V~M', 'V~O', 'V~O', 'V~O', 'V~O', 'V~O', 'V~O'];
$itemFieldsDisplayType = ['10', '7', '19', '19', '7', '7', '83', '83', '83'];
...
for ($i = 0; $i < php7_count($inventoryModules); ++$i) {
    ...
    $relatedmodules = ['Products', 'Services'];
...
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_tables(webservice_entity_id,table_name) VALUES (?,?)', [$webserviceObject->getEntityId(), 'vtiger_inventoryproductrel']);

$fieldTypeId = $adb->getUniqueID('vtiger_ws_entity_fieldtype');
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name, field_name,fieldtype) VALUES (?,?,?,?);', [$fieldTypeId, 'vtiger_inventoryproductrel', 'productid', 'reference']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'Products']);

$fieldTypeId = $adb->getUniqueID('vtiger_ws_entity_fieldtype');
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name, field_name,fieldtype) VALUES (?,?,?,?);', [$fieldTypeId, 'vtiger_inventoryproductrel', 'id', 'reference']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'Invoice']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'SalesOrder']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'PurchaseOrder']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'Quotes']);

$fieldTypeId = $adb->getUniqueID('vtiger_ws_entity_fieldtype');
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);', [$fieldTypeId, 'vtiger_inventoryproductrel', 'incrementondel', 'autogenerated']);

$adb->getUniqueID('vtiger_inventoryproductrel');
Migration_Index_View::ExecuteQuery('UPDATE vtiger_inventoryproductrel_seq SET id=coalesce((select max(lineitem_id) from vtiger_inventoryproductrel),0);', []);
Migration_Index_View::ExecuteQuery("UPDATE vtiger_ws_entity SET handler_path='include/Webservices/LineItem/VtigerInventoryOperation.php',handler_class='VtigerInventoryOperation' where name in ('Invoice','Quotes','PurchaseOrder','SalesOrder');", []);
...
Migration_Index_View::ExecuteQuery(
    'UPDATE vtiger_field SET block=? where tabid=? and block=?;',
    [$invoiceTabIdAddressInformationBlockId, $invoiceTabId, $purchaseOrderAddressInformationBlockId],
);
...
vtws_addActorTypeWebserviceEntityWithName(
    ...
    'VtigerTaxOperation',
    ['fieldNames' => 'taxlabel', 'indexField' => 'taxid', 'tableName' => 'vtiger_inventorytaxinfo'],
    true,
);

$webserviceObject = VtigerWebserviceObject::fromName($adb, 'Tax');
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_tables(webservice_entity_id,table_name) VALUES (?,?)', [$webserviceObject->getEntityId(), 'vtiger_inventorytaxinfo']);

vtws_addActorTypeWebserviceEntityWithoutName(
    'ProductTaxes',
    'include/Webservices/LineItem/VtigerProductTaxesOperation.php',
    'VtigerProductTaxesOperation',
    [],
);

$webserviceObject = VtigerWebserviceObject::fromName($adb, 'ProductTaxes');
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_tables(webservice_entity_id,table_name) VALUES (?,?)', [$webserviceObject->getEntityId(), 'vtiger_producttaxrel']);

$fieldTypeId = $adb->getUniqueID('vtiger_ws_entity_fieldtype');

Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);', [$fieldTypeId, 'vtiger_producttaxrel', 'productid', 'reference']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'Products']);

$fieldTypeId = $adb->getUniqueID('vtiger_ws_entity_fieldtype');
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);', [$fieldTypeId, 'vtiger_producttaxrel', 'taxid', 'reference']);
Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)', [$fieldTypeId, 'Tax']);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_cvcolumnlist SET columnname=? WHERE cvid=? AND columnindex=?', [$leadsFirstName, '1', '1']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_cvcolumnlist SET columnname=? WHERE cvid=? AND columnindex=?', [$leadsLastName, '1', '2']);

// Changed the Currency Symbol of Moroccan, Dirham to DH
Migration_Index_View::ExecuteQuery('UPDATE vtiger_currencies SET currency_symbol=? WHERE currency_name=? AND currency_code=?', ['DH', 'Moroccan, Dirham', 'MAD']);

// Changing picklist values for sales stage of opportunities
Migration_Index_View::ExecuteQuery('UPDATE vtiger_sales_stage SET sales_stage=? WHERE sales_stage=?', ['Proposal or Price Quote', 'Proposal/Price Quote']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_sales_stage SET sales_stage=? WHERE sales_stage=?', ['Negotiation or Review', 'Negotiation/Review']);

// Updating the new picklist values of sales stage in opportunities for migration instances
Migration_Index_View::ExecuteQuery('UPDATE vtiger_potential SET sales_stage=? WHERE sales_stage=?', ['Proposal or Price Quote', 'Proposal/Price Quote']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_potential SET sales_stage=? WHERE sales_stage=?', ['Negotiation or Review', 'Negotiation/Review']);

// Updating Sales Stage History in opportunities related list for migration instances
Migration_Index_View::ExecuteQuery('UPDATE vtiger_potstagehistory SET stage=? WHERE stage=?', ['Proposal or Price Quote', 'Proposal/Price Quote']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_potstagehistory SET stage=? WHERE stage=?', ['Negotiation or Review', 'Negotiation/Review']);
...
$discountResult = Migration_Index_View::ExecuteQuery('SELECT * FROM vtiger_selectcolumn WHERE columnname LIKE "vtiger_inventoryproductrel:discount:%" ORDER BY columnindex', []);
...
for ($i = 0; $i < $num_rows; ++$i) {
    ...
    $updateParams = [$updatedColumnName, $columnIndex, $queryId];

    Migration_Index_View::ExecuteQuery($updateQuery, $updateParams);
}

Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_ws_referencetype VALUES (?,?)', [31, 'Campaigns']);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_productcurrencyrel MODIFY COLUMN actual_price decimal(28,8)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_productcurrencyrel MODIFY COLUMN converted_price decimal(28,8)', []);
...
for ($i = 0; $i < $count; ++$i) {
    $tableName = $adb->query_result($result, $i, 'tablename');
    $columnName = $adb->query_result($result, $i, 'columnname');
    Migration_Index_View::ExecuteQuery('ALTER TABLE ' . $tableName . ' MODIFY COLUMN ' . $columnName . ' decimal(?,?)', [25, 8]);
}

Migration_Index_View::ExecuteQuery('DELETE FROM vtiger_no_of_currency_decimals WHERE no_of_currency_decimalsid=?', [1]);

Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET uitype=?, typeofdata=? WHERE fieldname=?', [71, 'N~O', 'listprice']);

Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET typeofdata=? WHERE fieldname=?', ['N~O', 'quantity']);

// --
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET typeofdata=?, uitype =?, fieldlabel=? WHERE fieldname =? and tablename=?', ['N~O', 71, 'Discount', 'discount_amount', 'vtiger_inventoryproductrel']);
...
$allTabIdResult = Migration_Index_View::ExecuteQuery('SELECT tabid, name FROM vtiger_tab', []);
...
$pickListValues = ['--None--', 'Open', 'In Progress', 'Completed', 'Deferred', 'Canceled '];
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_blocks SET blocklabel ="LBL_DESCRIPTION_INFORMATION" WHERE blockid=20', []);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET displaytype=1 WHERE fieldname="location" AND tabid = ?', [$calendarTabId]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET displaytype=1 WHERE fieldname="visibility" AND tabid = ?', [$eventTabId]);

$eventBlockId = getBlockId($eventTabId, 'LBL_EVENT_INFORMATION');
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET block = ? WHERE block = 41', [$eventBlockId]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_blocks SET blocklabel = "LBL_REMINDER_INFORMATION", show_title = 0 WHERE blockid = 40', []);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_blocks SET blocklabel = "LBL_DESCRIPTION_INFORMATION", show_title = 0 WHERE blockid = 41', []);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET block = 41 WHERE fieldname = "description" AND tabid = ?', [$eventTabId]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET block = ? WHERE fieldname = "contact_id" AND tabid = ?', [$eventBlockId, $eventTabId]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET displaytype = 3 WHERE fieldname = ? AND tabid = ?', ['notime', $eventTabId]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET displaytype = 3 WHERE fieldname = ? AND tabid = ?', ['duration_hours', $eventTabId]);
...
Migration_Index_View::ExecuteQuery(
    ...
    ['add', $helpDeskTabId, $projectTabId, $calendarTabId, $projectTaskTabId, $projectMilestoneTabId],
);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET presence = 1 WHERE tabid = ? AND fieldname = ?', [$helpDeskTabId, 'comments']);
$faqTabId = getTabid('Faq');
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET presence = 1 WHERE tabid = ? AND fieldname = ?', [$faqTabId, 'comments']);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_modtracker_basic ADD INDEX crmidx (crmid)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_modtracker_basic ADD INDEX idx (id)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_modtracker_detail ADD INDEX idx (id)', []);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_links MODIFY column linktype VARCHAR(50)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_links MODIFY column linklabel VARCHAR(50)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_links MODIFY column handler_class VARCHAR(50)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_links MODIFY column handler VARCHAR(50)', []);
...
if (!$customer) {
    ...
    $customer->setRelatedModules(['Contacts']);
}

require_once 'modules/ModComments/ModComments.php';
ModComments::addWidgetTo(['HelpDesk', 'Faq']);
...
$pageCount = 0;
do {
    $ticketComments = Migration_Index_View::ExecuteQuery(sprintf('SELECT * FROM vtiger_ticketcomments ORDER BY commentid ASC LIMIT %s,%s', $pageCount * 1_000, 1_000), []);
    ...
    for ($i = 0; $i < $rows; ++$i) {
        ...
        Migration_Index_View::ExecuteQuery(
            'UPDATE vtiger_crmentity SET modifiedtime = ?, smcreatorid = ?, modifiedby = ? WHERE crmid = ?',
            [$modComments->column_fields['createdtime'], $ownerId, $ownerId, $modComments->id],
        );
    }
    ++$pageCount;
} while (!$stopLoop);
...
$pageCount = 0;
do {
    $faqComments = Migration_Index_View::ExecuteQuery(sprintf('SELECT * FROM vtiger_faqcomments ORDER BY commentid ASC LIMIT %s, %s', $pageCount * 1_000, 1_000), []);
    ...
    for ($i = 0; $i < $rows; ++$i) {
        ...
        Migration_Index_View::ExecuteQuery(
            'UPDATE vtiger_crmentity SET modifiedtime = ?, smcreatorid = ?, modifiedby = ? WHERE crmid = ?',
            [$modComments->column_fields['createdtime'], $current_user->id, $current_user->id, $modComments->id],
        );
    }
    ++$pageCount;
} while (!$stopLoop);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_crmentity ADD COLUMN label varchar(255)', []);

// To avoid infinite-loop if we not able fix label for non-entity/special modules.
$lastMaxCRMId = 0;

while (true) {
    $rs = $adb->pquery('SELECT crmid,setype FROM vtiger_crmentity INNER JOIN vtiger_tab ON vtiger_tab.name=vtiger_crmentity.setype WHERE label IS NULL AND crmid > ? LIMIT 500', [$lastMaxCRMId]);
    if (!$adb->num_rows($rs)) {
        break;
    }
    ...
    while ($row = $adb->fetch_array($rs)) {
         ...
         */
        $labelInfo = getEntityName($row['setype'], [intval($row['crmid'])], true);
...
Migration_Index_View::ExecuteQuery('CREATE INDEX vtiger_crmentity_labelidx ON vtiger_crmentity(label)', []);
...
if (file_exists('modules/ModTracker/ModTrackerUtils.php')) {
    ...
    $modules = $adb->pquery('SELECT * FROM vtiger_tab WHERE isentitytype = 1', []);
...
Migration_Index_View::ExecuteQuery(
    'UPDATE vtiger_blocks SET sequence = sequence+1 WHERE tabid=? AND sequence >= 3',
    [$tabId],
);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET block=? WHERE fieldname=? and tabid=?', [$blockId, 'recurringtype', $tabId]);

// Update/Increment the sequence for the succeeding blocks of Users module, with starting sequence 2
$moduleInstance = Vtiger_Module::getInstance('Users');
$tabId = getTabid('Users');
Migration_Index_View::ExecuteQuery('UPDATE vtiger_blocks SET sequence = sequence+1 WHERE tabid=? AND sequence >= 2', [$tabId]);
...
$dayOfTheWeek->setPicklistValues(['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']);
...
$defaultCallDuration->setPicklistValues(['5', '10', '30', '60', '120']);
...
$otherEventDuration->setPicklistValues(['5', '10', '30', '60', '120']);
...
Migration_Index_View::ExecuteQuery($sql, [$blockId, 1, $tabId, 'vtiger_users', 'time_zone', 'activity_view', 'reminder_interval', 'date_format', 'start_hour', 'hour_format']);

Migration_Index_View::ExecuteQuery(
    'UPDATE vtiger_field SET uitype = ? WHERE tabid = ? AND tablename = ? AND columnname in (?,?)',
    [16, $tabId, 'vtiger_users', 'hour_format', 'start_hour'],
);

$fieldid = getFieldid($tabId, 'hour_format');
$hour_format = Vtiger_Field::getInstance($fieldid, $moduleInstance);
$hour_format->setPicklistValues([12, 24]);

$fieldid = getFieldid($tabId, 'start_hour');
$start_hour = Vtiger_Field::getInstance($fieldid, $moduleInstance);
$start_hour->setPicklistValues(['00:00', '01:00', '02:00', '03:00', '04:00', '05:00', '06:00', '07:00', '08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00', '20:00', '21:00', '22:00', '23:00']);

// update hour_format value in existing customers
Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET hour_format = ? WHERE hour_format = ? OR hour_format = ?', [12, 'am/pm', '']);

// add user default values
Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET dayoftheweek = ?, callduration = ?, othereventduration = ?, start_hour = ? ', ['Sunday', 5, 5, '00:00']);

$moduleInstance = Vtiger_Module::getInstance('Events');
$tabId = getTabid('Events');

// Update/Increment the sequence for the succeeding blocks of Events module, with starting sequence 4
Migration_Index_View::ExecuteQuery('UPDATE vtiger_blocks SET sequence = sequence+1 WHERE tabid=? AND sequence >= 4', [$tabId]);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET block=? WHERE fieldname IN (?,?) and tabid=?', [$blockId, 'contact_id', 'parent_id', $tabId]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET displaytype=1 WHERE fieldname=? and tabid=?', ['recurringtype', $tabId]);

// END 2012.12.02

// //////////////////////////////////////////////
$inventoryModules = [
    'Invoice' => ['LBL_INVOICE_INFORMATION', 'vtiger_invoice', 'invoiceid'],
    'SalesOrder' => ['LBL_SO_INFORMATION', 'vtiger_salesorder', 'salesorderid'],
    'PurchaseOrder' => ['LBL_PO_INFORMATION', 'vtiger_purchaseorder', 'purchaseorderid'],
    'Quotes' => ['LBL_QUOTE_INFORMATION', 'vtiger_quotes', 'quoteid'],
];
...
foreach ($inventoryModules as $module => $details) {
    ...
    $result = $adb->pquery("SELECT {$tableId}, subtotal, s_h_amount, discount_percent, discount_amount FROM {$tableName}", []);
    ...
    for ($i = 0; $i < $numOfRows; ++$i) {
        ...
        Migration_Index_View::ExecuteQuery("UPDATE {$tableName} set pre_tax_total = ? WHERE {$tableId} = ?", [$preTaxTotalValue, $id]);
    }
}
...
$calendarsharedtype->setPicklistValues(['public', 'private', 'seletedusers']);
...
foreach ($allUsers as $id => $name) {
    ...
    $count = $adb->num_rows($result);
    if ($count > 0) {
        Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET calendarsharedtype = ? WHERE id = ?', ['selectedusers', $id]);
    } else {
        Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET calendarsharedtype = ? WHERE id = ? ', ['public', $id]);
        foreach ($allUsers as $sharedid => $name) {
            if ($sharedid != $id) {
                $sql = 'INSERT INTO vtiger_sharedcalendar VALUES (?,?)';
                Migration_Index_View::ExecuteQuery($sql, [$id, $sharedid]);
            }
        }
    }
}
...
$moduleArray = ['Accounts' => 'LBL_ACCOUNT_INFORMATION', 'Contacts' => 'LBL_CONTACT_INFORMATION', 'Potentials' => 'LBL_OPPORTUNITY_INFORMATION'];
...
$viewField->setPicklistValues(['Summary', 'Detail']);

Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET default_record_view = ?', ['Summary']);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_module_dashboard_widgets ADD COLUMN position VARCHAR(50)', []);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_cvadvfilter MODIFY comparator VARCHAR(20)', []);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET typeofdata = ? WHERE columnname = ? AND tablename = ?', ['V~O', 'company', 'vtiger_leaddetails']);
...
if (Vtiger_Utils::CheckTable('vtiger_cron_log')) {
    Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_cron_log MODIFY COLUMN start INT(11) UNSIGNED', []);
    Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_cron_log MODIFY COLUMN end INT(11) UNSIGNED', []);
}
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_mailscanner ADD COLUMN timezone VARCHAR(10) default NULL', []);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_mailscanner SET timezone=? WHERE server LIKE ? AND timezone IS NULL', ['-8:00', '%.gmail.com']);

Migration_Index_View::ExecuteQuery('UPDATE vtiger_report SET state=?', ['CUSTOM']);

Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_relcriteria MODIFY value VARCHAR(512)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_cvadvfilter MODIFY value VARCHAR(512)', []);
// End 2013.03.19
...
// Start 2013.04.30
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_users MODIFY signature TEXT', []);
...
if (!defined('INSTALLATION_MODE')) {
    Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_salutationtype ADD COLUMN sortorderid INT(1)', []);
}

$summaryFields = [
    'Accounts'	=> ['assigned_user_id', 'email1', 'phone', 'bill_city', 'bill_country', 'website'],
    'Contacts'	=> ['assigned_user_id', 'email', 'phone', 'mailingcity', 'mailingcountry'],
    'Leads'		=> ['assigned_user_id', 'email', 'phone', 'city', 'country', 'leadsource'],
    'HelpDesk'	=> ['assigned_user_id', 'ticketstatus', 'parent_id', 'ticketseverities', 'description'],
    'Potentials' => ['assigned_user_id', 'amount', 'sales_stage', 'closingdate'],
    'Project'	=> ['assigned_user_id', 'targetenddate']];
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? AND fieldname= ?', ['Active', 'vtiger_users', 'status']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? AND fieldname= ?', ['12', 'vtiger_users', 'hour_format']);
...
for ($i = 0; $i < $adb->num_rows($result); ++$i) {
    ...
    $fieldsResult = $adb->pquery($sql, [$profileId, $tabId]);
    $profile2Fields = [];
    ...
    foreach ($userAccessbleFields as $fieldId => $fieldName) {
        if (!in_array($fieldId, $profile2Fields)) {
            ...
            Migration_Index_View::ExecuteQuery($insertQuery, [$profileId, $tabId, $fieldId, 0, 0]);
        }
    }
}
...
foreach ($userAccessbleFields as $fieldId => $fieldName) {
    if (!in_array($fieldId, $def_org_fields)) {
        ...
        Migration_Index_View::ExecuteQuery($insertQuery, [$tabId, $fieldId, 0, 0]);
    }
}
...
$result = $adb->pquery($sql, [15, 16, 33, 55]);
$num_rows = $adb->num_rows($result);
for ($i = 0; $i < $num_rows; ++$i) {
    $fieldName = $adb->query_result($result, $i, 'fieldname');
    $checkTable = $adb->pquery('SHOW TABLES LIKE "vtiger_' . $fieldName . '"', []);
    if ($adb->num_rows($checkTable) > 0) {
        $query = "DELETE FROM vtiger_{$fieldName} WHERE {$fieldName} = ? OR {$fieldName} = ?";
        Migration_Index_View::ExecuteQuery($query, ['--None--', 'None']);
    }
}
...
if (!$leadsOptOutField) {
    ...
    Migration_Index_View::ExecuteQuery('UPDATE vtiger_leaddetails SET emailoptout=0 WHERE emailoptout IS NULL', []);
}
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_module_dashboard_widgets MODIFY data TEXT', []);

$linkIdResult = $adb->pquery('SELECT linkid FROM vtiger_links WHERE vtiger_links.linklabel="Notebook"', []);
...
$result = $adb->pquery('SELECT vtiger_homestuff.stufftitle, vtiger_homestuff.userid, vtiger_notebook_contents.contents FROM
						...
						WHERE vtiger_homestuff.stufftype = ?', ['Notebook']);
...
for ($i = 0; $i < $adb->num_rows($result); ++$i) {
    ...
    $params = [$noteBookLinkId, $userId, 0, $noteBookTitle, $noteBookContent];
    Migration_Index_View::ExecuteQuery($query, $params);
}
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_invoice MODIFY balance decimal(25,8)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_invoice MODIFY received decimal(25,8)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_purchaseorder MODIFY balance decimal(25,8)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_purchaseorder MODIFY paid decimal(25,8)', []);

$labels = ['LBL_ADD_NOTE', 'Add Note'];
$sql = 'UPDATE vtiger_links SET handler = ?, handler_class = ?, handler_path = ? WHERE linklabel IN (?, ?)';
Migration_Index_View::ExecuteQuery($sql, ['isLinkPermitted', 'Documents', 'modules/Documents/Documents.php', $labels]);

$sql = 'UPDATE vtiger_links SET handler = ?, handler_class = ?, handler_path = ? WHERE linklabel = ?';
Migration_Index_View::ExecuteQuery($sql, ['isLinkPermitted', 'ProjectTask', 'modules/ProjectTask/ProjectTask.php', 'Add Project Task']);

Migration_Index_View::ExecuteQuery('DELETE FROM vtiger_settings_field WHERE name=?', ['EMAILTEMPLATES']);
...
$result = $adb->pquery($query, ['received', 'paid', 'balance', $tabIdList]);
$numrows = $adb->num_rows($result);

for ($i = 0; $i < $numrows; ++$i) {
    $fieldid = $adb->query_result($result, $i, 'fieldid');
    $query = 'Update vtiger_profile2field set readonly = 0 where fieldid=?';
    Migration_Index_View::ExecuteQuery($query, [$fieldid]);
}
...
Migration_Index_View::ExecuteQuery("UPDATE vtiger_leaddetails SET salutation='' WHERE salutation = ?", ['--None--']);

// Update contacts salutation value of none to empty value
Migration_Index_View::ExecuteQuery("UPDATE vtiger_contactdetails SET salutation='' WHERE salutation = ?", ['--None--']);
// END 2013-06-25

// Start 2013-09-24
Migration_Index_View::ExecuteQuery(
    'UPDATE vtiger_eventhandlers SET handler_path = ? WHERE handler_class = ?',
    ['modules/Vtiger/handlers/RecordLabelUpdater.php', 'Vtiger_RecordLabelUpdater_Handler'],
);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_invoice_recurring_info ADD PRIMARY KEY (salesorderid)', []);
...
$result = $adb->pquery('SELECT task_id FROM com_vtiger_workflowtasks WHERE workflow_id IN
						...
						AND summary = ?', ['Calendar', 'Events', 'Send Notification Email to Record Owner']);
$numOfRows = $adb->num_rows($result);
...
Migration_Index_View::ExecuteQuery(
    'UPDATE vtiger_field SET masseditable = ? where fieldname = ? and tabid = ?',
    ['1', 'accountname', getTabid('Accounts')],
);

$result = $adb->pquery('SELECT taxname FROM vtiger_shippingtaxinfo', []);
$numOfRows = $adb->num_rows($result);
$shippingTaxes = [];
$tabIds = [];
for ($i = 0; $i < $numOfRows; ++$i) {
...
$modules = ['Invoice', 'Quotes', 'PurchaseOrder', 'SalesOrder'];
...
Migration_Index_View::ExecuteQuery(
    ...
    ['Hong Kong, Dollars', 'LvHong Kong, Dollars', 'HKD'],
);
Migration_Index_View::ExecuteQuery(
    'UPDATE vtiger_currency_info SET currency_name = ? where currency_name = ?',
    ['Hong Kong, Dollars', 'LvHong Kong, Dollars'],
);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=1 WHERE fieldname = ?', ['filestatus']);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_role ADD allowassignedrecordsto INT(2) NOT NULL DEFAULT 1', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_assets MODIFY datesold date', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_assets MODIFY dateinservice date', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_assets MODIFY serialnumber varchar(200)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_assets MODIFY account int(19)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE com_vtiger_workflowtask_queue ADD COLUMN task_contents text', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE com_vtiger_workflowtask_queue DROP INDEX com_vtiger_workflowtask_queue_idx', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_mailscanner_ids modify column messageid varchar(512)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_mailscanner_ids add index scanner_message_ids_idx (scannerid, messageid)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_mailscanner_folders add index folderid_idx (folderid)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_leaddetails add index email_idx (email)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_contactdetails add index email_idx (email)', []);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_account add index email_idx (email1, email2)', []);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET leftpanelhide = ?', [0]);
...
$relatedToField->unsetRelatedModules(['Contacts']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET typeofdata = ? WHERE fieldid = ?', ['V~O', $relatedToField->id]);
...
if (!$contactField) {
    ...
    Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET summaryfield=1 WHERE fieldid = ?', [$contactField->id]);
}
...
while (true) {
    $result = $adb->pquery(
        'SELECT potentialid ,related_to FROM vtiger_potential WHERE potentialid > ? LIMIT 500',
        [$lastPotentialId],
    );
    if (!$adb->num_rows($result)) {
        break;
    ...
    while ($row = $adb->fetch_array($result)) {
        ...
        if ($relatedToType != 'Accounts') {
            Migration_Index_View::ExecuteQuery(
                'UPDATE vtiger_potential SET contact_id = ?, related_to = null WHERE potentialid = ?',
                [$relatedTo, $potentialId],
            );
        }
...
$filterResult = $adb->pquery(
    'SELECT * FROM vtiger_cvadvfilter WHERE columnname like ?',
    ['vtiger_potential:related_to:related_to:Potentials_Related_%'],
);
...
for ($i = 0; $i < $rows; ++$i) {
    ...
    Migration_Index_View::ExecuteQuery('UPDATE vtiger_cvadvfilter SET groupid = 2, column_condition = ? WHERE cvid = ?', ['or', $cvid]);
    Migration_Index_View::ExecuteQuery('UPDATE vtiger_cvadvfilter_grouping SET groupid = 2 WHERE cvid = ?', [$cvid]);
    ...
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_cvadvfilter(cvid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', [$cvid, ++$columnIndex, 'vtiger_potential:contact_id:contact_id:Potentials_Contact_Name:V',
        $comparator, $value, 2, '']);
}
unset($filterResult);

$filterColumnList = $adb->pquery(
    'SELECT * FROM vtiger_cvcolumnlist WHERE columnname like ?',
    ['vtiger_potential:related_to:related_to:Potentials_Related_%'],
);
$filterColumnRows = $adb->num_rows($filterColumnList);
for ($j = 0; $j < $filterColumnRows; ++$j) {
    $cvid = $adb->query_result($filterColumnList, $j, 'cvid');
    $filterResult = $adb->pquery('SELECT MAX(columnindex) AS maxcolumn FROM vtiger_cvcolumnlist WHERE cvid = ?', [$cvid]);
    $maxColumnIndex = $adb->query_result($filterResult, 0, 'maxcolumn');
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_cvcolumnlist(cvid,columnindex,columnname) VALUES (?,?,?)', [$cvid, ++$maxColumnIndex,
        'vtiger_potential:contact_id:contact_id:Potentials_Contact_Name:V']);
    unset($filterResult);
}
unset($filterColumnList);

$reportColumnResult = $adb->pquery(
    'SELECT * FROM vtiger_selectcolumn WHERE columnname = ?',
    ['vtiger_potential:related_to:Potentials_Related_To:related_to:V'],
);
...
for ($k = 0; $k < $reportColumnRows; ++$k) {
    ...
    $filterResult = $adb->pquery('SELECT MAX(columnindex) AS maxcolumn FROM vtiger_selectcolumn WHERE queryid = ?', [$reportId]);
    $maxColumnIndex = $adb->query_result($filterResult, 0, 'maxcolumn');
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_selectcolumn(queryid,columnindex,columnname) VALUES (?,?,?)', [$reportId,
...
$filterResult = $adb->pquery(
    'SELECT * FROM vtiger_relcriteria WHERE columnname = ?',
    ['vtiger_potential:related_to:Potentials_Related_To:related_to:V'],
);
$rows = $adb->num_rows($filterResult);
for ($i = 0; $i < $rows; ++$i) {
    ...
    Migration_Index_View::ExecuteQuery('UPDATE vtiger_relcriteria SET groupid = 2, column_condition = ? WHERE queryid = ?', ['or', $reportId]);
    Migration_Index_View::ExecuteQuery('UPDATE vtiger_relcriteria_grouping SET groupid = 2 WHERE queryid = ?', [$reportId]);
    ...
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_relcriteria(queryid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', [$reportId, ++$columnIndex, 'vtiger_potential:contact_id:Potentials_Contact_Name:contact_id:V',
        $comparator, $value, 2, '']);
}
unset($filterResult);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET uitype = 10 WHERE fieldid = ?', [$relatedToField->id]);
$relatedToField->setRelatedModules(['Accounts']);
...
while (true) {
    $ticketsResult = $adb->pquery('SELECT ticketid ,parent_id FROM vtiger_troubletickets WHERE ticketid > ?
						LIMIT 500', [$lastTicketId]);
    if (!$adb->num_rows($ticketsResult)) {
        break;
    }
    ...
    while ($row = $adb->fetch_array($ticketsResult)) {
        ...
        if ($parentType != 'Accounts') {
            Migration_Index_View::ExecuteQuery(
                'UPDATE vtiger_troubletickets SET contact_id = ?, parent_id = null WHERE ticketid = ?',
                [$parent, $ticketId],
            );
        }
...
$ticketFilterResult = $adb->pquery(
    'SELECT * FROM vtiger_cvadvfilter WHERE columnname like ?',
    ['vtiger_troubletickets:parent_id:parent_id:HelpDesk_Related%'],
);
...
for ($i = 0; $i < $rows; ++$i) {
    ...
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_cvadvfilter(cvid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', [$cvid, ++$columnIndex, 'vtiger_troubletickets:contact_id:contact_id:HelpDesk_Contact_Name:V',
        $comparator, $value, 2, '']);
}
...
$filterColumnList = $adb->pquery(
    'SELECT * FROM vtiger_cvcolumnlist WHERE columnname like ?',
    ['vtiger_troubletickets:parent_id:parent_id:HelpDesk_Related_%'],
);
$filterColumnRows = $adb->num_rows($filterColumnList);
for ($j = 0; $j < $filterColumnRows; ++$j) {
    $cvid = $adb->query_result($filterColumnList, $j, 'cvid');
...
$reportColumnResult = $adb->pquery(
    'SELECT * FROM vtiger_selectcolumn WHERE columnname like ?',
    ['vtiger_troubletickets:parent_id:HelpDesk_Related_To:parent_id%'],
);
$reportColumnRows = $adb->num_rows($reportColumnResult);
for ($k = 0; $k < $reportColumnRows; ++$k) {
    $reportId = $adb->query_result($reportColumnResult, $k, 'queryid');
...
$filterResult = $adb->pquery(
    'SELECT * FROM vtiger_relcriteria WHERE columnname like ?',
    ['vtiger_troubletickets:parent_id:HelpDesk_Related_To:parent_id%'],
);
$rows = $adb->num_rows($filterResult);
for ($i = 0; $i < $rows; ++$i) {
    ...
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_relcriteria(queryid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', [$reportId, ++$columnIndex, 'vtiger_troubletickets:contact_id:HelpDesk_Contact_Name:contact_id:V',
        $comparator, $value, 2, '']);
}
unset($filterResult);
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', ['Active', 'vtiger_users', 'status']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', ['12', 'vtiger_users', 'hour_format']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', ['softed', 'vtiger_users', 'theme']);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', ['Monday', 'vtiger_users', 'dayoftheweek']);
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_shorturls ADD COLUMN onetime int(5)', []);

$checkQuery = 'SELECT 1 FROM vtiger_currencies  WHERE currency_name=?';
$checkResult = $adb->pquery($checkQuery, ['Iraqi Dinar']);
if ($adb->num_rows($checkResult) <= 0) {
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_currencies VALUES (' . $adb->getUniqueID('vtiger_currencies') . ',"Iraqi Dinar","IQD","ID")', []);
}
...
$result = $adb->pquery('SELECT sequence,block FROM vtiger_field WHERE fieldid = ? and tabid = ?', [$relatedToField->id, $potentialTabId]);
$relatedToFieldSequence = $adb->query_result($result, 0, 'sequence');
$relatedToFieldBlock = $adb->query_result($result, 0, 'block');

Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET sequence = sequence+1 WHERE sequence > ? and tabid = ? and block = ?', [$relatedToFieldSequence, $potentialTabId, $relatedToFieldBlock]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET sequence = ? WHERE fieldid = ?', [$relatedToFieldSequence + 1, $contactField->id]);
...
$result = $adb->pquery('SELECT sequence,block FROM vtiger_field WHERE fieldid = ? and tabid = ?', [$relatedToField->id, $ticketsTabId]);
$relatedToFieldSequence = $adb->query_result($result, 0, 'sequence');
$relatedToFieldBlock = $adb->query_result($result, 0, 'block');

Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET sequence = sequence+1 WHERE sequence > ? and tabid = ? and block = ?', [$relatedToFieldSequence, $ticketsTabId, $relatedToFieldBlock]);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_field SET sequence = ? WHERE fieldid = ?', [$relatedToFieldSequence + 1, $contactField->id]);

$checkQuery = 'SELECT 1 FROM vtiger_currencies  WHERE currency_name=?';
$checkResult = $adb->pquery($checkQuery, ['Maldivian Ruffiya']);
if ($adb->num_rows($checkResult) <= 0) {
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_currencies VALUES (' . $adb->getUniqueID('vtiger_currencies') . ',"Maldivian Ruffiya","MVR","MVR")', []);
}


$result = $adb->pquery('SELECT count(*) AS count FROM vtiger_emailtemplates', []);
Migration_Index_View::ExecuteQuery('UPDATE vtiger_emailtemplates_seq SET id = ?', [1 + ((int) $adb->query_result($result, 0, 'count'))]);
...
if (!$usersRowHeightField) {
    ...
    $field->setPicklistValues(['wide', 'medium', 'narrow']);
}
...
$result = $adb->pquery('SELECT DISTINCT workflow_id FROM com_vtiger_workflowtasks WHERE workflow_id IN
				...
				AND task LIKE ?', ['ModComments', 1, '%VTEntityMethodTask%']);
$numOfRows = $adb->num_rows($result);
...
$result = $adb->pquery('SELECT DISTINCT workflow_id FROM com_vtiger_workflowtasks WHERE workflow_id IN
				(SELECT workflow_id FROM com_vtiger_workflows WHERE module_name IN (?) AND defaultworkflow = ?)
				AND task LIKE ?', [$moduleName, 1, '%VTEntityMethodTask%']);
$numOfRows = $adb->num_rows($result);
    ...
    foreach ($tasks as $task) {
        ...
        switch ($properties['methodName']) {
            ...
            case 'NotifyOnPortalTicketComment':
                ...
                Migration_Index_View::ExecuteQuery('DELETE FROM com_vtiger_workflows WHERE workflow_id = ?', [$workflowModel->id]);
                break;
...
$result = $adb->pquery('SELECT blockid FROM vtiger_blocks where tabid = ? AND (blocklabel is NULL OR blocklabel = "")', [getTabid('Emails')]);
$numOfRows = $adb->num_rows($result);
...
$result = $adb->pquery('SELECT task_id FROM com_vtiger_workflowtasks WHERE workflow_id IN
																	(SELECT workflow_id FROM com_vtiger_workflows WHERE module_name IN (?, ?))
																	AND task LIKE ?', ['Calendar', 'Events', '%$(contact_id : (Contacts) lastname) $(contact_id : (Contacts) firstname)%']);
$numOfRows = $adb->num_rows($result);
...
$result = $adb->pquery('SELECT 1 FROM vtiger_currencies WHERE currency_name = ?', ['Ugandan Shilling']);
if (!$adb->num_rows($result)) {
    Migration_Index_View::ExecuteQuery('INSERT INTO vtiger_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
        [$adb->getUniqueID('vtiger_currencies'), 'Ugandan Shilling', 'UGX', 'Sh']);
}
$em = new VTEventsManager($adb);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE vtiger_inventoryproductrel MODIFY comment varchar(500)', []);
