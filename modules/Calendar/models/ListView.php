...
class Calendar_ListView_Model extends Vtiger_ListView_Model
{
    ...
    public function getListViewEntries($pagingModel)
    {
        ...
        $queryGenerator = $this->get('query_generator');
        $listViewContoller = $this->get('listview_controller');
        $listViewFields = ['visibility', 'assigned_user_id'];
        ...
        } elseif (!empty($orderBy) && $orderByFieldModel) {
        ...
        foreach ($listViewEntries as $recordId => $record) {
            ...
            $visibleFields = ['activitytype', 'date_start', 'due_date', 'assigned_user_id', 'visibility', 'smownerid'];
            ...
            if (!$currentUser->isAdminUser() && $rawData['activitytype'] != 'Task' && $rawData['visibility'] == 'Private' && $ownerId && $visibility) {
                foreach ($record as $data => $value) {
                    if (in_array($data, $visibleFields) != -1) {
                        unset($rawData[$data], $record[$data]);
            ...
            if (isset($record['activitytype']) && $record['activitytype'] == 'Task') {
                unset($record['visibility'], $rawData['visibility']);
