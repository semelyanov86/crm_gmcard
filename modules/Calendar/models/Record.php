...
class Calendar_Record_Model extends Vtiger_Record_Model
{
    ...
    public function delete()
    {
        ...
        if (!empty($recurringEditMode) && $recurringEditMode != 'current') {
            ...
            if ($recurringEditMode == 'current') {
                ...
                $adb->pquery('DELETE FROM vtiger_activity_recurring_info WHERE activityid=? AND recurrenceid=?', [$parentRecurringId, $this->getId()]);
            }
    ...
    {
        $db = PearDatabase::getInstance();
        $db->pquery('UPDATE vtiger_activity_reminder_popup set status = ? where recordid = ?', [$status, $this->getId()]);
    ...
    public function getParentRecurringRecord()
    {
        $adb = PearDatabase::getInstance();
        $recordId = $this->getId();
        $result = $adb->pquery('SELECT * FROM vtiger_activity_recurring_info WHERE activityid=? OR activityid = (SELECT activityid FROM vtiger_activity_recurring_info WHERE recurrenceid=?) LIMIT 1', [$recordId, $recordId]);
    ...
    {
        $adb = PearDatabase::getInstance();
        $recurringRecordsList = [];
        $recordId = $this->getId();
        $result = $adb->pquery('SELECT * FROM vtiger_activity_recurring_info WHERE activityid=? OR activityid = (SELECT activityid FROM vtiger_activity_recurring_info WHERE recurrenceid=?)', [$recordId, $recordId]);
