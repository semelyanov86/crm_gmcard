...
class Calendar_Edit_View extends Vtiger_Edit_View
{
    ...
    public function Events($request, $moduleName)
    {
        ...
        $viewer->assign('TOMORROWDATE', Vtiger_Date_UIType::getDisplayDateValue(date('Y-m-d', time() + 86_400)));
        ...
        if (!empty($requestContactIdValue)) {
            $existingRelatedContacts[] = ['name' => decode_html(Vtiger_Util_Helper::getRecordName($requestContactIdValue)), 'id' => $requestContactIdValue];
        }
        ...
        if (!empty($idsList)) {
            ...
            foreach ($contactIdsList as $contactId) {
                $existingRelatedContacts[] = ['name' => decode_html(Vtiger_Util_Helper::getRecordName($contactId)), 'id' => $contactId];
            }
        }
