...
class Appointment
{
    ...
    public static function readAppointment($userid, &$from_datetime, &$to_datetime, $view)
    {
        ...
        while ($a < $n) {
            ...
            for ($j = $from; $j <= $to; $j = $j + (60 * 60 * 24)) {
                ...
                [$obj->temphour, $obj->tempmin] = explode(':', $result['time_start']);
                if ($start_timestamp != $end_timestamp && $view == 'day') {
                    if ($j == $start_timestamp) {
                        ...
                        [$obj->temphour, $obj->tempmin] = $current_user->start_hour != '' ? explode(':', $current_user->start_hour) : explode(':', '08:00');
                        $result['duration_hours'] = 24 - $obj->temphour;
                    } elseif ($j == $end_timestamp) {
                        [$obj->temphour, $obj->tempmin] = $current_user->start_hour != '' ? explode(':', $current_user->start_hour) : explode(':', '08:00');
                        [$ehr, $emin] = explode(':', $result['time_end']);
    ...
    public function readResult($act_array, $view)
    {
        ...
        [$eventStartHour] = explode(':', $date->getDisplayTime());
        [$styear, $stmonth, $stday] = explode('-', $act_array['date_start']);
        [$sthour, $stmin] = explode(':', $act_array['time_start']);
        ...
        [$eyear, $emonth, $eday] = explode('-', $act_array['due_date']);
        [$end_hour, $end_min] = explode(':', $act_array['time_end']);
        ...
        if ($view == 'day' || $view == 'week') {
            ...
            [$year, $month, $date] = explode('-', $eventStartDate);
