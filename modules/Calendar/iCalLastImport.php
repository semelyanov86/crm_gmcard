...
class iCalLastImport
{
    ...
    public function clearRecords($userId)
    {
        ...
        if (Vtiger_Utils::CheckTable($this->tableName)) {
            $adb->pquery('DELETE FROM ' . $this->tableName . ' WHERE userid = ?', [$userId]);
        }
    }
    ...
    public function save()
    {
        ...
        if (php7_count($this->fieldData) == 0) {
            return;
        }
        ...
        $adb->pquery(
            'INSERT INTO ' . $this->tableName . '(' . implode(',', $fieldNames) . ') VALUES (' . generateQuestionMarks($fieldValues) . ')',
            [$fieldValues],
        );
    }
