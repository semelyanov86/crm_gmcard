...
class ModuleLinkCreator_Module_Model extends ModuleLinkCreatorModule_Module_Model
{
    ...
    {
        $settingsLinks[] = array("linktype" => "MODULESETTING", "linklabel" => vtranslate("Settings", $this->name), "linkurl" => "index.php?module=" . $this->name . "&view=List", "linkicon" => "");
        $settingsLinks[] = array("linktype" => "MODULESETTING", "linklabel" => vtranslate("Uninstall", $this->name), "linkurl" => "index.php?module=" . $this->name . "&parent=Settings&view=Uninstall", "linkicon" => "");
    ...
    public function getRelation_1_1()
    {
        ...
        if ($adb->num_rows($result)) {
            while ($row = $adb->fetch_array($result)) {
                $relations[] = array("fieldid" => $row["fieldid"], "module" => $row["module"], "relmodule" => $row["relmodule"], "sequence" => $row["sequence"], "tabid" => $row["tabid"], "related_tabid" => $row["related_tabid"], "tab_name" => $row["tab_name"], "tab_tablabel" => vtranslate($row["tab_tablabel"], $row["tab_name"]), "related_tab_name" => $row["related_tab_name"], "related_tab_tablabel" => vtranslate($row["related_tab_tablabel"], $row["related_tab_name"]));
            }
        }
    ...
    public function getRelation_1_M()
    {
        ...
        $nameRelatedList = array("get_dependents_list", "get_activities", "get_related_list");
    ...
    public function getRelation_1_None()
    {
        ...
        if ($adb->num_rows($result)) {
            while ($row = $adb->fetch_array($result)) {
                $relations[] = array("fieldid" => $row["fieldid"], "fieldlabel" => $row["fieldlabel"], "module1" => vtranslate($row["module"], $row["module"]), "module2" => vtranslate($row["relmodule"], $row["relmodule"]));
            }
        }
        return $relations;
    }
