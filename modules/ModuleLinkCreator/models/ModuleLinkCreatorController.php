...
class ModuleLinkCreator_ModuleController extends Vtiger_Tools_Console_ModuleController
{
     ...
     */
    public function createModule($moduleName, $parent = "Tools", $entityfieldlabel = "Name", $singularModuleLabel, $base_permissions = 0)
    {
        global $vtiger_current_version;
        global $adb;
        if ($parent == "PROJECT") {
    ...
    public function makeModuleAvailableForAdmin($moduleInstance)
    {
        ...
        foreach ($profileids as $profileid) {
            $adb->pquery("INSERT INTO vtiger_profile2tab (profileid, tabid, permissions) VALUES (?,?,?)", array($profileid, $moduleInstance->id, 1));
            if ($moduleInstance->isentitytype) {
                foreach ($actionids as $actionid) {
                    $adb->pquery("INSERT INTO vtiger_profile2standardpermissions (profileid, tabid, Operation, permissions) VALUES(?,?,?,?)", array($profileid, $moduleInstance->id, $actionid, 1));
                }
            }
        }
    }
