...
class ModuleLinkCreator_ActionAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function saveRelatedFields(Vtiger_Request $request)
    {
        ...
        if (50 < strlen($request->get("field_label"))) {
            $fieldsResponse["result"] = "fail";
                    ...
                    if (($relatedFieldModel->validateField($module1, $module2) || $relatedFieldModel->validateCustomBuildFields($module1, $module2)) && $actionRelatedList != "true") {
                        ...
                        try {
                            ...
                            if ($actionRelatedList == "undefined" || $actionRelatedList != true) {
                                if ($module1 == "Events" || $module1 == "Calendar") {
                                    $module21->setRelatedList($module12, vtranslate("Activities"), array("ADD"), "get_activities");
                                    $activityFieldTypeId = 34;
                                    $sqlCheckProject = "SELECT * FROM `vtiger_ws_referencetype` WHERE fieldtypeid = ? AND type = ?";
                                    $rsCheckProject = $adb->pquery($sqlCheckProject, array($activityFieldTypeId, $module21->name));
                                    if ($adb->num_rows($rsCheckProject) < 1) {
                                        $adb->pquery("INSERT INTO `vtiger_ws_referencetype` (`fieldtypeid`, `type`) VALUES (?, ?)", array($activityFieldTypeId, $module21->name));
                                    }
                                } else {
                                    if (version_compare($vtiger_current_version, "7.0.0", ">=")) {
                                        $module21->setRelatedList($module12, $module1, array($actions), "get_dependents_list", $fieldId);
                                        $maxId = $adb->pquery("SELECT max(relation_id) as relation_id FROM `vtiger_relatedlists` ", array());
                                        $adb->pquery("UPDATE `vtiger_relatedlists` SET `relationtype` = '1:N' WHERE `relation_id` = ?", array($adb->query_result($maxId, 0, "relation_id")));
                                    } else {
                                        $module21->setRelatedList($module12, $module1, array($actions), "get_dependents_list");
                                    }
                                }
                            }
                            if ($fieldId) {
    ...
    public function saveRelationship11(Vtiger_Request $request)
    {
        ...
        $initData = array($module1 => array("LBL_CUSTOM_INFORMATION" => array("cf_" . strtolower($module2) . "_id" => array("label" => $txtModule21, "table" => $moduleModel1->table_name, "uitype" => 10, "related_to_module" => $module2))), $module2 => array("LBL_CUSTOM_INFORMATION" => array("cf_" . strtolower($module1) . "_id" => array("label" => $txtModule12, "table" => $moduleModel2->table_name, "uitype" => 10, "related_to_module" => $module1))));
    ...
    public function saveRelationshipMM(Vtiger_Request $request)
    {
        ...
        try {
            ...
            $rs = $adb->pquery($sql, array($module12->getId(), $module21->getId(), "get_related_list"));
            if ($adb->num_rows($rs) == 0) {
                $module12->setRelatedList($module21, $txtModule21, array("ADD", "SELECT"), "get_related_list");
                $maxId = $adb->pquery("SELECT max(relation_id) as relation_id FROM `vtiger_relatedlists` ", array());
                $adb->pquery("UPDATE `vtiger_relatedlists` SET `relationtype` = 'N:N' WHERE `relation_id` = ?", array($adb->query_result($maxId, 0, "relation_id")));
                $module21->setRelatedList($module12, $txtModule12, array("ADD", "SELECT"), "get_related_list");
                $maxId = $adb->pquery("SELECT max(relation_id) as relation_id FROM `vtiger_relatedlists` ", array());
                $adb->pquery("UPDATE `vtiger_relatedlists` SET `relationtype` = 'N:N' WHERE `relation_id` = ?", array($adb->query_result($maxId, 0, "relation_id")));
        ...
        foreach ($relations as $rel) {
            ...
            if ($moduleModel) {
                if ($relationType == "1-1") {
                    ...
                    if ($fieldModel) {
                        $unsetModuleNames = array($rel["related_tab_name"]);
