...
class Project extends CRMEntity
{
     ...
     */
    public $tab_name = ['vtiger_crmentity', 'vtiger_project', 'vtiger_projectcf'];
    ...
    public function vtlib_handler($modulename, $event_type)
    {
        if ($event_type == 'module.postinstall') {
            ...
            $projectsResult = $adb->pquery('SELECT tabid FROM vtiger_tab WHERE name=?', ['Project']);
            ...
            if (getTabid('CustomerPortal') && $projectTabid) {
                $checkAlreadyExists = $adb->pquery('SELECT 1 FROM vtiger_customerportal_tabs WHERE tabid=?', [$projectTabid]);
                if ($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
                    ...
                    $adb->pquery('INSERT INTO vtiger_customerportal_tabs(tabid,visible,sequence) VALUES (?, ?, ?)', [$projectTabid, 1, $nextSequence]);
                    $adb->pquery('INSERT INTO vtiger_customerportal_prefs(tabid,prefkey,prefvalue) VALUES (?, ?, ?)', [$projectTabid, 'showrelatedinfo', 1]);
                }
            }
            ...
            $max_sequence = 0;
            $result = $adb->pquery('SELECT max(sequence) as maxsequence FROM vtiger_relatedlists WHERE tabid=?', [$projectTabid]);
            ...
            $adb->pquery(
                ...
                [$relation_id, $projectTabid, 0, 'get_gantt_chart', $sequence, 'Charts', 0],
            );

            // Add Project module to the related list of Accounts module
            $accountsModuleInstance = Vtiger_Module::getInstance('Accounts');
            $accountsModuleInstance->setRelatedList($moduleInstance, 'Projects', ['ADD', 'SELECT'], 'get_dependents_list');

            // Add Project module to the related list of Accounts module
            $contactsModuleInstance = Vtiger_Module::getInstance('Contacts');
            $contactsModuleInstance->setRelatedList($moduleInstance, 'Projects', ['ADD', 'SELECT'], 'get_dependents_list');

            // Add Project module to the related list of HelpDesk module
            $helpDeskModuleInstance = Vtiger_Module::getInstance('HelpDesk');
            $helpDeskModuleInstance->setRelatedList($moduleInstance, 'Projects', ['SELECT'], 'get_related_list');
            ...
            if (!$adb->num_rows($result)) {
                ...
                $adb->pquery('INSERT INTO vtiger_modentity_num values(?,?,?,?,?,?)', [$adb->getUniqueId('vtiger_modentity_num'), $modulename, 'PROJ', 1, 1, 1]);
            }
    ...
    public function get_gantt_chart($id, $cur_tab_id, $rel_tab_id, $actions = false)
    {
        ...
        $related_projecttasks = $adb->pquery(
            'SELECT pt.* FROM vtiger_projecttask AS pt
            ...
            [$record],
        ) or exit('Please install the ProjectMilestone and ProjectTasks modules first.');
        ...
        $related_projectmilestones = $adb->pquery(
            'SELECT pm.* FROM vtiger_projectmilestone AS pm
    ...
    public function transferRelatedRecords($module, $transferEntityIds, $entityId)
    {
        ...
        $rel_table_arr = ['ProjectTask' => 'vtiger_projecttask', 'ProjectMilestone' => 'vtiger_projectmilestone',
            'Documents' => 'vtiger_senotesrel', 'Attachments' => 'vtiger_seattachmentsrel'];

        $tbl_field_arr = ['vtiger_projecttask' => 'projecttaskid', 'vtiger_projectmilestone' => 'projectmilestoneid',
            'vtiger_senotesrel' => 'notesid', 'vtiger_seattachmentsrel' => 'attachmentsid'];

        $entity_tbl_field_arr = ['vtiger_projecttask' => 'projectid', 'vtiger_projectmilestone' => 'projectid',
    ...
    public function get_emails($recordId, $currentTabId, $relTabId, $actions = false)
    {
        $button = '';
