    ...
    public function handleEvent($eventName, $entityData)
    {
        ...
        if ($eventName == 'vtiger.entity.aftersave.final') {
            ...
            if ($moduleName == 'HelpDesk') {
                ...
                $adb->pquery('UPDATE vtiger_ticketcf SET from_portal=0 WHERE ticketid=?', [$ticketId]);
            }
        }
    }
}
...
function HelpDesk_nofifyOnPortalTicketCreation($entityData)
{
    ...
    $result = $adb->pquery("SELECT email, concat (firstname,' ',lastname) as name FROM vtiger_contactdetails WHERE contactid=?", [$parentId]);
...
function HelpDesk_notifyOnPortalTicketComment($entityData)
{
    ...
    // get the contact email id who creates the ticket from portal and use this email as from email id in email
    $result = $adb->pquery('SELECT lastname, firstname, email FROM vtiger_contactdetails WHERE contactid=?', [$parentId]);
...
function HelpDesk_notifyParentOnTicketChange($entityData)
{
    ...
    if (!empty($contactId[0])) {
        ...
        if ($parent_email != '') {
            ...
            $isPortalUser = $adb->query_result($adb->pquery($sql, [$contact_mailid]), 0, 'isactive');
        }
    } elseif (!empty($accountId[0])) {
        $result = $adb->pquery(
            'SELECT accountname, emailoptout, email1 FROM vtiger_account WHERE accountid=?',
            [$accountId[1]],
        );
        $emailoptout = $adb->query_result($result, 0, 'emailoptout');
