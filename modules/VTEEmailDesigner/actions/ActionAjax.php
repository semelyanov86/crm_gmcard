...
class VTEEmailDesigner_ActionAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function enableModule(Vtiger_Request $request)
    {
        ...
        $adb->pquery("UPDATE `vte_custom_header_settings` SET `active`=?", array($value));
    ...
    {
        global $adb;
        $rs = $adb->pquery("SELECT `enable` FROM `vte_custom_header_settings`;", array());
    ...
    public function doDeleteTemplate(Vtiger_Request $request)
    {
        ...
        $adb->pquery("DELETE FROM vtiger_emailtemplates WHERE templateid=?", array($templateid));
        $adb->pquery("DELETE FROM vteemaildesigner_template_blocks WHERE templateid=?", array($templateid));
        $response = new Vtiger_Response();
        $response->setResult("success");
        $response->emit();
    }
    public function loadTemplates(Vtiger_Request $request)
    {
        ...
        $data["files"] = $rows;
        $response = new Vtiger_Response();
        $response->setResult(array("response" => $data));
        $response->emit();
    }
    ...
    public function doSaveTemplates(Vtiger_Request $request)
    {
        ...
        if (!empty($tempId)) {
            $adb->pquery("UPDATE `vtiger_emailtemplates` SET `templatename`=?, `subject`=?, description=?, `module`=?, `templatepath`=?, bg_color=?, email_width=?, bg_color_inner=? WHERE templateid=?", array($name, $subject, $description, $module, $thumbnail, $bg_color, $email_width, $bg_color_inner, $tempId));
            $adb->pquery("DELETE FROM vteemaildesigner_template_blocks WHERE templateid=?", array($tempId));
        } else {
            $tempId = $adb->getUniqueID("vtiger_emailtemplates");
            $adb->pquery("INSERT INTO  `vtiger_emailtemplates` (`templateid`,`templatename`, `subject`, description, `module`,`templatepath`, bg_color, email_width, bg_color_inner) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?)", array($tempId, $name, $subject, $description, $module, "", $bg_color, $email_width, $bg_color_inner));
        }
        ...
        for ($i = 0; $i < sizeof($contentArr); $i++) {
            if (isset($contentArr[$i]["id"])) {
                ...
                $adb->pquery("INSERT INTO  `vteemaildesigner_template_blocks` (`templateid`,`blockid`,`content`) VALUES (?, ?, ?)", array($tempId, $contentArr[$i]["id"], $contentArr[$i]["content"]));
            }
        }
        if ($bg_color && $bg_color != "") {
            $body .= "</td></tr></tbody></table>";
        }
        $adb->pquery("UPDATE `vtiger_emailtemplates` SET body=? WHERE templateid=?", array($body, $tempId));
        ...
        if ($isTableExist) {
            $rs_check = $adb->pquery("SELECT count(*) as count_data FROM vtiger_vteemailmarketing_emailtemplate WHERE idtemplate=?", array($tempId));
            if ($adb->query_result($rs_check, 0, "count_data") == 0) {
                $adb->pquery("INSERT INTO vtiger_vteemailmarketing_emailtemplate (idtemplate) VALUES (?)", array($tempId));
            }
        }
        $response = new Vtiger_Response();
        $response->setResult(array("templateid" => $tempId));
        $response->emit();
    }
    ...
    public function uploadImageLocal(Vtiger_Request $request)
    {
        ...
        $response = new Vtiger_Response();
        $response->setResult(array("data_arr" => $data_arr));
        $response->emit();
    }
}
