...
class VTEEmailDesigner extends CRMEntity
{
    ...
    public static function addWidgetTo()
    {
        ...
        $rs = $adb->pquery("SELECT * FROM `vtiger_settings_field` WHERE `name` = ?", array("Email Designer"));
        if ($adb->num_rows($rs) == 0) {
            $max_id = $adb->getUniqueID("vtiger_settings_field");
            $adb->pquery("INSERT INTO `vtiger_settings_field` (`fieldid`, `blockid`, `name`, `description`, `linkto`, `sequence`) VALUES (?, ?, ?, ?, ?, ?)", array($max_id, "4", "Email Designer", "Settings area for Email Designer", "index.php?module=VTEEmailDesigner&view=List", $max_id));
        }
        $rs = $adb->pquery("SELECT * FROM `vtiger_ws_entity` WHERE `name` = ?", array("VTEEmailDesigner"));
        if ($adb->num_rows($rs) == 0) {
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`name`, `handler_path`, `handler_class`, `ismodule`)            VALUES (?, 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1');", array("VTEEmailDesigner"));
        }
    }
    public static function removeWidgetTo()
    {
        ...
        global $vtiger_current_version;
        include_once "vtlib/Vtiger/Module.php";
        $adb->pquery("DELETE FROM vtiger_settings_field WHERE `name` = ?", array("Email Designer"));
        $adb->pquery("DELETE FROM vtiger_ws_entity WHERE `name` = ?", array("VTEEmailDesigner"));
    }
    public static function removeValid()
    {
        global $adb;
        $adb->pquery("DELETE FROM `vte_modules` WHERE module=?;", array("VTEEmailDesigner"));
    }
    public static function resetValid()
    {
        global $adb;
        $adb->pquery("DELETE FROM `vte_modules` WHERE module=?;", array("VTEEmailDesigner"));
        $adb->pquery("INSERT INTO `vte_modules` (`module`, `valid`) VALUES (?, ?);", array("VTEEmailDesigner", "0"));
    }
    public static function defaultData()
    {
        ...
        while ($row = $adb->fetch_row($result)) {
            ...
            $adb->pquery("INSERT INTO `vteemaildesigner_template_blocks` (`templateid`, `blockid`, `content`) VALUES (?, ?, ?)", array($templateid, "3", $body));
        }
    }
    public static function updateEmailDesignerBlocks()
    {
        ...
        global $root_directory;
        global $site_URL;
        $adb->pquery("DELETE FROM vteemaildesigner_blocks", array());
        $adb->pquery("ALTER TABLE vteemaildesigner_blocks CONVERT TO CHARACTER SET utf8 COLLATE utf8_unicode_ci;", array());
        ...
        $adb->pquery($sql_vteemaildesigner_blocks, array());
    }
    ...
    public static function addFields()
    {
        ...
        if (!self::checkColumnExist("vtiger_emailtemplates", "bg_color")) {
            $adb->pquery("ALTER TABLE `vtiger_emailtemplates` ADD COLUMN `bg_color` VARCHAR(50) NULL ", array());
        }
        if (!self::checkColumnExist("vtiger_emailtemplates", "email_width")) {
            $adb->pquery("ALTER TABLE `vtiger_emailtemplates` ADD COLUMN `email_width` VARCHAR(50) NULL ", array());
        }
        if (!self::checkColumnExist("vtiger_emailtemplates", "bg_color_inner")) {
            $adb->pquery("ALTER TABLE `vtiger_emailtemplates` ADD COLUMN `bg_color_inner` VARCHAR(50) NULL ", array());
        }
    }
}
