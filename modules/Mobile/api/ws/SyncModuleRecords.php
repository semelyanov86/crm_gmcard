...
{
    public static $SYNC_MODE_PUBLIC  = 'PUBLIC';

    public static $SYNC_MODE_PRIVATE = 'PRIVATE';
    ...
    public function process(Mobile_API_Request $request)
    {
        ...
        $resolvedModifiedRecords = [];
        $resolvedDeletedRecords = [];
        ...
        if (!empty($lastSyncTime) && $currentPage === 0) {
            ...
            $queryDeletedParameters = [$module, date('Y-m-d H:i:s', $lastSyncTime)];
            $andsmowneridequal = '';
            ...
            if ($module == 'Calendar') {
                $queryDeleted = $adb->pquery(
                    "SELECT activityid as crmid, activitytype as setype FROM vtiger_activity
        ...
        $response->setResult(['sync' => $result]);
