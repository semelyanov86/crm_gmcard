...
class VTEExportToXLS_ExportData_Action extends Vtiger_Mass_Action
{
    ...
    public function outputxls($request, $headers, $entries, $listViewHeaders_label)
    {
        ...
        $list_column_name = "";
        $rs = $adb->pquery("SELECT `download_to_server`,custom_filename, file_name FROM `vteexport_to_xls_settings`;", array());
        ...
        $count = 0;
        $cell_name = "";
        $styleArray = array("font" => array("bold" => true, "size" => 11, "color" => array("rgb" => "FFFFFF")), "fill" => array("type" => PHPExcel_Style_Fill::FILL_SOLID, "color" => array("rgb" => "606060")));
        $show_columns = array();
                ...
                foreach ($listViewHeaders_label as $key => $tittle) {
                    if (in_array($key, array_keys($line))) {
                        ...
                        if (strpos($key, ";") !== false) {
                            ...
                            list($rel_fieldName, $parentModule) = $fieldName;
                            ...
                            if (0 < $parentRecord) {
                                ...
                                $re = $adb->pquery($sql1_check, array($parentRecord));
                                if (0 < $adb->num_rows($re)) {
                                    ...
                                    if ($uitype == 72 || $uitype == 71 || $uitype == 7 && $fieldInfo->get("typeofdata") == "N~O" || $uitype == 9) {
                                        ...
                                        if ($uitype == 10 || $fieldInfo->getFieldDataType() == "reference") {
                                            if (0 < $parentFieldValue) {
                                                $sql = "SELECT * FROM vtiger_crmentity WHERE crmid = ? AND deleted = 0;";
                                                $re = $adb->pquery($sql, array($parentFieldValue));
                                                if (0 < $adb->num_rows($re)) {
