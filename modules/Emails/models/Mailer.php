    ...
    public static function updateMessageIdByCrmId($messageId, $crmId)
    {
        ...
        if ($num_rows > 0) {
            ...
            if (is_array($existingResult)) {
                ...
                $db->pquery('UPDATE vtiger_mailscanner_ids SET refids=? WHERE crmid=? ', [$refIds, $crmId]);
            }
        } else {
            $db->pquery('INSERT INTO vtiger_mailscanner_ids (messageid, crmid) VALUES(?,?)', [$messageId, $crmId]);
        }
    }
