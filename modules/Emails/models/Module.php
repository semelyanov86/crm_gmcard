...
class Emails_Module_Model extends Vtiger_Module_Model
{
    ...
    public function searchEmails($searchValue, $moduleName = false)
    {
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $emailsResult[vtranslate($row['setype'], $row['setype'])][$row['crmid']][] = ['value' => $row['value'],
    ...
    public function getEmailOptOutRecordIds()
    {
        $emailOptOutIds = [];
        $db = PearDatabase::getInstance();
        $contactResult = $db->pquery('SELECT crmid FROM vtiger_crmentity INNER JOIN vtiger_contactdetails ON vtiger_contactdetails.contactid = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted = ? AND vtiger_contactdetails.emailoptout = ?', ['0', '1']);
        $contactCount = $db->num_rows($contactResult);
        for ($i = 0; $i < $contactCount; ++$i) {
            $emailOptOutIds[] = $db->query_result($contactResult, $i, 'crmid');
        }
        $accountResult = $db->pquery('SELECT crmid FROM vtiger_crmentity INNER JOIN vtiger_account ON vtiger_account.accountid = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted = ? AND vtiger_account.emailoptout = ?', ['0', '1']);
        $accountCount = $db->num_rows($accountResult);
        for ($i = 0; $i < $accountCount; ++$i) {
            $emailOptOutIds[] = $db->query_result($accountResult, $i, 'crmid');
        }
        $leadResult = $db->pquery('SELECT crmid FROM vtiger_crmentity INNER JOIN vtiger_leaddetails ON vtiger_leaddetails.leadid = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted = ? AND vtiger_leaddetails.emailoptout = ?', ['0', '1']);
