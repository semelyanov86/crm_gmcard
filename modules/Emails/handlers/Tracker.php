...
class Emails_Tracker_Handler
{
    ...
    protected function clickHandler($data = [])
    {
        ...
        if ($parentId && $recordId) {
             ...
             */
            $result = $db->pquery('SELECT 1 FROM vtiger_email_access WHERE crmid = ? AND mailid = ? ', [$parentId, $recordId]);
            if (!$db->num_rows($result)) {
                $this->openHandler(['record' => $recordId, 'parentId' => $parentId]);
            }
    ...
    protected function openHandler($data = [])
    {
        $recordId = $data['record'];
        $parentId = $data['parentId'];
