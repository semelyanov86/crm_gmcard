        ...
        if ($recordModel->checkUploadSize($documentIds)) {
            ...
            if (is_array($existingAttachments)) {
                foreach ($existingAttachments as $index =>  $existingAttachInfo) {
                    ...
                    $params1 = [$current_id, $current_user->getId(), $ownerId, $moduleName . ' Attachment', $recordModel->get('description'), $adb->formatDate($date_var, true), $adb->formatDate($date_var, true)];
                    ...
                    $adb->pquery($sql3, [$recordModel->getId(), $current_id]);
                }
            }
            $success = true;
            $message = '';
