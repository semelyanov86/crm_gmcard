...
class EmailTemplate
{
    ...
    protected $multiRefValues = [];

    protected $multiRefIds = [];
    ...
    public function setDescription($description)
    {
        ...
        if ($result != 0) {
            ...
            for ($i = 0; $i < php7_count($templateVariablePair); ++$i) {
                ...
                [$module, $columnName] = explode('-', $templateVariablePair[$i]);
                if (isset($columnName) && strpos($columnName, ':') !== false) {
                    [$parentColumn, $childColumn] = explode(':', $columnName);
...
s								 */
                        ...
                        } elseif ($this->module == 'Events' && $fieldModel->getFieldDataType() == 'multireference') {
                    ...
                    if (isset($values[$fieldName])
                        ...
                        if (strcasecmp($webserviceField->getFieldDataType(), 'reference') === 0) {
                            ...
                            if (!$this->isProcessingReferenceField($params) && !empty($values[$fieldName])) {
                                $this->process(['parentMeta' => $meta, 'referencedMeta' => $referencedObjectMeta, 'field' => $fieldName, 'id' => $values[$fieldName]]);
                            }
                            $values[$fieldName]
                            ...
                            */
                            if (in_array($fieldColumnMapping[$fieldName], array_keys($this->referencedFields))) {
                                $this->process(['parentMeta' => $meta, 'referencedMeta' => $referencedObjectMeta, 'field' => $fieldName, 'id' => $values[$fieldName], 'owner' => true]);
                            }
                ...
                if (!$this->isProcessingReferenceField($params)) {
                    foreach ($columnList as $column) {
                        ...
                        $this->processedDescription = str_replace(
                            $needle,
                            $replaceValue,
