...
function sendPrdStckMail($product_id, $upd_qty, $prod_name, $qtyinstk, $qty, $module)
{
    ...
    if ($upd_qty < $reorderlevel) {
        ...
        $result = $adb->pquery($query, [$notification_table]);
...
function getTaxId($type)
{
    ...
    $res = $adb->pquery('SELECT taxid FROM vtiger_inventorytaxinfo WHERE taxname=?', [$type]);
...
function getTaxPercentage($type)
{
    ...
    $res = $adb->pquery('SELECT percentage FROM vtiger_inventorytaxinfo WHERE taxname = ?', [$type]);
...
function getProductTaxPercentage($type, $productid, $default = '')
{
    ...
    $res = $adb->pquery('SELECT taxpercentage, vtiger_producttaxrel.regions FROM vtiger_inventorytaxinfo
							...
							WHERE vtiger_producttaxrel.productid = ? AND vtiger_inventorytaxinfo.taxname = ?', [$productid, $type]);
    ...
    return ['percentage' => $taxpercentage, 'regions' => Zend_Json::decode(html_entity_decode(!empty($regions) ? $regions : ''))];
}
...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
    ...
    $qparams = [$histid, $id, $relatedname, $total, $history_fldval, $modifiedtime];
    ...
    if ($sh != '' && $sh == 'sh') {
        ...
        if ($mode == 'edit' && $id != '') {
            ...
            for ($i = 0; $i < $noofrows; ++$i) {
                ...
                $inventory_tax_val_result	= $adb->pquery("SELECT {$taxname} FROM {$value_table} WHERE id=?", [$id]);
            ...
            $result = $adb->pquery("SELECT * FROM {$tablename} {$where} ORDER BY deleted", []);
            $noofrows = $adb->num_rows($result);
            for ($i = 0; $i < $noofrows; ++$i) {
                $taxtypes[$i]['taxid']		= $adb->query_result($result, $i, 'taxid');
        ...
        if ($mode == 'edit' && $id != '') {
            ...
            $noofrows = $adb->num_rows($result);
            $inventory_tax_val_result = $adb->pquery("select * from {$value_table} where id=?", [$id]);
...
function deleteInventoryProductDetails($focus)
{
    ...
    $product_info = $adb->pquery('SELECT productid, quantity, sequence_no, incrementondel from vtiger_inventoryproductrel WHERE id=?', [$focus->id]);
    ...
    for ($index = 0; $index < $numrows; ++$index) {
        ...
        if ($incrementondel) {
            ...
            $subProdQuery = $adb->pquery('SELECT productid, quantity from vtiger_inventorysubproductrel WHERE id=? AND sequence_no=?', [$focus->id, $sequence_no]);
    ...
    $focus->update_product_array ??= ''; // to avoid undefined property warning.
    $updateInventoryProductRel_update_product_array = $focus->update_product_array;
    $adb->pquery('delete from vtiger_inventoryproductrel where id=?', [$focus->id]);
    $adb->pquery('delete from vtiger_inventorysubproductrel where id=?', [$focus->id]);
    $adb->pquery('delete from vtiger_inventoryshippingrel where id=?', [$focus->id]);
...
function updateInventoryProductRel($entity)
{
    ...
    if ($statusChanged && $moduleName === 'Invoice') {
        if ($recordDetails[$statusFieldName] == $statusFieldValue) {
            $adb->pquery('UPDATE vtiger_inventoryproductrel SET incrementondel=0 WHERE id=?', [$entity_id]);
    ...
    if ($updateInventoryProductRel_deduct_stock) {
        $adb->pquery('UPDATE vtiger_inventoryproductrel SET incrementondel=1 WHERE id=?', [$entity_id]);

        $product_info = $adb->pquery('SELECT productid,sequence_no, quantity from vtiger_inventoryproductrel WHERE id=?', [$entity_id]);
        $numrows = $adb->num_rows($product_info);
        for ($index = 0; $index < $numrows; ++$index) {
            $productid = $adb->query_result($product_info, $index, 'productid');
            $qty = $adb->query_result($product_info, $index, 'quantity');
            ...
            $sub_prod_query = $adb->pquery('SELECT productid, quantity from vtiger_inventorysubproductrel WHERE id=? AND sequence_no=?', [$entity_id, $sequence_no]);
    ...
    if ($moduleName === 'PurchaseOrder') {
        ...
        if ($statusFieldValue === 'Cancelled') {
            ...
            if ($statusChanged && $itemQuantitiesList) {
                ...
                $result = $db->pquery($query, [$entity_id, $statusFieldName]);
                ...
                for ($i = 1; $i <= $cancelledKey; ++$i) {
                    if ($statusStack[$i] === 'Received Shipment') {
                        ...
                        $deletedItems = ($itemQuantitiesList['deleted']) ? $itemQuantitiesList['deleted'] : [];
            ...
            if ($itemQuantitiesList && ($isUpdateNeeded || $isItemsRequest)) {
                ...
                $newItems = ($itemQuantitiesList['new']) ? $itemQuantitiesList['new'] : [];
                ...
                $updatedItems = ($itemQuantitiesList['updated']) ? $itemQuantitiesList['updated'] : [];
                foreach ($updatedItems as $productId => $quantity) {
                    addToProductStock($productId, $quantity);
                }
            } elseif ($isUpdateNeeded) {
                addProductsToStock($entity_id);
            }
        }
    }
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '')
{
    ...
    for ($i = 1; $i <= $tot_no_prod; ++$i) {
        ...
        $query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description, purchase_cost, margin)
					VALUES(?,?,?,?,?,?,?,?,?)';
        $qparams = [$focus->id, $prod_id, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost, $margin];
        $adb->pquery($query, $qparams);
        ...
        if (!empty($sub_prod_str)) {
            ...
            foreach ($sub_prod as $subProductInfo) {
                ...
                $qparams = [$focus->id, $prod_seq, $subProductId, $subProductQty];
                $adb->pquery($query, $qparams);
            }
        }
        ++$prod_seq;
    ...
    $adb->pquery('DELETE FROM vtiger_inventorychargesrel WHERE recordid = ?', [$focus->id]);
    $chargesInfo = [];
    if (isset($_REQUEST['charges'])) {
        $chargesInfo = $_REQUEST['charges'];
    }
    $adb->pquery('INSERT INTO vtiger_inventorychargesrel VALUES (?, ?)', [$focus->id, Zend_Json::encode($chargesInfo)]);
...
function getInventoryTaxType($module, $id)
{
    ...
    $res = $adb->pquery("SELECT taxtype FROM {$inventoryTable} WHERE {$inventoryId}=?", [$id]);
...
function getInventorySHTaxPercent($id, $taxname, $taxnum = null)
{
    ...
    $taxname = $taxname;
    $res = $adb->pquery("select {$taxname} from vtiger_inventoryshippingrel where id= ?", [$id]);
...
function getBaseConversionRateForProduct($productid, $mode = 'edit', $module = 'Products')
{
    ...
    if ($mode == 'edit') {
        ...
        $params = [fetchCurrency($current_user->id)];
    }
...
{
    global $adb;
    $result = $adb->pquery('select currency_id from vtiger_pricebook where pricebookid=?', [$pricebook_id]);
...
{
    global $adb;
    $adb->pquery('UPDATE vtiger_inventoryproductrel SET incrementondel=1 WHERE id=?', [$recordId]);

    $product_info = $adb->pquery('SELECT productid,sequence_no, quantity from vtiger_inventoryproductrel WHERE id=?', [$recordId]);
    $numrows = $adb->num_rows($product_info);
    for ($index = 0; $index < $numrows; ++$index) {
        $productid = $adb->query_result($product_info, $index, 'productid');
        $qty = $adb->query_result($product_info, $index, 'quantity');
        ...
        $upd_qty = $qtyinstk - $qty;
        updateProductQty($productid, $upd_qty);
        $sub_prod_query = $adb->pquery('SELECT productid, quantity FROM vtiger_inventorysubproductrel WHERE id=? AND sequence_no=?', [$recordId, $sequence_no]);
        if ($adb->num_rows($sub_prod_query) > 0) {
            for ($j = 0; $j < $adb->num_rows($sub_prod_query); ++$j) {
                $sub_prod_id = $adb->query_result($sub_prod_query, $j, 'productid');
                $subProductQty = $adb->query_result($sub_prod_query, $j, 'quantity');
...
function createRecords($obj)
{
    ...
    for ($i = 0; $i < $numberOfRecords; ++$i) {
        ...
        if ($entityInfo == null) {
            $entityInfo = ['id' => null, 'status' => $obj->getImportRecordStatus('failed')];
        } else {
...
function getImportStatusCount($obj)
{
    global $adb;
    $tableName = Import_Utils_Helper::getDbTableName($obj->user);
    $result = $adb->pquery('SELECT status FROM ' . $tableName . ' GROUP BY subject', []);
...
function undoLastImport($obj, $user)
{
    ...
    $result = $adb->pquery("SELECT recordid FROM {$dbTableName} WHERE status = ? AND recordid IS NOT NULL GROUP BY subject", [Import_Data_Controller::$IMPORT_RECORD_CREATED]);
...
function getLineItemFields()
{
    ...
    $result = $adb->pquery($sql, ['vtiger_inventoryproductrel']);
    $lineItemdFields = [];
...
function getInventoryImportableMandatoryFeilds($module)
{
    ...
    $defaultMandatoryFields = ['quantity', 'listprice'];
...
function getAllCharges()
{
    ...
    $result = $db->pquery('SELECT * FROM vtiger_inventorycharges WHERE deleted = 0', []);
...
function getAllRegions()
{
    ...
    $result = $db->pquery('SELECT * FROM vtiger_taxregions', []);

    while ($rowData = $db->fetch_array($result)) {
        $allRegionsInfo[$rowData['regionid']] = ['id' => $rowData['regionid'], 'name' => $rowData['name']];
    }
...
function getCompoundTaxesInfoForInventoryRecord($recordId, $moduleName)
{
    ...
    $tableName = '';
    switch ($moduleName) {
        case 'Quotes': $tableName = 'vtiger_quotes';
            $index = 'quoteid';
            break;
        case 'Invoice': $tableName = 'vtiger_invoice';
            $index = 'invoiceid';
            break;
        case 'SalesOrder': $tableName = 'vtiger_salesorder';
            $index = 'salesorderid';
            break;
        case 'PurchaseOrder': $tableName = 'vtiger_purchaseorder';
    ...
    if ($recordId && $tableName) {
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT compound_taxes_info FROM {$tableName} WHERE {$index} = ?", [$recordId]);
