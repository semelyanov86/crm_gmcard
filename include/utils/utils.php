...
function getActionid($action)
{
    ...
    if (file_exists('tabdata.php') && (filesize('tabdata.php') != 0)) {
        include 'tabdata.php';
        $actionid = $action_id_array[$action] ?? 0;
    } else {
...
function relateEntities($focus, $sourceModule, $sourceRecordId, $destinationModule, $destinationRecordIds)
{
    ...
    $data = ['sourceModule' => $sourceModule, 'sourceRecordId' => $sourceRecordId,
 ...
 * @return bool $status - true if column exists; false otherwise
 */
...
function dateDiff($d1, $d2)
{
    ...
    return [
        ...
        'days_total' => floor($diffSecs / (3_600 * 24)),
        'days' => date('j', $diff) - 1,
        'hours_total' => floor($diffSecs / 3_600),
        'hours' => date('G', $diff),
...
function getLastWeekRange($date)
{
    ...
    $lastWeekRange = ['start' => $lastSunday, 'end' => $lastSaturday];
...
function getCurrentWeekRange($date)
{
    ...
    $currentWeekRange = ['start' => $thissunday, 'end' => $thisSaturday];
    ...
    foreach ($fieldModels as $fieldName => $fieldModel) {
        if ($fieldModel->isUniqueField() && $fieldModel->isViewable()) {
            ...
            switch ($fieldModel->getFieldDataType()) {
                case 'reference':	[$refModuleId, $refRecordId] = vtws_getIdComponents($fieldValue);
