...
function export($type)
{
    ...
    [$idstring, $export_data] = explode('#@@#', getExportRecordIds($type, $viewid, $_REQUEST));
    ...
    $fields_array = array_diff($fields_array, ['user_name']);
...
class ExportUtils
{
     ...
     */
    public static function getInformationArray($module)
    {
        require_once 'include/utils/utils.php';
        global $adb;
        $tabid = getTabid($module);

        $result = $adb->pquery('select * from vtiger_field where tabid=?', [$tabid]);
