...
class PopulateComboValues
{
    ...
    public function insertComboValues($values, $tableName, $picklistid)
    {
        ...
        foreach ($values as $val => $cal) {
            ...
            if ($val != '') {
                $params = [$id, $val, 1, $picklist_valueid, $i];
                $adb->pquery("insert into vtiger_{$tableName} values(?,?,?,?,?)", $params);
            } else {
                $params = [$id, '--None--', 1, $picklist_valueid, $i];
                $adb->pquery("insert into vtiger_{$tableName} values(?,?,?,?,?)", $params);
            }
            ...
            for ($k = 0; $k < $numrow; ++$k) {
                ...
                $params = [$roleid, $picklist_valueid, $picklistid, $i];
    ...
    public function create_tables()
    {
        ...
        $noneditable_tables = ['ticketstatus', 'taskstatus', 'eventstatus', 'faqstatus', 'quotestage', 'postatus', 'sostatus', 'invoicestatus', 'activitytype'];
        $noneditable_values = [
    ...
    public function create_nonpicklist_tables()
    {
        ...
        $comboRes = $adb->pquery("SELECT distinct fieldname FROM vtiger_field WHERE uitype IN ('16','115','83') AND fieldname NOT IN ('hdnTaxType','email_flag') and vtiger_field.presence in (0,2)", []);
        $noOfCombos = $adb->num_rows($comboRes);
        for ($i = 0; $i < $noOfCombos; ++$i) {
            $comTab = $adb->query_result($comboRes, $i, 'fieldname');
