...
function expandRecord($table, $log)
{
    ...
    if ($table == 'vtiger_products') {
        $subfields =  ['productid', 'productname', 'productcode', 'productcategory', 'manufacturer', 'qty_per_unit', 'unit_price', 'weight', 'pack_size', 'sales_start_date', 'sales_end_date', 'start_date', 'expiry_date', 'cost_factor', 'commissionrate', 'commissionmethod', 'discontinued', 'usageunit', 'currency', 'reorderlevel', 'website', 'taxclass', 'mfr_part_no', 'vendor_part_no', 'serialno', 'qtyinstock', 'productsheet', 'qtyindemand', 'glacct', 'vendor_id', 'imagename'];
    }
    ...
    elseif ($table == 'vtiger_activity') {
        $subfields =  ['activityid', 'subject', 'semodule', 'activitytype', 'date_start', 'due_date', 'time_start', 'time_end', 'sendnotification', 'duration_hours', 'duration_minutes', 'status', 'eventstatus', 'priority', 'location', 'notime', 'visibility', 'recurringtype'];
    }

    // vtiger_notes table
    elseif ($table == 'vtiger_notes') {
        $subfields =  ['notesid', 'contact_id', 'title', 'filename', 'notecontent'];
    }

    // vtiger_faq table
    elseif ($table == 'vtiger_faq') {
        $subfields =  ['id', 'product_id', 'question', 'answer', 'category', 'status'];
    }
    ...
    elseif ($table == 'vtiger_field') {
        $subfields =  ['tabid', 'fieldid', 'columnname', 'tablename', 'generatedtype', 'uitype', 'fieldname', 'fieldlabel', 'readonly', 'presence', 'selected', 'maximumlength', 'sequence', 'block', 'displaytype', 'typeofdata', 'quickcreate', 'quickcreatesequence', 'info_type'];
    }
    ...
    elseif ($table == 'vtiger_contactdetails') {
        $subfields = ['lastname', 'contactid', 'accountid', 'salutation', 'firstname', 'email', 'phone', 'mobile', 'title', 'department', 'fax', 'reportsto', 'training', 'usertype', 'contacttype', 'otheremail', 'yahooid', 'donotcall', 'emailoptout', 'imagename', 'reference', 'notify_owner'];
    }

    // vtiger_quotes
    elseif ($table == 'vtiger_quotes') {
        $subfields = ['quoteid', 'subject', 'potentialid', 'quotestage', 'validtill', 'contactid', 'currency', 'subtotal', 'carrier', 'shipping', 'inventorymanager', 'type', 'adjustment', 'total', 'taxtype', 'discount_percent', 'discount_amount', 's_h_amount', 'accountid', 'terms_conditions'];
    }

    // vtiger_crmentity
    elseif ($table == 'vtiger_crmentity') {
        $subfields = ['crmid', 'smcreatorid', 'smownerid', 'modifiedby', 'setype', 'description', 'createdtime', 'modifiedtime', 'viewedtime', 'status', 'version', 'presence', 'deleted'];
    }

    // vtiger_salesorder
    elseif ($table == 'vtiger_salesorder') {
        $subfields = ['salesorderid', 'subject', 'potentialid', 'customerno', 'quoteid', 'vendorterms', 'contactid', 'vendorid', 'duedate', 'carrier', 'pending', 'type', 'adjustment', 'salescommission', 'exciseduty', 'total', 'subtotal', 'taxtype', 'discount_percent', 'discount_amount', 's_h_amount', 'accountid', 'terms_conditions', 'purchaseorder', 'sostatus'];
    }

    // vtiger_invoice
    elseif ($table == 'vtiger_invoice') {
        $subfields = ['invoiceid', 'subject', 'salesorderid', 'customerno', 'contactid', 'notes', 'invoicedate', 'duedate', 'invoiceterms', 'type', 'adjustment', 'salescommission', 'exciseduty', 'subtotal', 'total', 'taxtype', 'discount_percent', 'discount_amount', 's_h_amount', 'shipping', 'accountid', 'terms_conditions', 'purchaseorder', 'invoicestatus', 'invoice_no'];
    }

    // vtiger_seactivityrel
    elseif ($table == 'vtiger_seactivityrel') {
        $subfields = ['crmid', 'activityid'];
    }

    // vtiger_cntactivityrel
    elseif ($table == 'vtiger_cntactivityrel') {
        $subfields = ['contactid', 'activityid'];
    }

    // vtiger_purchaseorder
    elseif ($table == 'vtiger_purchaseorder') {
        $subfields = ['purchaseorderid', 'subject', 'quoteid', 'vendorid', 'requisition_no', 'tracking_no', 'contactid', 'duedate', 'carrier', 'type', 'adjustment', 'salescommission', 'exciseduty', 'total', 'subtotal', 'taxtype', 'discount_percent', 'discount_amount', 's_h_amount', 'terms_conditions', 'postatus'];
    }

    // vtiger_leaddetails
    elseif ($table == 'vtiger_leaddetails') {
        $subfields = ['leadid', 'email', 'interest', 'firstname', 'salutation', 'lastname', 'company', 'annualrevenue', 'industry', 'campaign', 'rating', 'leadstatus', 'leadsource', 'converted', 'designation', 'space', 'comments', 'priority', 'demorequest', 'partnercontact', 'productversion', 'product', 'maildate', 'nextstepdate', 'fundingsituation', 'purpose', 'evaluationstatus', 'transferdate', 'revenuetype', 'noofemployees', 'yahooid', 'assignleadchk'];
    }
