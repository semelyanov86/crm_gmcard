...
class ChartUtils
{
    ...
    public static function getBarChart($xaxisData, $yaxisData, $title = '', $width = '', $height = '', $charttype = 'vertical', $cachedFileName = false, $target = false, $color = '')
    {
        ...
        $canvas = & Image_Canvas::factory('png', ['width' => $width, 'height' => $height, 'usemap' => true]);
        ...
        $titlestr = & Image_Graph::factory('title', [$title, 8]);
        ...
        if ($charttype == 'horizontal') {
            $fill = & Image_Graph::factory('gradient', [IMAGE_GRAPH_GRAD_VERTICAL_MIRRORED, $color, 'white']);
        } else {
            $fill = & Image_Graph::factory('gradient', [IMAGE_GRAPH_GRAD_HORIZONTAL_MIRRORED, $color, 'white']);
        }
        ...
        $bplot->setBackground(Image_Graph::factory('gradient', [IMAGE_GRAPH_GRAD_HORIZONTAL, 'white', 'white']));
        ...
        $array_data = & Image_Graph::factory(
        ...
        if ($width > 400 && $height > 400) {
            ...
            if ($charttype == 'horizontal') {
                $marker_pointing = & $graph->addNew('Image_Graph_Marker_Pointing', [10, 0, & $marker]);
            } else {
    ...
    public static function getPieChart($xaxisData, $yaxisData, $title = '', $width = '', $height = '', $charttype = 'vertical', $cachedFileName = false, $target = false, $color = '')
    {
        ...
        $title = & Image_Graph::factory('title', [$title, 10]);
        $plotarea = & Image_Graph::factory('plotarea', [
            'category',
            'axis',
        ]);
        ...
        $plotarea->setPadding(['top' => 0, 'bottom' => 0, 'left' => 0, 'right' => ($width / 20)]);
        $plotarea->hideAxis();
        $gbplot->setFillStyle($fills);
        // format the data values
        $marker_array = & Image_Graph::factory('Image_Graph_DataPreprocessor_Array', [$pcvalues]);
        // set markers
        $marker = & $graph->addNew('value_marker', IMAGE_GRAPH_VALUE_Y);
        ...
        $marker->setFontSize(8);
        $pointingMarker = & $graph->addNew('Image_Graph_Marker_Pointing_Angular', [20, &$marker]);
        $gbplot->setMarker($pointingMarker);
        $legend_box = & $plotarea->addNew('legend');
        $legend_box->setPadding(['top' => 20, 'bottom' => 0, 'left' => 0, 'right' => 0]);
    ...
    public static function generateChartDataFromReports($queryResult, $groupbyField, $fieldDetails = '', $reportid = '')
    {
        ...
        $inventorymodules = ['Quotes', 'SalesOrder', 'PurchaseOrder', 'Invoice', 'Products', 'PriceBooks', 'Vendors', 'Services'];
