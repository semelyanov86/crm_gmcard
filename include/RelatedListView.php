...
function getPriceBookRelatedProducts($query, $focus, $returnset = '')
{
    ...
    for ($i = 0; $i < $numRows; ++$i) {
        ...
        if ($currency_id != null) {
            $prod_prices = getPricesForProducts($currency_id, [$entity_id]);
...
function CheckColumnPermission($tablename, $columnname, $module)
{
    ...
    if (!array_key_exists($cachekey, $cache)) {
        $res = $adb->pquery('select fieldname from vtiger_field where tablename=? and columnname=? and vtiger_field.presence in (0,2)', [$tablename, $columnname]);
