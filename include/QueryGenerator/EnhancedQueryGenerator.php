        ...
        foreach ($advFilterList as $groupindex => $groupcolumns) {
            ...
            if (php7_count($filtercolumns) > 0) {
                ...
                foreach ($filtercolumns as $index => $filter) {
                    ...
                    if (!in_array($filter['comparator'], ['e', 'n'])) {
    ...
    public function getWhereClause()
    {
        ...
        foreach ($this->conditionals as $index => $conditionInfo) {
            ...
            if ($operator == 'between' && $this->isDateType($field->getFieldDataType())) {
                ...
                if (php7_count($end) == 2) {
                    if ($start[0] == $end[0]) {
                        $dateTime = new DateTimeImmutable($conditionInfo['value'][0]);
            ...
            foreach ($valueSqlList as $valueSql) {
                if (in_array($baseFieldName, $this->referenceFieldList)) {
                    ...
                    if (in_array('Users', $moduleList)) {
                        ...
                        $columnSql = getSqlForNameInDisplayFormat(['first_name' => $columnSqlTable . '.first_name',
                    ...
                    $concatSql = getSqlForNameInDisplayFormat(['first_name' => 'vtiger_users' . $ownerTableName . '.first_name',
    ...
    public function getOrderByColumn($fieldName)
    {
        ...
        if ($orderByFieldModel && $orderByFieldModel->getFieldDataType() == 'reference') {
            ...
            if (in_array('DocumentFolders', $referenceModules)) {
                ...
                $columnSqlTable = 'vtiger_users' . $parentReferenceField . $fieldName;
                $orderByColumn = getSqlForNameInDisplayFormat(['first_name' => $columnSqlTable . '.first_name',
                    'last_name' => $columnSqlTable . '.last_name'], 'Users');
            } else {
    ...
    public static function getSpecialDateConditionValue($comparator, $value, $type, $queryGenerator = false)
    {
        switch ($comparator) {
            case 'lessthandaysago': $days = $value;
                ...
                $today = date('Y-m-d');
                if ($queryGenerator) {
                    return ['comparator' => 'bw', 'date' => $olderDate . ',' . $today];
                }
            ...
            case 'morethandaysago': $days = $value - 1;
                ...
                return ['comparator' => 'l', 'date' => $olderDate];

            case 'inlessthan': $days = $value;
                $today = date('Y-m-d');
                $futureDate = date('Y-m-d', strtotime('+' . $days . ' days'));
                if ($queryGenerator) {
                    return ['comparator' => 'bw', 'date' => $today . ',' . $futureDate];
                }
            ...
            case 'inmorethan': $days = $value - 1;
                ...
                return ['comparator' => 'g', 'date' => $futureDate];

            case 'daysago': $olderDate = date('Y-m-d', strtotime('-' . $value . ' days'));
                if ($type == 'DT') {
                    return ['comparator' => 'c', 'date' => $olderDate];
                }

                return ['comparator' => 'e', 'date' => $olderDate];

            case 'dayslater': $futureDate = date('Y-m-d', strtotime('+' . $value . ' days'));
                if ($type == 'DT') {
                    return ['comparator' => 'c', 'date' => $futureDate];
                }

                return ['comparator' => 'e', 'date' => $futureDate];
            ...
            case 'lessthanhoursbefore': $currentTime = date('Y-m-d H:i:s');
                ...
                if ($queryGenerator) {
                    ...
                    return ['comparator' => 'bw', 'date' => $olderDateTime . ',' . $currentTime];
                }
            ...
            case 'lessthanhourslater': $currentTime = date('Y-m-d H:i:s');
                ...
                if ($queryGenerator) {
                    ...
                    return ['comparator' => 'bw', 'date' => $currentTime . ',' . $futureDateTime];
                }
            ...
            case 'morethanhoursbefore': $olderDateTime = date('Y-m-d H:i:s', strtotime('-' . $value . ' hours'));
                ...
                return ['comparator' => 'l', 'date' => $olderDateTime];
            ...
            case 'morethanhourslater': $futureDateTime = date('Y-m-d H:i:s', strtotime('+' . $value . ' hours'));
                ...
                return ['comparator' => 'g', 'date' => $futureDateTime];
