...
class DataTransform
{
        ...
        foreach ($references as $field => $typeList) {
            ...
            if (strtolower($meta->getEntityName()) == 'emails') {
                if (isset($row['parent_id']) && $row['parent_id'] !== null && strpos($row['parent_id'], '@') !== false) {
                    [$row['parent_id'], $fieldId] = explode('@', $row['parent_id']);
                }
            }
            ...
            if (!empty($row[$fieldName])) {
                if ($fieldObj->getFieldDataType() == 'currency') {
                    if ($fieldObj->getUIType() == '71') {
                        ...
                        $currencyConversionRate = $row['conversion_rate'] ?? 0;
    ...
    public static function sanitizeStringFields($row, $meta)
    {
        if (in_array($meta->getEntityName(), ['Groups', 'Currency', 'Tax', 'ProductTaxes'])) {
            foreach ($row as $field => $value) {
                if (is_string($value)) {
