...
class WebserviceField
{
    ...
    {
        $this->uitype = $row['uitype'] ?? 0;
        $this->blockId = $row['block'] ?? 0;
        $this->blockName = null;
        $this->tableName = $row['tablename'] ?? null;
        $this->columnName = $row['columnname'] ?? null;
        $this->fieldName = $row['fieldname'] ?? null;
        $this->fieldLabel = $row['fieldlabel'] ?? null;
        $this->displayType = $row['displaytype'] ?? null;
        $this->massEditable = (isset($row['masseditable']) && $row['masseditable'] === '1') ? true : false;
        $this->presence = $row['presence'] ?? null;
        $this->isunique = isset($row['isunique']) && $row['isunique'] ? true : false;
        $typeOfData = $row['typeofdata'] ?? null;
        ...
        $this->tabid = $row['tabid'] ?? 0;
        $this->fieldId = $row['fieldid'] ?? 0;
        $this->pearDB = $adb;
    ...
    public function getReferenceList($hideDisabledModules = true)
    {
        ...
        if ($this->referenceList === null) {
            ...
            if ($this->getUIType() != $this->genericUIType) {
                ...
                $params = [$fieldTypeData['fieldtypeid'] ?? 0];
            } else {
