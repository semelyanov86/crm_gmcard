...
if ($activevalue[0] == 1) {
    ...
    while ($myrow = $adb->fetch_array($result)) {
        ...
        $user_id = $myrow[4];
        if ($user_id != '') {
            $user_res = $adb->pquery('select user_name from vtiger_users where id=?', [$user_id]);
...
$activevalue = $adb->fetch_array($result);
if ($activevalue[0] == 1) {
    $result = $adb->pquery("SELECT sales_stage,amount,potentialid,potentialname FROM vtiger_potential inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_potential.potentialid where vtiger_crmentity.deleted=0 and sales_stage='Closed Won' and amount > 10000", []);
...
$activevalue = $adb->fetch_array($result);
if ($activevalue[0] == 1) {
    $result = $adb->pquery("SELECT vtiger_troubletickets.status,ticketid FROM vtiger_troubletickets INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_troubletickets.ticketid WHERE vtiger_crmentity.deleted='0' AND vtiger_troubletickets.status <> 'Completed' AND vtiger_troubletickets.status <> 'Closed'", []);
...
$activevalue = $adb->fetch_array($result);
if ($activevalue[0] == 1) {
    $result = $adb->pquery("SELECT count(*) as count FROM vtiger_troubletickets INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_troubletickets.ticketid WHERE vtiger_crmentity.deleted='0' AND vtiger_troubletickets.status <> 'Completed' AND vtiger_troubletickets.status <> 'Closed'", []);
...
$activevalue = $adb->fetch_array($result);
if ($activevalue[0] == 1) {
    $result = $adb->pquery('SELECT vtiger_products.productname FROM vtiger_products inner join vtiger_crmentity on vtiger_products.productid = vtiger_crmentity.crmid where vtiger_crmentity.deleted=0 and start_date like ?', [date('Y-m-d') . '%']);
...
$activevalue = $adb->fetch_array($result);
if ($activevalue[0] == 1) {
    $result = $adb->pquery('SELECT vtiger_products.productname from vtiger_products inner join vtiger_crmentity on vtiger_products.productid = vtiger_crmentity.crmid where vtiger_crmentity.deleted=0 and expiry_date like ?', [date('Y-m-d') . '%']);
