...
{
    public const PROMPT_ANY      = 1;
    public const PROMPT_OPTIONAL = 2;
    public const PROMPT_NUMERIC  = 3;
    public const PROMPT_ALPHANUMERIC = 4;
    public const PROMPT_NAME     = 5;
    public const PROMPT_LABEL    = 6;
    public const PROMPT_PATH     = 7;
...
class Vtiger_Tools_Console_ModuleController extends Vtiger_Tools_Console_Controller
{
    public function handle()
    {
        ...
        $entityfieldlabel = ucwords($this->prompt(sprintf(
            'Entity field (%s): ',
            $moduleInformation['entityfieldlabel'],
...
class Vtiger_Tools_Console_LayoutController extends Vtiger_Tools_Console_Controller
{
    ...
    public const VIEWERREGEX = '/\$viewer->(view|fetch)[^\(]*\(([^\)]+)/';
    public const RETURNTPLREGEX = '/(return)([ ]+[\'"]+[^;]+)/';
    public const TPLREGEX    = '/[\'"]([^\'"]+)/';
...
{
    public const BASE_LANG_PREFIX = 'en_us';
    ...
    protected function deploy($languageInformation)
    {
        ...
        if ($check && $db->num_rows($check)) {
            ...
            $db->pquery(
                'INSERT INTO vtiger_language (id,name,prefix,label,lastupdated,isdefault,active) VALUES(?,?,?,?,?,?,?)',
                [$db->getUniqueId('vtiger_language'), $languageInformation['label'], $languageInformation['prefix'],
