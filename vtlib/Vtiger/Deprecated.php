...
class Vtiger_Deprecated
{
    ...
    public static function getParentTab()
    {
        return '';
    ...
    public static function getParentTabFromModule($module)
    {
        return '';
    ...
    public static function copyValuesFromRequest($focus)
    {
    ...
    public static function createModuleGroupMetaFile()
    {
        global $adb;
    ...
    public static function getTemplateDetails($templateid)
    {
        global $adb;
        $returndata = [];
        $result = $adb->pquery('select body, subject from vtiger_emailtemplates where templateid=?', [$templateid]);
    ...
    public static function getTranslatedCurrencyString($str)
    {
        ...
        if (isset($app_currency_strings, $app_currency_strings[$str])) {
    ...
    public static function getIdOfCustomViewByNameAll($module)
    {
        ...
        return $cvidCache[$module] ?? '0';
    }

    public static function SaveTagCloudView($id = '')
    {
        global $adb;
        $tag_cloud_status = $_REQUEST['tagcloudview'] ?? false;
                ...
                if ($file != '.' && $file != '..' && $file != '.svn') {
                    ...
                    if (is_dir($path . $file)) {
                        ...
                        if (strripos($file, '.tpl.php') == (strlen($file) - strlen('.tpl.php'))) {
                            unlink($path . $file) or exit("couldn't delete {$path}{$file}<br />");
                        }
                    }
                }
            }
    ...
    public static function getModuleSequenceNumber($module, $recordId)
    {
        global $adb;
        switch ($module) {
            case 'Invoice':
