...
class Vtiger_PhpLogHandler
{
    ...
    public static function getErrorHandler($basedir, $logfile = null)
    {
        ...
        return static function ($errno, $errstr, $errfile, $errline) use ($display_errors, $log_errors, $basedir, $logfile, $logctx, $reqtm) {
            ...
            $errtype = $errno;
            switch ($errno) {
                case E_DEPRECATED:          $errtype = 'Deprecated';
                    break;
                case E_ERROR:               $errtype = 'Error';
                    break;
                case E_WARNING:             $errtype = 'Warning';
                    break;
                case E_PARSE:               $errtype = 'Parse Error';
                    break;
                case E_NOTICE:              $errtype = 'Notice';
                    break;
                case E_CORE_ERROR:          $errtype = 'Core Error';
                    break;
                case E_CORE_WARNING:        $errtype = 'Core Warning';
                    break;
                case E_COMPILE_ERROR:       $errtype = 'Compile Error';
                    break;
                case E_COMPILE_WARNING:     $errtype = 'Compile Warning';
                    break;
                case E_USER_ERROR:          $errtype = 'User Error';
                    break;
                case E_USER_WARNING:        $errtype = 'User Warning';
                    break;
                case E_USER_NOTICE:         $errtype = 'User Notice';
                    break;
                case E_STRICT:              $errtype = 'Strict Notice';
                    break;
                case E_RECOVERABLE_ERROR:   $errtype = 'Recoverable Error';
                    break;

                default:                    $errtype = "Unknown error ({$errno})";
                    break;
            }
    ...
    public static function getExceptionHandler($basedir, $logfile = null)
    {
        ...
        return static function (Throwable $e) use ($display_errors, $log_errors, $basedir, $logfile, $logctx, $reqtm) {
            // nothing todo return early.
            if (!$display_errors && !$log_errors && ($log_errors && !$logfile)) {
                return;
            }
