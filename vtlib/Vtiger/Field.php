...
class Vtiger_Field extends Vtiger_FieldBasic
{
    ...
    public function setPicklistValues($values)
    {
        ...
        if (!Vtiger_Utils::CheckTable($picklist_table)) {
            ...
            $adb->pquery('INSERT INTO vtiger_picklist (picklistid,name) VALUES(?,?)', [$new_picklistid, $this->name]);
        ...
        foreach ($values as $value) {
            ...
            ++$sortid;
            if (is_array($value)) {
                $adb->pquery("INSERT INTO {$picklist_table}({$picklist_idcol}, {$this->name}, presence, picklist_valueid,sortorderid,color) VALUES(?,?,?,?,?,?)", [$new_id, $value[0], $presence, $new_picklistvalueid, $sortid, $value[1]]);
            } else {
                $adb->pquery("INSERT INTO {$picklist_table}({$picklist_idcol}, {$this->name}, presence, picklist_valueid,sortorderid) VALUES(?,?,?,?,?)", [$new_id, $value, $presence, $new_picklistvalueid, $sortid]);
            }
    ...
    {
        global $adb;
        $pickListName_ids = ['recurring_frequency', 'payment_duration'];
        $picklist_table = 'vtiger_' . $this->name;
        $picklist_idcol = $this->name . 'id';
        ...
        // Add value to picklist now
        $maxSortIdResult = $adb->pquery("SELECT MAX(sortorderid) as maxsortid FROM {$picklist_table}", []);
        ...
        foreach ($values as $value) {
            ...
            $new_id = $adb->getUniqueId($picklist_table);
            if (is_array($value)) {
                $adb->pquery("INSERT INTO {$picklist_table}({$picklist_idcol}, {$this->name}, sortorderid, presence, color) VALUES(?,?,?,?,?)", [$new_id, $value[0], $sortid, $presence, $value[1]]);
            } else {
                $adb->pquery("INSERT INTO {$picklist_table}({$picklist_idcol}, {$this->name}, sortorderid, presence) VALUES(?,?,?,?)", [$new_id, $value, $sortid, $presence]);
            }
            $sortid = $sortid + 1;
        }
    }
    ...
    public function setRelatedModules($moduleNames)
    {
        ...
        foreach ($moduleNames as $relmodule => $relLabel) {
            ...
            $checkres = $adb->pquery(
                ...
                [$this->id, $this->getModuleName(), $relmodule],
            );
            ...
            $adb->pquery(
                'INSERT INTO vtiger_fieldmodulerel(fieldid, module, relmodule) VALUES(?,?,?)',
                [$this->id, $this->getModuleName(), $relmodule],
            );
    ...
    public static function getAllForBlock($blockInstance, $moduleInstance = false)
    {
        ...
        if ($moduleInstance) {
            ...
            $queryParams = [$blockInstance->id, $moduleInstance->id];
        } else {
            $query = 'SELECT * FROM vtiger_field WHERE block=? ORDER BY sequence';
            $queryParams = [$blockInstance->id];
        }
