...
class dUnzip2
{
    ...
    public function unzip($compressedFileName, $targetFileName = false, $applyChmod = 0o644)
    {
        if (!sizeof($this->compressedList)) {
        ...
        if ($applyChmod && $targetFileName) {
            ...
            @chmod($targetFileName, $applyChmod == 0o755 ? 0o644 : $applyChmod);
        }

        return $ret;
    }

    public function unzipAll($targetDir = false, $baseDir = '', $maintainStructure = true, $applyChmod = 0o755)
    {
        if ($targetDir === false) {
        ...
        for ($x = 0; $x < 1_024; ++$x) {
            ...
            if ($signature == $this->dirSignatureE) {
                ...
                while ($signature == $this->dirSignature) {
                    ...
                    $lastmod_dateY = bindec(substr($BINlastmod_date, 0, 7)) + 1_980;
